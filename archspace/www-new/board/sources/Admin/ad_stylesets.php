...
class ad_settings {
	...
	function export()
	{
		...
		$DB->query("SELECT * from acf_skins WHERE uid='".$IN['id']."'");
		...
		$DB->query("SELECT * from acf_macro_name WHERE set_id='".$row['macro_id']."'");
		...
		$DB->query("SELECT * from acf_css WHERE cssid='".$row['css_id']."'");
		...
		$DB->query("SELECT * from acf_tmpl_names WHERE skid='".$row['set_id']."'");
		...
		$DB->query("SELECT * from acf_templates WHERE tmid='".$row['tmpl_id']."'");
		...
		$DB->query("SELECT macro_replace, macro_value FROM acf_macro WHERE macro_set='".$row['macro_id']."'");
		...
		$groups = $DB->query("SELECT DISTINCT(group_name) FROM acf_skin_templates WHERE set_id='".$row['set_id']."'");
		...
		while ( $trow = $DB->fetch_row($groups) )
		{
			...
			$aq = $DB->query("SELECT * FROM acf_skin_templates WHERE set_id='".$row['set_id']."' AND group_name='".$trow['group_name']."'");
	...
	function mem_skins()
	{
		...
		$DB->query("SELECT sid FROM acf_skins WHERE uid='".$IN['oid']."'");
		$old = $DB->fetch_row();
		
		if ($IN['nid'] == 'n')
		{
			$DB->query("UPDATE acf_members SET skin=NULL WHERE skin='".$old['sid']."'");
		}
		else
		{
			$DB->query("SELECT sid FROM acf_skins WHERE uid='".$IN['nid']."'");
			$new = $DB->fetch_row();
			$DB->query("UPDATE acf_members SET skin='".$new['sid']."' WHERE skin='".$old['sid']."'");
		}
	...
	function remove()
	{
		...
		$DB->query("UPDATE acf_members SET skin='' WHERE skin='".$skin['sid']."'");
		
		// Update the forums DB
		
		$DB->query("SELECT id FROM acf_forums WHERE skin_id='".$skin['sid']."'");
		...
		if ( $DB->get_num_rows() )
		{
			...
			$DB->query("UPDATE acf_forums SET skin_id='' WHERE skin_id='".$skin['sid']."'");
		...
		$DB->query("DELETE FROM acf_skins WHERE uid='".$IN['id']."'");
	...
	function save_skin( $type='add' )
	{
		...
		if ($type == 'add')
		{
			...
			$DB->query("SELECT MAX(sid) as new_id FROM acf_skins");
			...
			$DB->query("INSERT INTO acf_skins (".$db_string['FIELD_NAMES'].") VALUES(".$db_string['FIELD_VALUES'].")");
			
			if ($IN['default_set'] == 1)
			{
				$DB->query("UPDATE acf_skins SET default_set=0 WHERE sid <> '".$barney['sid']."'");
			}
		...
		else
		{
			...
			$DB->query("UPDATE acf_skins SET $db_string WHERE uid='".$IN['id']."'");
			
			if ($IN['default_set'] == 1)
			{
				$DB->query("UPDATE acf_skins SET default_set=0 WHERE uid <> '".$IN['id']."'");
			}
	...
	function do_form( $type='add' )
	{
		...
		$DB->query("SELECT * FROM acf_macro_name");
		...
		$DB->query("SELECT cssid, css_name FROM acf_css");
		...
		$DB->query("SELECT tmid, name FROM acf_templates");
		...
		$DB->query("SELECT skid, skname FROM acf_tmpl_names");
	...
	function list_sets()
	{
		...
		$DB->query("SELECT id, name FROM acf_forums");
		...
		$DB->query("select acf_skins.*, count(acf_members.id) as mcount from acf_skins left join acf_members on(acf_members.skin=acf_skins.sid) where (acf_members.skin is not null or acf_skins.default_set = 1) group by acf_skins.sid order by acf_skins.sname");
		...
		if ( count($used_ids) < 1 )
		{
			$DB->query("SELECT * FROM acf_skins");
			
			$left_one = $DB->get_num_rows();
		}
		else if ( count($used_ids) > 0 )
		{
			$DB->query("SELECT * FROM acf_skins WHERE uid NOT IN(".implode(",",$used_ids).")");
