...
class ad_settings {
	...
	function add_splash()
	{
		...
		$DB->query("INSERT INTO acf_tmpl_names (skname, author, email, url) VALUES('$pack_name', '$author', '$email', '$url')");
	...
	function upload_form()
	{
		...
		$DB->query("SELECT * FROM acf_skin_templates WHERE set_id='".$IN['setid']."' AND group_name='".$IN['group']."'");
		...
		$DB->query("SELECT skname FROM acf_tmpl_names WHERE skid='".$IN['setid']."'");
	...
	function export()
	{
		...
		$DB->query("SELECT * FROM acf_tmpl_names WHERE skid='".$IN['id']."'");
		...
		$groups = $DB->query("SELECT DISTINCT(group_name) FROM acf_skin_templates WHERE set_id='".$IN['id']."'");
	...
	function download_group($return=0, $setid="", $group="")
	{
		...
		$aq = $DB->query("SELECT * FROM acf_skin_templates WHERE set_id='".$IN['setid']."' AND group_name='".$IN['group']."'");
	...
	function show_cats()
	{
		...
		$all_cats = $DB->query("select group_name, set_id, suid, count(group_name) as number_secs, group_name FROM acf_skin_templates WHERE set_id='".$IN['id']."' group by group_name");
		...
		while ( $group = $DB->fetch_row($all_cats) )
		{
			...
			if ($IN['expand'] == $group['suid'])
			{
				...
				$new_q = $DB->query("SELECT func_name, LENGTH(section_content) as sec_length, suid FROM acf_skin_templates WHERE set_id='{$IN['id']}' AND group_name='{$group['group_name']}'");
	...
	function edit_info()
	{
		...
		$DB->query("UPDATE acf_tmpl_names SET $db_string WHERE skid='".$IN['id']."'");
	...
	function tools_build_tmpl()
	{
		...
		$DB->query("SELECT group_name, func_name FROM acf_skin_templates WHERE set_id='".$IN['id']."'");
				...
				while (($filename = readdir($handle)) !== false)
				{
					if (($filename != ".") && ($filename != ".."))
					{
						...
						if ( preg_match( "/\.php$/", $filename ) )
						{
							...
							$DB->query("DELETE FROM acf_skin_templates WHERE set_id='".$IN['id']."' AND group_name='$name'");
							...
							foreach($functions as $fname => $ftext)
							{
								...
								$DB->query("INSERT INTO acf_skin_templates SET $db_update");
							}
	...
	function add_templates()
	{
		...
		$DB->query("INSERT INTO acf_tmpl_names (".$db_string['FIELD_NAMES'].") VALUES(".$db_string['FIELD_VALUES'].")");
		...
		if ($INFO['safe_mode_skins'] != 1)
		{
			...
			{
				$DB->query("DELETE FROM acf_tmpl_names WHERE skid='$new_id'");
		...
		$get = $DB->query("SELECT * FROM acf_skin_templates WHERE set_id='".$IN['id']."'");
		...
		while( $r = $DB->fetch_row($get) )
		{
			...
			$put = $DB->query("INSERT INTO acf_skin_templates ({$row['FIELD_NAMES']}) VALUES({$row['FIELD_VALUES']})");
		}
	...
	function remove()
	{
		...
		$DB->query("DELETE FROM acf_tmpl_names WHERE skid='".$IN['id']."'");
		$DB->query("DELETE FROM acf_skin_templates WHERE set_id='".$IN['id']."'");
	...
	function find_popular()
	{
		...
		$DB->query("SELECT * FROM acf_skin_templates WHERE $sql AND set_id=$id");
	...
	function edit_bit()
	{
		...
		$DB->query("SELECT * FROM acf_skin_templates WHERE suid IN (".implode(",",$ids).")");
		...
		$DB->query("SELECT * from acf_tmpl_names WHERE skid='".$setid."'");
	...
	function do_edit()
	{
		...
		$DB->query("SELECT * from acf_tmpl_names WHERE skid='".$template['set_id']."'");
		...
		foreach( $ids as $id )
		{
			...
			$DB->query("UPDATE acf_skin_templates SET $string WHERE suid=$id");
		...
		if ($INFO['safe_mode_skins'] != 1)
		{
			...
			$DB->query("SELECT * FROM acf_skin_templates WHERE group_name='$real_name' AND set_id='{$template['set_id']}'"); 
	...
	function list_current()
	{
		...
		$DB->query("SELECT DISTINCT(s.set_id), s.sname, t.skid, t.skname from acf_tmpl_names t, acf_skins s WHERE s.set_id=t.skid ORDER BY t.skname ASC");
		...
		if ( count($used_ids) > 0 )
		{
			...
			$DB->query("SELECT skid, skname FROM acf_tmpl_names WHERE skid NOT IN(".implode(",",$used_ids).")");
	...
	function process_upload($raw, $setid, $group, $isnew=0)
	{
		...
		if ($INFO['safe_mode_skins'] != 1)
		{
			...
			if (SAFE_MODE_ON == 1)
			{
				if ($isnew == 1)
				{
					$DB->query("DELETE FROM acf_tmpl_names WHERE skid='$setid'");
				}
				$ADMIN->error("Safe mode detected, you will need to change the board configuration to switch 'Safe Mode Skins' on. To do this, click on the 'Board Settings' menu and choose 'Basic Config' when the sub menu appears.");
			}
		...
		$DB->query("SELECT func_name, group_name FROM acf_skin_templates WHERE set_id='$setid'");
		...
		foreach( $master as $func_name => $data )
		{
			...
			if ($isnew == 0)
			{
				...
				$DB->query("UPDATE acf_skin_templates SET $str WHERE set_id='$setid' AND group_name='$group' AND func_name='".trim($data['func_name'])."'");
			}
			else
			{
				$data['content'] = str_replace( '\\n' , '\\\\\\n', $data['content'] );
				...
				$DB->query("INSERT INTO acf_skin_templates ({$str['FIELD_NAMES']}) VALUES({$str['FIELD_VALUES']})");
			}
		}
	...
	/*foreach($functions as $fname => $ftext)
	  {
		  ...
		  if ($insert == 0)
		  {
			  ...
			  $DB->query("UPDATE acf_skin_templates SET $db_update WHERE func_name='$fname' AND set_id='".$IN['id']."' AND group_name='$name'");
		  }
		  else
		  {
			  ...
			  //$DB->query("DELETE FROM acf_skin_templates WHERE func_name='$fname' AND set_id='".$IN['id']."' AND group_name='$name'");
