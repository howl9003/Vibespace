...
class ad_multimod
{
	...
	function do_delete()
	{
		...
		$DB->query("DELETE FROM acf_topic_mmod WHERE mm_id='".$IN['id']."'");
		...
		$DB->query("SELECT id,name,topic_mm_id FROM acf_forums WHERE topic_mm_id LIKE '%,$mm_id,%'");
		...
		if ( count( $final ) > 0 )
		{
			foreach( $final as $fid => $raw_mm_id )
			{
				...
				$DB->query("UPDATE acf_forums SET topic_mm_id='{$new_mm_id}' WHERE id=$fid");
			}
		}
	...
	function do_save($type='new')
	{
		...
		if ( $type == 'edit' )
		{
			...
			$DB->query("UPDATE acf_topic_mmod SET $str WHERE mm_id=$mm_id");
		...
		else
		{
			...
			$DB->query("INSERT INTO acf_topic_mmod ({$str['FIELD_NAMES']}) VALUES({$str['FIELD_VALUES']})");
		...
		$DB->query("SELECT id,name,topic_mm_id FROM acf_forums");
	...
	function do_form($type='new')
	{
		...
		if ( $type == 'edit' )
		{
			...
			$DB->query("SELECT * FROM acf_topic_mmod WHERE mm_id=$id");
		...
		$DB->query("SELECT f.id as forum_id, f.name as forum_name, f.position, f.read_perms, f.parent_id, f.subwrap, f.sub_can_post, f.redirect_on, f.topic_mm_id, c.id as cat_id, c.name as cat_name from acf_forums f, acf_categories c where c.id=f.category ORDER BY c.position, f.position");
	...
	function list_current()
	{
		...
		$DB->query("SELECT * FROM acf_topic_mmod ORDER BY mm_title");
