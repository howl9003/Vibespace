...
class ad_emaillogs
{
	...
	function view_email()
	{
		...
		$DB->query("SELECT email.*, m.id, m.name, mem.id as to_id, mem.name as to_name FROM acf_email_logs email
					 LEFT JOIN acf_members m ON (m.id=email.from_member_id)
					 LEFT JOIN acf_members mem ON (mem.id=email.to_member_id)
	...
	function remove_entries()
	{
		...
		if ( $IN['type'] == 'all' )
		{
			$DB->query("DELETE FROM acf_email_logs");
		}
		else
		{
			$ids = array();
			...
			$DB->query("DELETE FROM acf_email_logs WHERE email_id IN (".implode(',', $ids ).")");
		}
	...
	function list_current()
	{
		...
		if ( $IN['type'] != "" )
		{
			...
			switch( $IN['type'] )
			{
				...
				case 'name_from':
					...
					if ( $IN['match'] == 'loose' )
					{
						$DB->query("SELECT id,name FROM acf_members WHERE name LIKE '%{$string}%'");
					...
					{
						$DB->query("SELECT id,name FROM acf_members WHERE name='{$string}'");
		...
		$DB->query("SELECT count(email.email_id) as cnt FROM acf_email_logs email".$dbe);
		...
		$DB->query("SELECT email.*, m.id, m.name, mem.id as to_id, mem.name as to_name FROM acf_email_logs email
					 LEFT JOIN acf_members m ON (m.id=email.from_member_id)
					 LEFT JOIN acf_members mem ON (mem.id=email.to_member_id) $dbe
