...
class ad_modlogs {
	...
	function view()
	{
		...
		if ($IN['search_string'] == "")
		{
			$DB->query("SELECT COUNT(id) as count FROM acf_moderator_logs WHERE member_id='".$IN['mid']."'");
			$row = $DB->fetch_row();
			...
			$DB->query("SELECT m.*, f.id as forum_id, f.name FROM acf_moderator_logs m
					     LEFT JOIN acf_forums f ON(f.id=m.forum_id)
		...
		else
		{
			...
			$DB->query("SELECT COUNT(m.id) as count FROM acf_moderator_logs m WHERE $dbq");
			$row = $DB->fetch_row();
	...
	function remove()
	{
		...
		$DB->query("DELETE FROM acf_moderator_logs WHERE member_id='".$IN['mid']."'");
	...
	function list_current()
	{
		...
		$DB->query("SELECT m.*, count(m.id) as act_count from acf_moderator_logs m GROUP BY m.member_id ORDER BY act_count DESC");
