...
class Online {
	...
	function list_all() {
		...
		$DB->query("SELECT f.id, f.name, f.read_perms, f.password, c.id as cat_id, c.name as cat_name from acf_forums f, acf_categories c where c.id=f.category ORDER BY c.position, f.position");
		...
		$DB->query("SELECT COUNT(id) as total_sessions FROM acf_sessions s WHERE login_type <> 1 AND running_time > $t_time".$db_mem);
		$max = $DB->fetch_row();
		...
		$DB->query("SELECT s.id, s.in_forum, s.in_topic, s.member_name, s.member_id, s.ip_address, s.running_time, s.location,
				    ...
				    FROM acf_sessions s, acf_groups g
		...
		if ( count($tid_array) > 0 )
		{
			...
			$DB->query("SELECT tid, title FROM acf_topics WHERE tid IN ($tid_string)");
