...
class tracker {
    ...
    function tracker($is_sub=0) {
        ...
        if ($this->type == 'forum')
        {
        	$DB->query("SELECT f.id as fid, f.read_perms, f.password FROM acf_forums f WHERE f.id='".$ibforums->input['f']."'");
        }
        else
        {
        	$DB->query("SELECT t.tid, f.id as fid, f.read_perms, f.password FROM acf_topics t, acf_forums f WHERE t.tid='".$ibforums->input['t']."' AND t.forum_id=f.id");
        }
		...
		if ($this->type == 'forum')
		{
			$DB->query("SELECT frid from acf_forum_tracker WHERE forum_id='".$this->topic['fid']."' AND member_id='".$ibforums->member['id']."'");
		}
		else
		{
		...
		if ($this->type == 'forum')
		{
			...
			$DB->query("INSERT INTO acf_forum_tracker (" .$db_string['FIELD_NAMES']. ") VALUES (". $db_string['FIELD_VALUES'] .")");
