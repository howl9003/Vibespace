...
class  warn {
    ...
    function warn()
    {
        ...
        $DB->query("SELECT m.*, g.* FROM acf_members m
        ...
        if ( $ibforums->input['CODE'] == "" OR $ibforums->input['CODE'] == "dowarn" )
        {
			...
			if ( $this->times_a_day > 0 )
			{
				...
				$DB->query("SELECT * FROM acf_warn_logs WHERE wlog_mid={$this->warn_member['id']} AND wlog_date > $time_to_check");
	...
	function view_log()
	{
		...
		$DB->query("SELECT count(*) as cnt FROM acf_warn_logs WHERE wlog_mid={$this->warn_member['id']}");
		...
		else
		{
			$DB->query("SELECT l.*,  p.id as punisher_id, p.name as punisher_name
					     FROM acf_warn_logs l
					      LEFT JOIN acf_members p ON ( p.id=l.wlog_addedby )
	...
	function do_warn()
	{
		...
		$DB->query("INSERT INTO acf_warn_logs ({$dbs['FIELD_NAMES']}) VALUES({$dbs['FIELD_VALUES']})");
		...
		$DB->query("UPDATE acf_members SET $dbs WHERE id={$this->warn_member['id']}");
		...
		if ( $tid > 0 )
		{
			$DB->query("SELECT t.tid, t.title, f.id, f.name FROM acf_topics t, acf_forums f WHERE tid=$tid AND t.forum_id=f.id");
