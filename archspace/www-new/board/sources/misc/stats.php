...
class stats {
    ...
    function stats() {
 	...
 	function who_posted()
 	{
 		...
 		$DB->query("SELECT COUNT(p.pid) as pcount, p.author_id, p.author_name FROM acf_posts p
 	...
 	function check_access($tid)
    {
		...
		$DB->query("SELECT t.title as topic_title, f.read_perms, f.password, f.id from acf_forums f, acf_topics t WHERE t.tid=$tid and f.id=t.forum_id");
 	...
 	function show_leaders()
 	{
    	...
    	$DB->query("SELECT g_id from acf_groups WHERE g_is_supmod = 1");
    	...
    	$DB->query("SELECT m.id, m.name, m.email, m.hide_email, m.location, m.aim_name, m.icq_number, g.g_access_cp
    			    FROM acf_members m, acf_groups g
    	...
    	if ( count($sup_ids) > 0 )
    	{
    		...
    		$DB->query("SELECT id, name, email, hide_email, location, aim_name, icq_number from acf_members WHERE mgroup IN (".implode( ',', $sup_ids ).") and id NOT IN(".implode(',', $admin_ids).") ORDER BY name");
    	...
    	$DB->query("SELECT m2.id, m2.name, m2.email, m2.hide_email, m2.location, m2.aim_name, m2.icq_number,
    	            ...
    	            FROM acf_moderators mod
    	              LEFT JOIN acf_forums f ON(f.id=mod.forum_id)
    	              LEFT JOIN acf_categories c ON(c.id=f.category AND c.state != 0)
    	              LEFT JOIN acf_members m2 ON (mod.member_id=m2.id)
    	            ");
    	...
    	$DB->query("SELECT m.id, m.name, m.email, m.hide_email, m.location, m.aim_name, m.icq_number,
    	            ...
    	            FROM acf_moderators mod
    	              LEFT JOIN acf_forums f ON(f.id=mod.forum_id)
    	              LEFT JOIN acf_categories c ON(c.id=f.category AND c.state != 0)
    	              LEFT JOIN acf_members m ON ((mod.is_group=1 and mod.group_id=m.mgroup))
    	            ");
 	...
 	function show_today_posters()
 	{
    	...
    	$DB->query("SELECT COUNT(pid) as count FROM acf_posts WHERE post_date < $time_high and post_date > $time_low");
    	$todays_posts = $DB->fetch_row();
    	
    	if ($todays_posts['count'] > 0)
    	{
    	
			$DB->query("SELECT COUNT(p.pid) as tpost, m.id, m.name, m.joined, m.posts FROM acf_posts p, acf_members m "
