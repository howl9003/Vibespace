...
class UserCP {
    ...
    function UserCP()
    {
    	...
    	$DB->query("SELECT m.*, me.notes,me.ta_size,me.photo_type,me.photo_location,me.photo_dimensions 
    			   FROM acf_members m
    			   LEFT JOIN acf_member_extra me ON(me.id=m.id)
 	...
 	function remove_forum_tracker() {
 		...
 		{
 			$DB->query("DELETE FROM acf_forum_tracker WHERE member_id='".$this->member['id']."'");
 		}
 		else
 		{
 			$id = intval($ibforums->input['f']);
 			
 			$DB->query("DELETE FROM acf_forum_tracker WHERE member_id='".$this->member['id']."' AND forum_id='$id'");
 		}
 	...
 	function do_pass_change()
 	{
 		...
 		$DB->query("UPDATE acf_members SET password='$md5_pass' WHERE id='".$this->member['id']."'");
 	...
 	function do_email_change()
 	{
		...
		{
			$DB->query("SELECT id FROM acf_members WHERE email='".$email_one."'");
		...
		if ($ibforums->vars['reg_auth_type'])
		{
			...
			$DB->query("UPDATE acf_members SET mgroup=".$ibforums->vars['auth_group'].", email='$email_one' WHERE id=".$this->member['id']);
			
			// Update their session with the new member group
			
			if ($ibforums->session_id)
			{
				$DB->query("UPDATE acf_sessions SET member_name='', member_id=0, member_group=".$ibforums->vars['guest_group']." WHERE member_id=".$this->member['id']." and id='".$ibforums->session_id."'");
 			}
		...
		else
		{
			...
			$DB->query("UPDATE acf_members SET email='$email_one' WHERE id='".$this->member['id']."'");
 	...
 	function tracker() {
 		...
 		if ($ibforums->vars['subs_autoprune'] > 0)
 		{
			if ( time() % 2 )
			{
				...
				$DB->query("SELECT tr.trid FROM acf_tracker tr, acf_topics t WHERE t.tid=tr.topic_id AND t.last_post < '$time_limit'");
				...
				if (count($trids) > 0)
				{
					$DB->query("DELETE FROM acf_tracker WHERE trid IN (".implode(",",$trids).")");
				}
			}
 		...
 		$DB->query("SELECT s.trid, s.member_id, s.topic_id, s.last_sent, s.start_date as track_started, t.*, f.id as forum_id, f.name as forum_name, f.read_perms "
 		          ."FROM acf_tracker s, acf_topics t, acf_forums f "
 	...
 	function do_delete_tracker() {
 		...
 		if ( count($ids) > 0 )
 		{
 			$DB->query("DELETE from acf_tracker WHERE member_id='".$this->member['id']."' and trid IN (".implode( ",", $ids ).")");
 		}
 	...
 	function skin_langs() {
		...
		while ( $l = $DB->fetch_row() )
		{
 		...
 		if ($ibforums->vars['allow_skins'] == 1)
 		{
 			...
 			$DB->query("SELECT uid, sid, sname FROM acf_skins WHERE hidden <> 1");
 	...
 	function personal() {
		...
		$DB->query("SELECT * from acf_pfields_content WHERE member_id='".$ibforums->member['id']."'");
		...
		$DB->query("SELECT * from acf_pfields_data WHERE fedit=1 ORDER BY forder");
	...
	function do_personal() {
 	...
 	function splash() {
		...
		//---------------------------------------------
 		...
 		$DB->query("SELECT COUNT(*) as msg_total FROM acf_messages WHERE member_id='".$this->member['id']."'");
 		$total = $DB->fetch_row();
 	...
 	function update_notepad() {
		...
		//+----------------------------------------
 		...
 		$DB->query("SELECT id from acf_member_extra WHERE id='".$this->member['id']."'");
 		
 		if ( $DB->get_num_rows() )
 		{ 
 			$DB->query("UPDATE acf_member_extra SET notes='".$ibforums->input['notes']."', ta_size='".$ibforums->input['ta_size']."' WHERE id='".$this->member['id']."'");
 		}
 		else
 		{
 			$DB->query("INSERT INTO acf_member_extra (id, notes, ta_size) ".
