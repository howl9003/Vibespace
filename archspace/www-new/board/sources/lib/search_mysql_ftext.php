...
class search_lib extends Search
{
	...
	function do_simple_search()
	{
		...
		if ( ! $ibforums->input['sid'] )
		{
			...
			$DB->query("SELECT COUNT(*) as dracula FROM acf_posts p WHERE p.forum_id IN ($forums) AND MATCH(post) AGAINST ('$check_keywords' $boolean)");
			...
			$cache = "SELECT MATCH(post) AGAINST ('$check_keywords' $boolean) as score, t.tid, t.title, t.posts, t.views,
					  ...
					  FROM acf_posts p
					   LEFT JOIN acf_forums f ON (p.forum_id=f.id)
					   LEFT JOIN acf_topics t on (p.topic_id=t.tid)
		...
		else
		{
			...
			$DB->query("SELECT id, name FROM acf_categories");
	...
	function do_main_search()
	{
		...
		if ($this->is->prune > 0)
		{
		 ...
		 if ( $name_filter != "" )
		 {
			...
			{
				$sql_query = "SELECT id from acf_members WHERE lower(name)='".$name_filter."'";
			}
			else
			{
				$sql_query = "SELECT id from acf_members WHERE name like '%".$name_filter."%'";
			}
