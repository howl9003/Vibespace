...
class Poll {
	...
	function Poll() {
		...
		if (! $ibforums->input[t] ) {
   		...
   		$DB->query("SELECT f.allow_pollbump, t.*, p.pid as poll_id,p.choices,p.starter_id,p.votes from acf_polls p, acf_topics t, acf_forums f WHERE t.tid='".$ibforums->input['t']."' and p.tid=t.tid and t.forum_id=f.id");
		...
		$DB->query("SELECT member_id from acf_voters WHERE tid='".$this->topic['tid']."' and member_id='".$ibforums->member['id']."'");
		...
		$DB->query("INSERT INTO acf_voters (" .$db_string['FIELD_NAMES']. ") VALUES (". $db_string['FIELD_VALUES'] .")");
		...
		if (!$ibforums->input['nullvote'])
		{
        	...
        	if ($this->topic['allow_pollbump'])
        	{
				...
				$DB->query("UPDATE acf_topics SET ".
