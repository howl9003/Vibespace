...
class modfunctions
{
	...
	function topic_add_reply($post="", $tids=array(), $incpost=0)
	{
		...
		foreach( $tids as $row )
		{
			...
			if ( $tid and $fid )
			{
				...
				$DB->query("INSERT INTO acf_posts (" .$db_string['FIELD_NAMES']. ") VALUES (". $db_string['FIELD_VALUES'] .")");
				...
				$DB->query("UPDATE acf_topics SET ".$db_string.", posts=posts+1 WHERE tid=$tid");
		...
		if ( $add_posts > 0 )
		{
			$DB->query("UPDATE acf_stats SET TOTAL_REPLIES=TOTAL_REPLIES+".$add_posts);
			...
			$DB->query("UPDATE acf_members SET ".$pcount.$mgroup."last_post=".time()." WHERE id=".$ibforums->member['id']);
	...
	function topic_delete($id)
	{
		...
		$DB->query("DELETE FROM acf_polls WHERE tid".$tid);
		
		//------------------------------------
		// Remove polls assigned to this topic
		//------------------------------------
		
		$DB->query("DELETE FROM acf_voters WHERE tid".$tid);
		
		//------------------------------------
		// Remove polls assigned to this topic
		//------------------------------------
		
		$DB->query("DELETE FROM acf_topics WHERE tid".$tid);
		
		//------------------------------------
		// Remove polls assigned to this topic
		//------------------------------------
		
		$DB->query("SELECT attach_id, attach_hits, attach_file FROM acf_posts WHERE attach_id <> '' AND topic_id".$tid);
		...
		$DB->query("DELETE FROM acf_posts WHERE topic_id".$tid);
	...
	function topic_move($topics, $source, $moveto, $leavelink=0)
	{
		...
		$DB->query("UPDATE acf_topics SET forum_id=$moveto WHERE forum_id=$source AND tid".$tid);
		
		//----------------------------------
		// Update the posts
		//----------------------------------
		
		$DB->query("UPDATE acf_posts SET forum_id=$moveto WHERE forum_id=$source AND topic_id".$tid);
		
		//----------------------------------
		// Update the polls
		//----------------------------------
		
		$DB->query("UPDATE acf_polls SET forum_id=$moveto WHERE forum_id=$source AND tid".$tid);
		...
		if ( $leavelink != 0 )
		{
			$oq = $DB->query("SELECT * FROM acf_topics WHERE tid".$tid);
		...
		$DB->query("SELECT tr.*, m.id, m.mgroup, m.org_perm_id, f.read_perms, f.id, t.tid, g.g_id, g.g_perm_id
				     FROM acf_tracker tr
				     LEFT JOIN acf_topics t ON (tr.topic_id=t.tid)
				     LEFT JOIN acf_forums f ON (t.forum_id=f.id)
				     LEFT JOIN acf_members m on (m.id=tr.member_id)
		...
		{
			$DB->query("DELETE FROM acf_tracker WHERE trid IN(".implode(',', $trid_to_delete ).")");
		}
	...
	function stats_recount()
	{
		...
		$DB->query("SELECT COUNT(tid) as tcount from acf_topics WHERE approved=1");
		$topics = $DB->fetch_row();
		
		$DB->query("SELECT COUNT(pid) as pcount from acf_posts WHERE queued <> 1");
		$posts  = $DB->fetch_row();
		
		$posts = $posts['pcount'] - $topics['tcount'];
		
		$DB->query("UPDATE acf_stats SET TOTAL_TOPICS=".$topics['tcount'].", TOTAL_REPLIES=".$posts);
	}
	...
	function forum_recount_queue($fid="")
	{
		...
		$DB->query("SELECT count(tid) as cnt FROM acf_topics WHERE forum_id=$fid and approved <> 1");
		
		$topic = $DB->fetch_row();
		
		$tcount = intval( $topic['cnt'] );
		
		$DB->query("SELECT count(pid) as cnt FROM acf_posts WHERE forum_id=$fid and queued=1");
		...
		{
			$DB->query("UPDATE acf_forums SET has_mod_posts=1 WHERE id=$fid");
		}
		else
		{
			$DB->query("UPDATE acf_forums SET has_mod_posts=0 WHERE id=$fid");
		}
	...
	function stm_exec($id)
	{
		...
		if ( is_array($id) )
		{
			if ( count($id) > 0 )
			{
				$DB->query("UPDATE acf_topics SET $db_string WHERE tid IN(".implode( ",", $id ).")");
		...
		{
			$DB->query("UPDATE acf_topics SET $db_string WHERE tid=".intval($id));
		}
		else
		{
			return FALSE;
	...
	{
		$sql = "SELECT tid FROM acf_topics WHERE approved=1 and forum_id=".intval($forum_id);
