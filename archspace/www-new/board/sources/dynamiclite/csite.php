...
class click_site {
    ...
    function click_site()
    {
        ...
        $DB->query("SELECT cs_key, cs_value FROM acf_cache_store WHERE cs_key IN ('csite_nav_contents', 'csite_fav_contents')");
		...
		$DB->query("SELECT id, read_perms, password FROM acf_forums");
        ...
        if ( $ibforums->vars['vb_configured'] )
        {
			$DB->query("SELECT t.*, f.read_perms, f.use_html, p.*, m.avatar, m.view_avs, m.avatar_size, m.id as member_id, m.name as member_name, m.mgroup, g.g_id, g.g_dohtml
						FROM acf_posts p
						 LEFT JOIN acf_topics t on (t.tid=p.topic_id and t.approved=1 and t.moved_to IS NULL)
        ...
        else
        {
			$DB->query("SELECT t.*, f.read_perms, f.use_html, p.*, m.avatar, m.view_avs, m.avatar_size,
						...
						FROM acf_topics t
						 LEFT JOIN acf_members m ON (t.starter_id=m.id)
						 LEFT JOIN acf_groups g ON (g.g_id=m.mgroup)
						 LEFT JOIN acf_posts p ON (p.topic_id=t.tid AND p.new_topic=1)
						 LEFT JOIN acf_forums f on (f.id=p.forum_id)
 	...
 	function _do_output()
 	{
        ...
        //------------------------------------------
      	...
      	$DB->query("SELECT macro_value, macro_replace FROM acf_macro WHERE macro_set={$ibforums->skin['macro_id']}");
 	...
 	function _show_poll()
 	{
		...
		{
			$extra = "LEFT JOIN acf_voters v ON (v.member_id={$ibforums->member['id']} and v.tid=t.tid)";
 	...
 	function _show_changeskin()
 	{
 		...
 		$DB->query("SELECT sname, sid, default_set FROM acf_skins where hidden=0");
