...
CBattleFleet::init_common()
{
	...
	switch( mCommander->get_special_ability() ){
		case CAdmiral::SA_ENGINEERING_SPECIALIST :
			NewEffect = new CFleetEffect( CFleetEffect::FE_ARMOR_DR, mCommander->get_level()*2, CFleetEffect::AT_PROPORTIONAL );
			add_static_effect( NewEffect );
		...
		case CAdmiral::SA_SHIELD_SYSTEM_SPECIALIST :
			NewEffect = new CFleetEffect( CFleetEffect::FE_SHIELD_STRENGTH, 10+mCommander->get_level()*2, CFleetEffect::AT_PROPORTIONAL );
			add_static_effect( NewEffect );
			NewEffect = new CFleetEffect( CFleetEffect::FE_SHIELD_RECHARGE_RATE, 10+mCommander->get_level()*2, CFleetEffect::AT_PROPORTIONAL );
			add_static_effect( NewEffect );
			NewEffect = new CFleetEffect( CFleetEffect::FE_IMPENETRABLE_SHIELD, 10+mCommander->get_level()*2, CFleetEffect::AT_ABSOLUTE );
			add_static_effect( NewEffect );
			if( mCommander->get_level() <= 12 ){
	...
	switch( mCommander->get_racial_ability() ){
		...
		case CAdmiral::RA_INTUITION :
			...
			NewEffect = new CFleetEffect( CFleetEffect::FE_CRITICAL_HIT, 5 + (mCommander->get_level() / 2), CFleetEffect::AT_ABSOLUTE );
			add_static_effect( NewEffect );
			break;
		case CAdmiral::RA_LONE_WOLF:
			...
			NewEffect = new CFleetEffect( CFleetEffect::FE_MOBILITY, 5 + mCommander->get_level(), CFleetEffect::AT_PROPORTIONAL );
 			add_static_effect( NewEffect );
 			NewEffect = new CFleetEffect( CFleetEffect::FE_STEALTH,  mCommander->get_level(), CFleetEffect::AT_PROPORTIONAL );
 			add_static_effect( NewEffect );
		...
		case CAdmiral::RA_XENOPHOBIC_FANATIC :
			...
			NewEffect = new CFleetEffect( CFleetEffect::FE_MOBILITY, 5 + (mCommander->get_level()/2), CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
		...
		case CAdmiral::RA_MENTAL_GIANT :
			mBerserkModifier -= 5;
			mMoraleModifier -= 5;
			NewEffect = new CFleetEffect( CFleetEffect::FE_PSI_ATTACK, mCommander->get_level(), CFleetEffect::AT_PROPORTIONAL );
			add_static_effect( NewEffect );
		...
		case CAdmiral::RA_PSYCHIC_PROGENITOR :
			...
			} else if( mCommander->get_level() >= 7 ){
				mAttribute += WEAK_CLOAKING;
			}
			break;
		...
		case CAdmiral::RA_LYING_DORMANT :
			mAttribute += WEAK_CLOAKING;
			NewEffect = new CFleetEffect( CFleetEffect::FE_STEALTH, mCommander->get_level()*3+10, CFleetEffect::AT_PROPORTIONAL );
			add_static_effect( NewEffect );
			break;
		...
		case CAdmiral::RA_JURY_RIGGER :
			NewEffect = new CFleetEffect( CFleetEffect::FE_REPAIR, mCommander->get_level() / 4, CFleetEffect::AT_PROPORTIONAL );
			NewEffect->set_period( 10 );
			add_static_effect( NewEffect );
			break;
		case CAdmiral::RA_PATTERN_BROADCASTER :
			...
			add_static_effect( NewEffect );
			NewEffect = new CFleetEffect( CFleetEffect::FE_DR_AGAINST_MISSILE, 20+mCommander->get_level()*2, CFleetEffect::AT_PROPORTIONAL );
			add_static_effect( NewEffect );
			break;
		case CAdmiral::RA_FAMOUS_PRIVATEER :
 			...
 			NewEffect = new CFleetEffect( CFleetEffect::FE_STEALTH, 10 + mCommander->get_level(), CFleetEffect::AT_PROPORTIONAL );
 			add_static_effect( NewEffect );
			if( mCommander->get_level() >= 7 ){
				mAttribute += WEAK_CLOAKING;
			}
		...
		case CAdmiral::RA_COMMERCE_KING :
			NewEffect = new CFleetEffect( CFleetEffect::FE_HP, mCommander->get_level(), CFleetEffect::AT_PROPORTIONAL);
			add_static_effect( NewEffect );
			NewEffect = new CFleetEffect( CFleetEffect::FE_SPEED, mCommander->get_level(), CFleetEffect::AT_PROPORTIONAL);
			add_static_effect( NewEffect );
			NewEffect = new CFleetEffect( CFleetEffect::FE_MOBILITY, mCommander->get_level(), CFleetEffect::AT_PROPORTIONAL);
			add_static_effect( NewEffect );
			break;
		case CAdmiral::RA_RETREAT_SHIELD :
			mBerserkModifier -= 5;
			mMoraleModifier -= 10;
			NewEffect = new CFleetEffect( CFleetEffect::FE_SHIELD_STRENGTH, mCommander->get_level()*5, CFleetEffect::AT_PROPORTIONAL );
 			add_static_effect( NewEffect );
			NewEffect = new CFleetEffect( CFleetEffect::FE_IMPENETRABLE_SHIELD, mCommander->get_level()*5, CFleetEffect::AT_ABSOLUTE );
 			add_static_effect( NewEffect );
			if( mCommander->get_level() <= 5 ){
		...
		case CAdmiral::RA_RIGID_THINKING :
			mBerserkModifier -= 5;
			mMoraleModifier -= 10;
			NewEffect = new CFleetEffect( CFleetEffect::FE_PSI_DEFENSE, 10+mCommander->get_level()*2, CFleetEffect::AT_PROPORTIONAL );
 			add_static_effect( NewEffect );
		...
		case CAdmiral::RA_SCAVENGER :
			NewEffect = new CFleetEffect( CFleetEffect::FE_REPAIR_SPEED, 10+mCommander->get_level()*2, CFleetEffect::AT_PROPORTIONAL );
 			add_static_effect( NewEffect );
 			NewEffect = new CFleetEffect( CFleetEffect::FE_ARMOR_DR, mCommander->get_level(), CFleetEffect::AT_PROPORTIONAL );
			add_static_effect( NewEffect );
			if( mCommander->get_level() <= 10 ){
		...
		case CAdmiral::RA_BLITZKRIEG :
			NewEffect = new CFleetEffect( CFleetEffect::FE_DAMAGE, 5 + (mCommander->get_level()/2), CFleetEffect::AT_PROPORTIONAL );
			add_static_effect( NewEffect );
			NewEffect = new CFleetEffect( CFleetEffect::FE_CRITICAL_HIT, mCommander->get_level()/4, CFleetEffect::AT_ABSOLUTE );
 			add_static_effect( NewEffect );
			break;
...
float
CBattleFleet::change_morale( float aMorale )
{
	mMorale += aMorale;
...
/*			if( is_psi_race() && aTurret->has_effect( CFleetEffect::WE_PSI_EMPOWER ) )
			...
			if( ShieldDistorted == true ){
				...
				if( aEnemy->get_shield_strength(Index) < Dam ){
					...
					*aTotalDamage += aEnemy->get_shield_strength(Index);
					...
					{
						*aTotalDamage += aEnemy->get_hp(Index);
						RealDam = aEnemy->get_hp(Index);
					}
					aEnemy->change_hp( Index, -Dam );
				} else {
			...
			float
			 ...
			 SLOG ("MORALE TEST: aTotalDamage: %d , Enemy_hp: %d , MoraleDrop: %f", aTotalDamage, aEnemy->get_total_hp(), MoraleDrop);
