...
CCouncilTable::remove_council(int aID)
{
	...
	if ( (Host != NULL && User != NULL && Password != NULL && Database != NULL) && MySQL.open(Host, User, Password, Database))	
    {
	   CString
		      Query;	
       Query.clear();           
       Query.format( "SELECT group_id FROM asbb_groups WHERE group_name='%d'", 
                     Council->get_id());
       ...
       if (MySQL.fetch_row())
       {
        ...
        if (aGroupIDString && (aGroupID = atoi(aGroupIDString)))
        {
         MySQL.free_result();
         Query.clear();
         Query.format( "DELETE FROM asbb_groups WHERE group_name='%d'",
                       Council->get_id());
         MySQL.query( (char*)Query );
         MySQL.use_result();
         MySQL.free_result();
         Query.clear();
         Query.format( "DELETE FROM asbb_auth_access WHERE group_id=%d",
                        aGroupID);
         MySQL.query( (char*)Query );
         MySQL.use_result();
         MySQL.free_result();
