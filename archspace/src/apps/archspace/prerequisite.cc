...
void
CPrerequisiteList::init()
{
	mPrerequisiteList = new std::vector<CPrerequisite>();
}
...
//	assert(*aPrerequisite);
	mPrerequisiteList->push_back(*aPrerequisite);
}
...
{	
	bool isRace = false;
	bool aRace = false;
	if (!mPrerequisiteList) return true;
	for(std::vector<CPrerequisite>::iterator current = mPrerequisiteList->begin(); 
		current < mPrerequisiteList->end(); current++)
	{
...
CPrerequisiteList::tech_description()
{
	...
	bool comma = false;
	if (!mPrerequisiteList) return "NONE";
	for(std::vector<CPrerequisite>::iterator current = mPrerequisiteList->begin(); 
		current < mPrerequisiteList->end(); current++)
	{
...
CPrerequisiteList::race_description()
{
	...
	bool comma = false;
	if (!mPrerequisiteList) return "ALL RACES";
	for(std::vector<CPrerequisite>::iterator current = mPrerequisiteList->begin(); 
		current < mPrerequisiteList->end(); current++)
	{
		if (current->get_type()==CPrerequisite::RT_RACE)
