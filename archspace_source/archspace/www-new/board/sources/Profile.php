...
class Profile {
    ...
    function Profile() {
 	...
 	function show_card()
 	{
    	...
    	$DB->query("SELECT * FROM acf_members WHERE id=$id");
    ...
    function load_photo($id)
    {
    	...
    	$DB->query("SELECT m.id, m.name, me.photo_type, me.photo_location, me.photo_dimensions FROM acf_member_extra me
    	              LEFT JOIN acf_members m ON me.id=m.id
    			    WHERE m.id=$id");
 	...
 	function view_profile()
 	{
    	...
    	$DB->query("SELECT m.*, g.g_id, g.g_title as group_title FROM acf_members m, acf_groups g WHERE m.id='$id' and m.mgroup=g.g_id");
    	$member = $DB->fetch_row();
    	...
    	$DB->query("SELECT DISTINCT(p.forum_id), f.name, COUNT(p.author_id) as f_posts FROM acf_posts p, acf_forums f ".
    	...
    	$DB->query("SELECT COUNT(pid) as total_posts FROM acf_posts WHERE author_id='".$member['id']."'");
    	
    	$total_posts = $DB->fetch_row();
    	
    	$DB->query("SELECT TOTAL_TOPICS, TOTAL_REPLIES FROM acf_stats");
        ...
        $DB->query("SELECT * from acf_pfields_content WHERE member_id='".$member['id']."'");
		...
		$DB->query("SELECT * from acf_pfields_data $query_extra ORDER BY forder");
    	...
    	if ( $ibforums->vars['warn_on'] and ( ! stristr( $ibforums->vars['warn_protected'], ','.$member['mgroup'].',' ) ) )
		{
			if ($ibforums->member['id'])
			{
				...
				{
					$DB->query("SELECT * FROM acf_moderators WHERE (member_id=".$ibforums->member['id']." OR (is_group=1 AND group_id=".$ibforums->member['mgroup']."))");
