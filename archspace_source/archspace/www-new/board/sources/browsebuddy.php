    ...
    function buddy() {
 	...
 	function splash() {
 		...
 		else
 		{
 			...
 			$DB->query("SELECT COUNT(pid) as posts FROM acf_posts WHERE post_date > '".$ibforums->member['last_visit']."' AND queued <> 1 AND forum_id IN($forum_string)");
 			...
 			$DB->query("SELECT COUNT(tid) as replies
 						FROM acf_topics WHERE last_post > '".$ibforums->member['last_visit']."'
