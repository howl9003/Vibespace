...
class Messenger {
    ...
    function Messenger() {
    	...
    	$DB->query("SELECT vdirs, msg_total, new_msg, msg_msg_id FROM acf_members WHERE id='".$this->member['id']."'");
 	...
 	function pm_popup() {
 		...
 		$DB->query("SELECT m.name, msg.title, msg.msg_date, msg.from_id FROM acf_members m, acf_messages msg ".
 	...
 	function start_empty_folders()
 	{
 		...
 		$DB->query("SELECT msg_id, vid FROM acf_messages WHERE member_id={$ibforums->member['id']} LIMIT 0,1000");
 	...
 	function end_empty_folders()
 	{
 		...
 		$DB->query("DELETE FROM acf_messages WHERE member_id={$ibforums->member['id']} AND vid IN('".implode("','", $ids)."')".$qe);
 		
 		$DB->query("SELECT COUNT(*) as msg_total FROM acf_messages WHERE member_id=".$this->member['id']." AND vid <> 'unsent'");
 		
 		$total = $DB->fetch_row();
 		
 		$total['msg_total'] = $total['msg_total'] > 0 ? $total['msg_total'] : 0;
 		
 		$DB->query("UPDATE acf_members SET msg_total=".$total['msg_total']." WHERE id=".$this->member['id']);
 	...
 	function do_archive()
 	{
		...
		$archive_query = $DB->query("SELECT mg.*, m.name, m.id, mr.id as rec_id, mr.name as rec_name
									 FROM acf_messages mg
									   LEFT JOIN acf_members m ON (m.id=mg.from_id)
									   LEFT JOIN acf_members mr ON (mr.id=mg.recipient_id)
 		...
 		if ( $DB->get_num_rows($archive_query) )
 		{
 			while ( $r = $DB->fetch_row($archive_query) )
 			{
 				...
 				//$from_member = $DB->query("SELECT id, name FROM acf_members WHERE id='".$r['from_id']."'");
			...
			if ($ibforums->input['delete'] == 'yes')
			{
				...
				if (!empty($msg_str))
				{
					$DB->query("DELETE FROM acf_messages WHERE msg_id IN ($msg_str)");
					
					$DB->query("UPDATE acf_members SET msg_total=msg_total-$num_msg WHERE id ='".$this->member['id']."'");
				}
 	...
 	function do_prefs() {
 		...
 		$DB->query("UPDATE acf_members SET vdirs='$v_dir' WHERE id='".$this->member['id']."'");
 	...
 	function del_member() {
 		...
 		$DB->query("DELETE FROM acf_contacts WHERE member_id='".$this->member['id']."' AND contact_id='".$ibforums->input['MID']."'");
	...
	/**********************************************************/
 	...
 	function edit_member() {
 		...
 		$DB->query("SELECT * FROM acf_contacts WHERE member_id='".$this->member['id']."' AND contact_id='".$ibforums->input['MID']."'");
 		$memb = $DB->fetch_row();
	...
	/**********************************************************/
 	...
 	function do_edit() {
 		...
 		$DB->query("UPDATE acf_contacts SET contact_desc='".$ibforums->input['mem_desc']."', allow_msg='".$ibforums->input['allow_msg']."' WHERE id='".$memb['id']."'");
 	...
 	function contact() {
 		...
 		$DB->query("SELECT * FROM acf_contacts WHERE member_id='".$this->member['id']."' ORDER BY contact_name ASC");
 		...
 		if ($ibforums->input['MID'])
 		{
 			if ( preg_match( "/^(\d+)$/", $ibforums->input['MID'] ) )
 			{
 				$DB->query("SELECT name, id FROM acf_members WHERE id='".$ibforums->input['MID']."'");
 				$memb = $DB->fetch_row();
 	...
 	function add_member() {
 		...
 		$DB->query("SELECT name, id FROM acf_members WHERE LOWER(name)='".$ibforums->input['mem_name']."'");
 		$memb = $DB->fetch_row();
 		...
 		$DB->query("SELECT contact_id FROM acf_contacts WHERE member_id='".$this->member['id']."' AND contact_id='".$memb['id']."'");
		...
		$DB->query("INSERT INTO acf_contacts (" .$db_string['FIELD_NAMES']. ") VALUES (". $db_string['FIELD_VALUES'] .")");
		unset($db_string);
 	...
 	function multi_act() {
 		...
 		if ( $affected_ids > 0 )
 		{
 			...
 			{
 				$DB->query("DELETE FROM acf_messages WHERE member_id='".$this->member['id']."' AND msg_id IN ($id_string)");
 				...
 				{
					$DB->query("UPDATE acf_members SET msg_total=msg_total-$affected_ids WHERE id='".$this->member['id']."'");
 			...
 			{
 				$DB->query("UPDATE acf_messages SET vid='".$this->vid."' WHERE member_id='".$this->member['id']."' AND msg_id IN ($id_string)");
 				$std->boink_it($this->base_url."act=Msg&CODE=01&VID={$this->vid}");
 				exit;
 			}
 			else
 	...
 	function end_tracking() {
 		...
 		if ( $affected_ids > 0 )
 		{
 			...
 			$DB->query("UPDATE acf_messages SET tracking=0 WHERE tracking=1 AND read_state=1 AND from_id='".$this->member['id']."' AND msg_id IN ($id_string)");
 	...
 	function del_tracked() {
 		...
 		if ( $affected_ids > 0 )
 		{
 			...
 			$DB->query("DELETE FROM acf_messages WHERE tracking=1 AND read_state=0 AND from_id='".$this->member['id']."' AND msg_id IN ($id_string)");
 	...
 	function delete() {
 		...
 		$DB->query("DELETE FROM acf_messages WHERE msg_id='".$ibforums->input['MSID']."' AND member_id='".$this->member['id']."'");
 		
 		$DB->query("UPDATE acf_members SET msg_total=msg_total-1 WHERE id='".$this->member['id']."'");
 	...
 	function view_msg() {
 		...
 		$DB->query("SELECT * FROM acf_messages WHERE msg_id='".$ibforums->input['MSID']."' and member_id='".$this->member['id']."'");
 		$msg = $DB->fetch_row();
 		...
 		{
 			$DB->query("UPDATE acf_members SET new_msg=new_msg-1 WHERE id='".$this->member['id']."'");
 		}
		...
		//--------------------------------------
 		...
 		if ($msg['read_state'] < 1)
 		{
 			$DB->query("UPDATE acf_messages SET read_state=1, read_date='".time()."' WHERE msg_id='".$ibforums->input['MSID']."'");
 		}
        ...
        $this->parser = new post_parser();
 		...
 		$DB->query("SELECT g.*, m.* ".
 				   "FROM acf_members m, acf_groups g WHERE id='".$msg['from_id']."' and g.g_id=m.mgroup");
 	...
 	function send_form($preview=0, $errors="") {
		...
		if (!empty($ibforums->input['MID']))
		{
 		...
 		{
 			$DB->query("SELECT message, title from acf_messages WHERE msg_id='".$ibforums->input['MSID']."' and member_id='".$this->member['id']."'");
 	...
 	function edit_saved() {
    	...
    	//--------------------------------------
 		...
 		$DB->query("SELECT mg.*, m.name as to_name, m.id as to_id from acf_messages mg, acf_members m WHERE msg_id='".$ibforums->input['MSID']."' and member_id='".$this->member['id']."' AND m.id=mg.recipient_id");
 		$msg = $DB->fetch_row();
 	...
 	function send_msg() {
		...
		$this->email = new emailer();
 		...
 		$DB->query("SELECT name, id, view_pop, mgroup, email_pm, language, email FROM acf_members WHERE ".$query);
 		...
 		if ($ibforums->input['save'] != "")
 		{
			...
			if ($ibforums->input['OID'])
			{
				...
				$DB->query("SELECT msg_id from acf_messages WHERE msg_id='".$ibforums->input['OID']."' AND member_id='".$ibforums->member['id']."' AND vid='unsent'");
				...
				if ( $DB->get_num_rows() )
				{
					...
					$DB->query("UPDATE acf_messages SET $db_string WHERE msg_id='".$ibforums->input['OID']."'");
				}
			}
			...
			if ($saved == 0)
			{
				...
				$DB->query("INSERT INTO acf_messages (" .$db_string['FIELD_NAMES']. ") VALUES (". $db_string['FIELD_VALUES'] .")");
			}
 		...
 		$DB->query("SELECT m.msg_total, g.g_use_pm, g.g_max_messages FROM acf_groups g, acf_members m WHERE m.id='".$to_member['id']."' AND g.g_id=m.mgroup");
		...
		if ( (($to_msg_stats['msg_total']) >= $to_msg_stats['g_max_messages']) and ($to_msg_stats['g_max_messages'] > 0) )
		{
 		...
 		$DB->query("SELECT contact_id, allow_msg FROM acf_contacts WHERE contact_id='".$this->member['id']."' AND member_id='".$to_member['id']."'");
 		...
 		if ($ibforums->member['g_max_mass_pm'] > 0)
 		{
 			...
 			if (isset($ibforums->input['carbon_copy']) and $ibforums->input['carbon_copy'] != "")
 			{
 				...
 				if ( is_array($new_array) and count($new_array) > 0 )
 				{
 					...
 					$DB->query("SELECT m.id, m.name, m.msg_total, m.view_pop, m.email_pm, m.language, m.email, g.g_max_messages, g.g_use_pm FROM acf_members m, acf_groups g ".
 				    ...
 				    else
 				    {
 						...
 						$DB->query("SELECT m.name, c.allow_msg FROM acf_members m, acf_contacts c WHERE contact_id='".$ibforums->member['id']."' AND member_id IN (".implode(",",$cc_id_array).") AND m.id=c.member_id");
 		...
 		foreach ($cc_array as $user_id => $to_member)
 		{
			...
			$DB->query("UPDATE acf_members SET ".
		...
		if ($ibforums->input['OID'])
		{
			...
			$DB->query("DELETE from acf_messages WHERE msg_id='".$ibforums->input['OID']."' AND member_id='".$ibforums->member['id']."' AND vid='unsent'");
		}	
 	...
 	function msg_list() {
 		...
 		$DB->query("SELECT COUNT(*) as msg_total FROM acf_messages WHERE member_id='".$this->member['id']."' AND vid='{$this->vid}'");
 		...
 		if ($this->vid == 'sent')
 		{
 			...
 			$DB->query("SELECT m.*, mp.name as from_name FROM acf_messages m, acf_members mp WHERE member_id='".$this->member['id']."' AND vid='".$this->vid."' and mp.id=m.recipient_id ORDER BY $sort_key LIMIT $start, $p_end");
 		
 		}
 		else
 		{
 			$DB->query("SELECT m.*, mp.name as from_name FROM acf_messages m, acf_members mp WHERE member_id='".$this->member['id']."' AND vid='".$this->vid."' and mp.id=m.from_id ORDER BY $sort_key LIMIT $start, $p_end");
 		}
...
//+------------------------------------------------------------------------------------
	...
	/**********************************************************/
 	...
 	function view_saved() {
 		...
 		$DB->query("SELECT m.*, mp.name as to_name FROM acf_messages m, acf_members mp WHERE member_id='".$this->member['id']."' AND vid='unsent' and mp.id=m.recipient_id ORDER BY msg_date DESC");
 	...
 	function show_tracking() {
 		...
 		$DB->query("SELECT m.*, mp.name as to_name, mp.id as memid
 					  FROM acf_messages m, acf_members mp
 					WHERE m.tracking=1
 					   AND m.read_state=1
 		...
 		$DB->query("SELECT m.*, mp.name as to_name, mp.id as memid FROM acf_messages m, acf_members mp WHERE m.tracking=1 AND m.read_state=0 AND m.from_id='".$this->member['id']."' AND m.member_id=mp.id ORDER BY msg_date DESC");
	...
	function parse_member($member=array(), $row=array()) {
 	...
 	function html_add_smilie_box()
 	{
		...
		$DB->query("SELECT * FROM acf_emoticons WHERE clickable='1'");
	...
	function build_contact_list()
	{
		...
		$contacts = "";
 		...
 		$DB->query("SELECT * FROM acf_contacts WHERE member_id='".$this->member['id']."' ORDER BY contact_name");
