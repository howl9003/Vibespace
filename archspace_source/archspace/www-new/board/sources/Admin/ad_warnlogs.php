...
class ad_warnlogs {
	...
	function view_note()
	{
		...
		$DB->query("SELECT l.*, m.id as a_id, m.name as a_name, p.id as p_id, p.name as p_name
						FROM acf_warn_logs l
						  LEFT JOIN acf_members m ON (m.id=l.wlog_mid)
	...
	function view()
	{
		...
		{
			$DB->query("SELECT COUNT(wlog_id) as count FROM acf_warn_logs WHERE wlog_mid='".$IN['mid']."'");
			$row = $DB->fetch_row();
		...
		else
		{
			...
			$DB->query("SELECT COUNT(l.wlog_id) as count FROM acf_warn_logs l WHERE $dbq");
			$row = $DB->fetch_row();
	...
	function remove()
	{
		...
		$DB->query("DELETE FROM acf_warn_logs WHERE wlog_mid={$IN['mid']}");
	...
	function list_current()
	{
		...
		$DB->query("SELECT l.*, m.name, count(l.wlog_mid) as act_count from acf_warn_logs l, acf_members m WHERE m.id=l.wlog_mid GROUP BY l.wlog_mid ORDER BY act_count DESC");
