...
class ad_settings {
	...
	function do_resynch()
	{
		...
		$DB->query("SELECT cssid, css_text, css_name, css_comments FROM acf_css WHERE cssid='".$IN['id']."'");
		...
		if ( $IN['favour'] == 'cache' )
		{
			...
			$DB->query("UPDATE acf_css SET $dbr WHERE cssid='".$IN['id']."'");
		}
		else
		{
			$cache_file = ROOT_PATH."cache/css_".$IN['id'].".css";
	...
	function css_upload($type='new')
	{
		...
		if ($type == 'new')
		{
			...
			$DB->query("INSERT INTO acf_css (".$dbs['FIELD_NAMES'].") VALUES(".$dbs['FIELD_VALUES'].")");
	...
	function optimize()
	{
		...
		$DB->query("SELECT * from acf_css WHERE cssid='".$IN['id']."'");
		...
		$DB->query("UPDATE acf_css SET $dbs WHERE cssid='".$IN['id']."'");
	...
	function remove()
	{
		...
		$DB->query("DELETE FROM acf_css WHERE cssid='".$IN['id']."'");
	...
	function save_wrapper( $type='add' )
	{
		...
		if ($type == 'add')
		{
			...
			$DB->query("INSERT INTO acf_css (".$db_string['FIELD_NAMES'].") VALUES(".$db_string['FIELD_VALUES'].")");
		...
		else
		{
			...
			$DB->query("UPDATE acf_css SET $db_string WHERE cssid='".$IN['id']."'");
	...
	function do_form( $type='add' )
	{
		...
		$DB->query("SELECT cssid, css_text, css_name, updated FROM acf_css WHERE cssid='".$IN['id']."'");
	...
	function list_sheets()
	{
		...
		$DB->query("SELECT DISTINCT(c.cssid), c.css_name, s.sname from acf_css c, acf_skins s WHERE s.css_id=c.cssid ORDER BY c.css_name ASC");
		...
		if ( count($used_ids) > 0 )
		{
			...
			$DB->query("SELECT cssid, css_name FROM acf_css WHERE cssid NOT IN(".implode(",",$used_ids).")");
