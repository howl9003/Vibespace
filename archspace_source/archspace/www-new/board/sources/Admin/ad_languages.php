...
class ad_langs {
	...
	function doimport()
	{
		...
		$DB->query("INSERT INTO acf_languages (ldir, lname) VALUES('temp', '$real_name (Import)')");
		...
		if ( ! mkdir($dest, 0777) )
		{
			$DB->query("DELETE FROM acf_languages WHERE lid='$new_id'");
		...
		$DB->query("UPDATE acf_languages SET $db_string WHERE lid='$new_id'");
	...
	function export()
	{
		...
		$DB->query("SELECT * from acf_languages WHERE lid='".$IN['id']."'");
	...
	function edit_info()
	{
		...
		$DB->query("UPDATE acf_languages SET $db_string WHERE lid='".$IN['id']."'");
	...
	function add_language()
	{
		...
		$DB->query("INSERT INTO acf_languages (".$db_string['FIELD_NAMES'].") VALUES(".$db_string['FIELD_VALUES'].")");
		...
		{
			$DB->query("UPDATE acf_languages SET ldir='$new_id' WHERE lid='$new_id'");
		}
	...
	function remove()
	{
		...
		$DB->query("UPDATE acf_members SET language='{$INFO['default_language']}' WHERE language='{$row['ldir']}'");
		
		if ( $ADMIN->rm_dir( $INFO['base_dir']."lang/".$row['ldir'] ) )
		{
		
			$DB->query("DELETE FROM acf_languages WHERE lid='".$IN['id']."'");
	...
	function list_current()
	{
		...
		$DB->query("select acf_languages.*, count(acf_members.id) as mcount from acf_languages left join acf_members on(acf_members.language=acf_languages.ldir) where (acf_members.language is not null or acf_members.language = 'en') group by acf_languages.ldir order by acf_languages.lname");
		...
		if ( count($used_ids) < 1 )
		{
			...
			$DB->query("SELECT lid, ldir, lname FROM acf_languages WHERE lid NOT IN(".implode(",",$used_ids).")");
