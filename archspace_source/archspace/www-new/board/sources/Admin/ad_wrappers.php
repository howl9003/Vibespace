...
class ad_settings {
	...
	function add_splash()
	{
		...
		$DB->query("INSERT INTO acf_templates ({$str['FIELD_NAMES']}) VALUES({$str['FIELD_VALUES']})");
	...
	function export()
	{
		...
		$DB->query("SELECT * from acf_templates WHERE tmid='".$IN['id']."'");
	...
	function remove()
	{
		...
		$DB->query("DELETE FROM acf_templates WHERE tmid='".$IN['id']."'");
	...
	function save_wrapper( $type='add' )
	{
		...
		if ($type == 'add')
		{
			...
			$DB->query("INSERT INTO acf_templates (".$db_string['FIELD_NAMES'].") VALUES(".$db_string['FIELD_VALUES'].")");
		...
		else
		{
			...
			$DB->query("UPDATE acf_templates SET $db_string WHERE tmid='".$IN['id']."'");
	...
	function list_wrappers()
	{
		...
		$DB->query("SELECT DISTINCT(w.tmid), w.name, s.sname from acf_templates w, acf_skins s WHERE s.tmpl_id=w.tmid ORDER BY w.name ASC");
		...
		if ( count($used_ids) > 0 )
		{
			...
			$DB->query("SELECT tmid, name FROM acf_templates WHERE tmid NOT IN(".implode(",",$used_ids).")");
