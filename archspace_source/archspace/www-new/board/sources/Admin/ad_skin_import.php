...
class ad_langs {
	...
	function set_import()
	{
		...
		$DB->query("SELECT COUNT(*) as count FROM acf_tmpl_names");
		...
		$DB->query("INSERT INTO acf_tmpl_names SET skname=\"".$this->name_translated." (Set Import).{$tmpl_count['count']}\"");
		
		$next_id['templates'] = $DB->get_insert_id();
		
		//------------------------------------------------------
		
		$DB->query("INSERT INTO acf_css SET css_name=\"".$this->name_translated." (Set Import)\"");
		
		$next_id['css'] = $DB->get_insert_id();
		
		//------------------------------------------------------
		
		$DB->query("INSERT INTO acf_templates SET name=\"".$this->name_translated." (Set Import)\"");
		...
		if ( file_exists($this->work_path."/".$new_dir."/templates_conf.inc") )
		{
			...
			$DB->query("UPDATE acf_tmpl_names SET $db_string WHERE skid='{$next_id['templates']}'");
		}
		...
		if ($FH = fopen( $this->work_path."/".$new_dir."/stylesheet.css", 'r' ) )
		{
			...
			$DB->query("UPDATE acf_css SET $css_string WHERE cssid='{$next_id['css']}'");
		...
		if ($FH = fopen( $this->work_path."/".$new_dir."/wrapper.html", 'r' ) )
		{
			...
			$DB->query("UPDATE acf_templates SET $wrap_string WHERE tmid='{$next_id['wrap']}'");
		...
		if ($FH = fopen( $this->work_path."/".$new_dir."/macro.txt", 'r' ) )
		{
			...
			$DB->query("INSERT INTO acf_macro_name SET set_id='".$next_id['macro']."', set_name='".$this->name_translated."'");
		...
		$DB->query("UPDATE acf_tmpl_names SET $tmpl_string WHERE skid='{$next_id['templates']}'");
		...
		$DB->query("INSERT INTO acf_skins (".$skin_string['FIELD_NAMES'].") VALUES(".$skin_string['FIELD_VALUES'].")");
	...
	function import_error( $error, $next_id)
	{
		...
		$DB->query("DELETE FROM acf_macro_name WHERE set_id='{$next_id['macro']}'");
		$DB->query("DELETE FROM acf_macro WHERE macro_id='{$next_id['macro']}'");
		$DB->query("DELETE FROM acf_tmpl_names WHERE skid='{$next_id['templates']}'");
		$DB->query("DELETE FROM acf_css WHERE cssid='{$next_id['css']}'");
		$DB->query("DELETE FROM acf_templates WHERE tmid='{$next_id['wrap']}'");
		$DB->query("DELETE FROM acf_skin_templates WHERE set_id='{$next_id['templates']}'");
