...
class ad_adlogs {
	...
	function view()
	{
		...
		if ($IN['search_string'] == "")
		{
			$DB->query("SELECT COUNT(id) as count FROM acf_admin_logs WHERE member_id='".$IN['mid']."'");
			$row = $DB->fetch_row();
		...
		else
		{
			...
			$DB->query("SELECT COUNT(m.id) as count FROM acf_admin_logs m WHERE $dbq");
			$row = $DB->fetch_row();
	...
	function remove()
	{
		...
		$DB->query("DELETE FROM acf_admin_logs WHERE member_id='".$IN['mid']."'");
	...
	function list_current()
	{
		...
		$DB->query("SELECT m.*, mem.name, count(m.id) as act_count FROM acf_admin_logs m, acf_members mem WHERE m.member_id=mem.id GROUP BY m.member_id ORDER BY act_count DESC");
