...
class ad_cat {
	...
	function do_reorder() {
		...
		$cat_query = $DB->query("SELECT id from acf_categories");
		
		while ( $r = $DB->fetch_row($cat_query) )
		{
			$order_query = $DB->query("UPDATE acf_categories SET position='".$IN[ 'POS_' . $r['id'] ]."' WHERE id='".$r['id']."'");
		}
	...
	function remove_form() {
		...
		$DB->query("SELECT id, name FROM acf_categories WHERE id > 0 ");
		...
		$DB->query("SELECT * FROM acf_categories WHERE id='".$IN['c']."'");
		$cat = $DB->fetch_row();
	...
	function do_remove() {
		...
		$DB->query("UPDATE acf_forums SET category='".$IN['MOVE_ID']."' WHERE category='".$IN['c']."'");
		
		$DB->query("DELETE FROM acf_categories WHERE id='".$IN['c']."'");
	...
	function do_edit() {
		...
		$DB->query("UPDATE acf_categories SET $db_string WHERE id='".$IN['c']."'");
	...
	function show_cats() {
		...
		$DB->query("SELECT uid, sname, sid FROM acf_skins");
	...
	function do_new() {
		...
		$DB->query("SELECT MAX(id) as top_cat FROM acf_categories");   //ooh, top cat - he's the leader of our gang..
		$row = $DB->fetch_row();
		...
		$DB->query("INSERT INTO acf_categories (".$db_string['FIELD_NAMES'].") VALUES (".$db_string['FIELD_VALUES'].")");
