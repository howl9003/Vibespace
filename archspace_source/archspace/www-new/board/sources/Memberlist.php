...
class Memberlist {
    ...
    function Memberlist()
    {
    	...
    	$DB->query("SELECT g_title, g_id, g_icon from acf_groups WHERE g_hide_from_list <> 1 ORDER BY g_title");
    	...
    	$DB->query("SELECT title, id, posts, pips from acf_titles ORDER BY posts DESC");
    	...
    	{
    		$DB->query("SELECT COUNT(m.id) as total_members FROM acf_members m
