...
class ad_settings {
	...
	function settings()
	{
		...
		$DB->query("SELECT * FROM acf_cache_store WHERE cs_key IN ('csite_nav_contents', 'csite_fav_contents')");
		...
		{
			$DB->query("REPLACE INTO acf_cache_store SET cs_key='csite_nav_contents', cs_value=\"$def_nav\"");
			$DB->query("REPLACE INTO acf_cache_store SET cs_key='csite_fav_contents', cs_value=''");
		}
	...
	function save_config( $new )
	{
		...
		$DB->query("UPDATE acf_cache_store SET cs_value='".str_replace( "'", '&#39;', $nav_contents )."' WHERE cs_key='csite_nav_contents'");
		$DB->query("UPDATE acf_cache_store SET cs_value='".str_replace( "'", '&#39;', $fav_contents )."' WHERE cs_key='csite_fav_contents'");
