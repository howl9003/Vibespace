...
class attach {
    ...
    function attach()
    {
        ...
        if ($ibforums->input['type'] == 'post')
        {
        	...
        	$DB->query("SELECT pid, attach_id, attach_type, attach_file FROM acf_posts WHERE pid='".$ibforums->input['id']."'");
        	...
        	if ( file_exists( $file ) and ( $post['attach_type'] != "" ) )
        	{
        		...
        		$DB->query("UPDATE acf_posts SET attach_hits=attach_hits+1 WHERE pid='".$post['pid']."'");
