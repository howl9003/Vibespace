...
EOF;
    ...
    function do_log_in() {
		...
		$DB->query("SELECT id, name, mgroup, password, new_pass FROM acf_members WHERE LOWER(name)='$username'");
		...
		if ($DB->get_num_rows())
		{
			...
			if ($ibforums->input['s'])
			{
				...
				$db_query = "UPDATE acf_sessions SET $db_string WHERE id='".$ibforums->input['s']."'";
			}
			else
			{
			...
			$DB->query("UPDATE acf_members SET password='$real_pass' WHERE id='".$member['id']."'");
