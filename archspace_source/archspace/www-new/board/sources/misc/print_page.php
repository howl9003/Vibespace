...
class Printable {
	...
	function get_posts()
	{
		...
		$DB->query( "SELECT * FROM acf_posts WHERE topic_id='".$this->topic['tid']."'".
		...
		if ($mem_ids != "")
		{
			$DB->query("SELECT g.*, m.* FROM acf_members m, acf_groups g WHERE m.id in ($mem_ids) AND m.mgroup=g.g_id");
