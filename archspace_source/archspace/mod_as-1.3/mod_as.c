...
util_read(request_rec *aRequest, const char **aRequestBuffer)
{
	...
	if ((RC = ap_setup_client_block(aRequest, REQUEST_CHUNKED_ERROR)) != OK)
		return RC;
...
error_message(request_rec *aRequest, const char *aMessage)
{
	...
	ap_rprintf(aRequest,
		...
		"<CENTER>\n"
		"<A HREF=\"/main.php\">\n"
		"<IMG SRC=\"/image/as_game/banner.gif\" BORDER=0 ></A>\n"
		"</CENTER>\n"
		"</TD>\n"
		"</TR>\n"
		...
		"<TR>\n"
		"<TD ALIGN=\"center\" HEIGHT=\"18\"><IMG SRC=\"/image/as_game/message/message.gif\" WIDTH=\"175\" HEIGHT=\"131\">\n"
		"</TD>\n"
		"</TR>\n"
...
check_redirect(request_rec *aRequest)
{
	...
	//ap_log_printf(aRequest->server, "YOSHIKI : Cookie:%s", Cookie);
	if (!Cookie) return 1;
	...
	while(*Buffer && (Pair = ap_getword(aRequest->pool, &Buffer, ';')))
	{
		...
		//ap_log_printf(aRequest->server, "Cookie: name:%s, value:%s", Name, Pair);

		if (Name && strlen(Name))
			if (strcmp(Name, "phpbb2mysql_sid") == 0) return 0;
...
//	get_connection_info(aRequest, &Host, &Port);
	...
	// Uncommented some stuff to see if it helps
//	if (!Host)
...
//		Socket = make_connection(Host, Port, aRequest);
	...
	if (Packet) {
		//ap_log_printf(aRequest->server, "testlog cookie send" );
	...
	if (Packet) {
		//ap_log_printf(aRequest->server, "testlog connection send" );
		link_packet(Send, Packet);
	}
	Packet = make_query_send(aRequest, (char*)Buffer);
	if (Packet) {
		//ap_log_printf(aRequest->server, "testlog query send" );
		link_packet(Send, Packet);
	}
	...
	ap_log_printf(aRequest->server, "Wait to receive");
			...
			&Header, &Cookie, &Content) < 0)
	{
		...
		close(Socket);
		ap_log_printf(aRequest->server, "failed to receive");
		return OK;
	}
...
//	set_header(aRequest, Header);
	...
	ap_log_printf(aRequest->server, "successfully");
