...
/* CVS ship schematic techs. The schematic for hull class C is tech
 * (SHIP_SCHEMATIC_TECH_BASE + C), i.e. class 1 -> 1600 ... class 12 -> 1611.
 * Following CVS, every hull class 1..MAX_SHIP_CLASS is gated on the cumulative
 * count of Schematics techs the player has researched (you can build up to
 * class N once you know N schematic techs), so NO schematic is locked from play
 * -- all are researchable / grantable / auctionable. IS_LOCKED_SHIP_SCHEMATIC is
 * kept (always false) only so its former call sites compile unchanged. */
#define SHIP_SCHEMATIC_TECH_BASE	1599
#define SHIP_SCHEMATIC_TECH(c)		(SHIP_SCHEMATIC_TECH_BASE + (c))
#define FIRST_MEGACLASS				11
#define IS_LOCKED_SHIP_SCHEMATIC(id)	(false)
