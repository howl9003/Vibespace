...
CFleet::create_as_empire_fleet(int aFleetRating, int aTechLevel, int aSizeRating, CAdmiral *aAdmiral, int aOrder)
{
	...
	while (1)
	{
		for (int i=0 ; i<EMPIRE_SHIP_DESIGN_TABLE->length() ; i++)
		{
			...
			{
				if (ShipDesign->get_design_id() - ShipSize*100 != CurrentTechLevel) continue;
	...
	CFleetList::reassignment_fleet_list_javascript(CPlayer *aPlayer)
{
	...
	if (!length())
	{
		Info = GETTEXT("You don't have any fleets.");
		return (char *)Info;
	}

	Info = "<TABLE CLASS=\"as-sortable\" WIDTH=570 BORDER=1 CELLSPACING=0 CELLPADDING=0 BORDERCOLOR=#2A2A2A>\n";
	Info += "<THEAD>\n";
	Info += "<TR BGCOLOR=#171717>\n";

	Info += "<TH CLASS=\"tabletxt nosort\" WIDTH=29><FONT COLOR=666666>";
	Info += "<INPUT TYPE=checkbox onClick=\"var b=this.checked,f=this.form.elements;for(var k=0;k<f.length;k++){if(f[k].name&&f[k].name.indexOf('FLEET')==0)f[k].checked=b;}\">";
	Info += "</FONT></TH>\n";

	Info += "<TH CLASS=tabletxt WIDTH=29><FONT COLOR=666666>";
	Info += GETTEXT("ID");
	Info += "</FONT></TH>\n";

	Info += "<TH WIDTH=112 CLASS=tabletxt><FONT COLOR=666666>";
	Info += GETTEXT("Name");
	Info += "</FONT></TH>\n";

	Info += "<TH WIDTH=144 CLASS=tabletxt><FONT COLOR=666666>";
	Info += GETTEXT("Commander");
	Info += "</FONT></TH>\n";

	Info += "<TH WIDTH=36 CLASS=tabletxt><FONT COLOR=666666>";
	Info += GETTEXT("Exp.");
	Info += "</FONT></TH>\n";

	Info += "<TH WIDTH=64 CLASS=tabletxt><FONT COLOR=666666>";
	Info += GETTEXT("Status");
	Info += "</FONT></TH>\n";

	Info += "<TH WIDTH=99 CLASS=tabletxt><FONT COLOR=666666>";
	Info += GETTEXT("Ship Class");
	Info += "</FONT></TH>\n";

	Info += "<TH WIDTH=49 CLASS=tabletxt><FONT COLOR=666666>";
	Info += GETTEXT("Ships");
	Info += "</FONT></TH>\n";
	...
	for (int i=0 ; i<length() ; i++)
	{
		...
		Info.format("<TD CLASS=tabletxt WIDTH=29 ALIGN=CENTER><INPUT TYPE=checkbox NAME=FLEET%d></TD>\n",
					i);

		Info.format("<TD CLASS=tabletxt WIDTH=29 ALIGN=CENTER>%d</TD>\n",
					Fleet->get_id());

		Info.format("<TD WIDTH=112 CLASS=tabletxt ALIGN=LEFT>%s</TD>\n",
					Fleet->get_name());

		Info += "<TD WIDTH=144 CLASS=tabletxt ALIGN=LEFT>";
		Info.format("<A HREF=\"/archspace/fleet/fleet_commander_information.as?ADMIRAL_ID=%d\">%s</A>",
					Admiral->get_id(), Admiral->get_name());
		Info += "</TD>\n";

		Info.format("<TD WIDTH=36 CLASS=tabletxt ALIGN=CENTER>%d</TD>\n",
					Fleet->get_exp());

		Info.format("<TD WIDTH=64 CLASS=tabletxt ALIGN=LEFT>%s</TD>\n",
					Fleet->get_detailed_status_string());

		Info += "<TD WIDTH=99 CLASS=tabletxt ALIGN=LEFT>";
		Info.format("<A HREF=\"/archspace/fleet/class_specification.as?DESIGN_ID=%d\">%s</A>",
					Fleet->get_design_id(), Fleet->CShipDesign::get_name());
		Info += "</TD>\n";

		Info.format("<TD WIDTH=49 CLASS=tabletxt ALIGN=CENTER>&nbsp;%d/%d</TD>\n",
					Fleet->get_current_ship(), Fleet->get_max_ship());
