...
CPageBlockadeResult::handler(CPlayer *aPlayer)
{
	...
	if (AttackRatio < 60) HonorLoss += 2;
	else if (AttackRatio < 70) HonorLoss += 1;
	else if (AttackRatio < 80) HonorLoss += 1;
	else if (AttackRatio < 90) HonorLoss += 0;
	else if (AttackRatio < 100) HonorLoss += 0;

	// modify the loss by the difference of honor divided by 6
	HonorLoss += (int)((TargetPlayer->get_honor()-aPlayer->get_honor())/24);
...
#undef SHIP_SIZE_CRUISER
		...
		CDefenseFleet *OffenseFleet = new CDefenseFleet();
	...
	aPlayer->set_seige_blockade_restriction();
#endif
#ifdef DEFENDER_COOLDOWN
	TargetPlayer->set_seige_blockade_protection();
#endif
