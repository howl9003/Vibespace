...
CSpy::steal_secret_info(CPlayer *aSpyPlayer, CPlayer *aTargetPlayer)
{
	...
	switch (DetailedInfo)
	{
		case 1 :
		{
			...
			aTargetPlayer->time_news(
					GETTEXT("Some of your detailed information has been drained by someone!"));
...
CSpy::red_death(CPlayer *aSpyPlayer, CPlayer *aTargetPlayer)
{
	...
	for (int i=0 ; i<PlanetList->length() ; i++)
	{
		...
		else
		{
			...
			{
				aTargetPlayer->time_news(
						(char *)format(GETTEXT("You lost many people on planet %1$s by someone's spy, and the planet has an obstinate microbe now."),
										Planet->get_name()));
			}
			else
			{
				aTargetPlayer->time_news(
						(char *)format(GETTEXT("You lost many people on planet %1$s by someone's spy."),
										Planet->get_name()));
			}
		}
	}
