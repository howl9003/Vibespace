...
{ 
  ...
  if (this.level>0) 
    if (this.isLastNode) //the last child in the children array 
	    leftSide = leftSide + "<td valign=middle>" + auxEv + "<img name='nodeIcon" + this.id + "' id='nodeIcon" + this.id + "' src='" + nodeName + "' border=0></a></td>"
    else 
      leftSide = leftSide + "<td valign=middle>" + auxEv + "<img name='nodeIcon" + this.id + "' id='nodeIcon" + this.id + "' src='" + nodeName + "' border=0></a></td>"
  ...
  if (WRAPTEXT)
	  docW = docW + "</td>"+this.prependHTML+"<td valign=middle width=100% class=\"c1Top\">"
  else
	  docW = docW + "</td>"+this.prependHTML+"<td valign=middle nowrap width=100% class=\"c1Top\">"
  if (USETEXTLINKS) 
  { 
    ...
    docW = docW + "<font class=topmenu>" + this.desc + "</font>"
  docW = docW + "</td>"
...
{ 
  ...
  { 
	...
	{
      if (this.target)
		docW = docW + "<a href='" + this.hreference + "' class=topmenu TARGET=\""+this.target+"\" "
	  else
		docW = docW + "<a href='" + this.hreference + "' class=topmenu TARGET=\"contents\" "
	}
	else
	  docW = docW + "<a href='" + this.hreference + "' class=topmenu TARGET=_top "
  ...
  {
    docW = docW + "<a class=topmenu style=\"cursor: pointer;\"" 
    if (browserVersion > 0) 
...
function nodeImageSrc() {
  ...
  { 
    if (this.nChildren == 0)
		srcStr = ICONPATH + "as_game/minus.gif";
...
//        srcStr = ICONPATH + "ftv2plastnode.gif"  
		srcStr = ICONPATH + "as_game/plus.gif";
  } 
  else 
  { 
   if (this.nChildren == 0)
...
{ 
  ...
  var fullLink = "href=\""+this.link+"\" target=\""+this.target+"\""; ///*target=\"contents\"*///" onClick=\"clickOnLink('"+this.getID()+"\', '"+this.link+"','"+this.target+"');return false;\"";
  ...
  if (WRAPTEXT)
    docW = docW + "</td>"+this.prependHTML+"<td valign=middle width=100% class=c1Sub>"
  else
    docW = docW + "</td>"+this.prependHTML+"<td valign=middle nowrap width=100% class=c1Sub>"

  if (USETEXTLINKS) 
    docW = docW + "<a class=submenu " + fullLink + " id=\"itemTextLink"+this.id+"\">" + this.desc + "</a>"
  else 
    docW = docW + this.desc
...
{ 
/*  
  This would be evil treeview people trying to make stuff not work!
  if (browserVersion == 1 || browserVersion == 3) { 
    if (t==-1)
      return
...
*/
  if (!this.isRendered && navigator.family != 'opera') // addedto fix an opera bug (messy).
...
function gFld(optionFlags, description, hreference) 
{ 
  if (optionFlags >= 0)
  {
     folder = new Folder(description, hreference);
//	 alert("optionFlags must be set for"+description);
	 return folder;
  }	
  folder = new Folder(description, hreference);
  setItemLink(folder, optionFlags, hreference);	
  return folder;
} 
...
function preLoadIcons() {
	var auxImg
	auxImg = new Image();
	auxImg.src = ICONPATH + "as_game/minus.gif";
	auxImg.src = ICONPATH + "as_game/plus.gif";
