...
class Forums {
    ...
    function Forums()
    {
        ...
        if ( $this->forum['redirect_on'] and $this->forum['redirect_url'] )
        {
        	...
        	$DB->query("UPDATE acf_forums SET redirect_hits=redirect_hits+1 WHERE id=".$this->forum['id']);
     ...
     function show_subforums() {
        ...
        $DB->query("SELECT f.*, m.member_name as mod_name, m.member_id as mod_id, m.is_group, m.group_id, m.group_name, m.mid
        			FROM acf_forums f
        			 LEFT JOIN acf_moderators m ON (f.id=m.forum_id)
        			WHERE parent_id='$fid'
        			ORDER BY position");
	...
	function render_forum()
	{
		...
		$DB->query("SELECT COUNT(tid) as max FROM acf_topics WHERE forum_id=".$this->forum['id']." and approved=1 and (pinned=1 or last_post > $Prune)");
		...
		{
			$query = "SELECT DISTINCT(acf_posts.author_id), acf_topics.* FROM acf_topics
			           LEFT JOIN acf_posts ON 
			           (acf_topics.tid=acf_posts.topic_id AND acf_posts.author_id=".$ibforums->member['id'].")
			           WHERE acf_topics.forum_id=".$this->forum['id']."
			           and acf_topics.approved=1
			           and (acf_topics.pinned=1 or acf_topics.last_post > $Prune)";
		}
		else
		{
			$query = "SELECT * from acf_topics WHERE forum_id=".$this->forum['id']." and approved=1 and (last_post > $Prune OR pinned=1)";
		}
		...
		if ($ibforums->vars['no_au_forum'] != 1)
		{
			...
			$DB->query("SELECT s.member_id, s.member_name, s.login_type, s.location, g.suffix, g.prefix, g.g_perm_id, t.forum_id, m.org_perm_id
					    FROM acf_sessions s
					     ...
					     LEFT JOIN acf_topics t ON (t.tid=s.in_topic)
