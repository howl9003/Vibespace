...
class Post {
    ...
    function Post()
    {
        ...
        $DB->query("SELECT f.*, c.id as cat_id, c.name as cat_name from acf_forums f, acf_categories c WHERE f.id=".$ibforums->input[f]." and c.id=f.category");
        ...
        if ( $ibforums->member['mod_posts'] )
		{
			...
			else
			{
				...
				if ( time() >= $mod_arr['date_end'] )
				{
					...
					$DB->query("UPDATE acf_members SET mod_posts=0 WHERE id=".intval($ibforums->member['id']) );
        ...
        if ($ibforums->member['id'])
        {
        	if ( $ibforums->member['restrict_post'] )
        	{
        		...
        		if ( time() >= $post_arr['date_end'] )
        		{
        			...
        			$DB->query("UPDATE acf_members SET restrict_post=0 WHERE id=".intval($ibforums->member['id']) );
        		}
        		else
        		{
        ...
        if ($this->obj['action_codes'][ $ibforums->input['CODE'] ][0])
        {
        	...
        	if (!$ibforums->member['id'])
        	{
        		...
        		{
        			$DB->query("SELECT id FROM acf_members WHERE LOWER(name)='".trim(strtolower($ibforums->input['UserName']))."'");
				...
				if ( preg_match( "/Post,.*,(01|03|07|11)$/", $ibforums->location ) )
				{
					if ( time() - $ibforums->lastclick < 2 )
					{
						if ( $ibforums->input['CODE'] == '01' or $ibforums->input['CODE'] == '11' )
						{
							...
							$DB->query("SELECT tid from acf_topics WHERE forum_id='".$this->forum['id']."' AND approved=1 "
	...
	function topic_tracker($tid="", $post="", $poster="", $last_post="" )
	{
		...
		$DB->query("SELECT tr.trid, tr.topic_id, m.name, m.email, m.id, m.email_full, m.language, m.last_activity, t.title, t.forum_id
				    FROM acf_tracker tr, acf_topics t,acf_members m
	...
	function forum_tracker($fid="", $this_tid="", $title="", $forum_name="")
	{
		...
		$DB->query("SELECT tr.frid, m.name, m.email, m.id, m.language, m.last_activity, m.org_perm_id, g.g_perm_id
				    FROM acf_forum_tracker tr,acf_members m, acf_groups g
	...
	function html_checkboxes($type="", $tid="") 
	{
		...
		if ( $type == 'reply' )
		{
			...
			{
				$DB->query("SELECT trid FROM acf_tracker WHERE topic_id=$tid AND member_id=".$ibforums->member['id']);
	...
	function html_topic_summary($topic_id) {
		...
		$post_query = $DB->query("SELECT post, pid, post_date, author_id, author_name FROM acf_posts WHERE topic_id=$topic_id and queued <> 1 ORDER BY pid DESC LIMIT 0,10");
