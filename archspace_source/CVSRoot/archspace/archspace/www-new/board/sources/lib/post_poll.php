...
class post_functions extends Post {
	...
	function add_new_poll($class) {
		...
		$DB->query("INSERT INTO acf_polls (" .$db_string['FIELD_NAMES']. ") VALUES (". $db_string['FIELD_VALUES'] .")");
		...
		// the value stored in the acf_stats database.
		
		$DB->query("SELECT COUNT(tid) as topic_cnt FROM acf_topics");
		$stats = $DB->fetch_row();
		
		$DB->query("UPDATE acf_stats SET TOTAL_TOPICS='".$stats['topic_cnt']."'");
