...
class post_functions extends Post {
	...
	function post_functions($class) {
		...
		$DB->query("SELECT * FROM acf_topics WHERE forum_id='".$class->forum['id']."' AND tid='".$ibforums->input['t']."'");
...
function add_reply($class) {
		...
		{
			$DB->query("UPDATE acf_forums SET has_mod_posts=1 WHERE id=".$class->forum['id']);
		...
		$DB->query("UPDATE acf_forums    SET last_title='"      .$class->forum['last_title']       ."', ".
		...
		$DB->query("SELECT COUNT(pid) as posts FROM acf_posts WHERE topic_id={$this->topic['tid']} and queued != 1");
		...
		$DB->query("UPDATE acf_topics     SET last_poster_id='"       .$class->forum['last_poster_id']    ."', ".
											  "last_poster_name='"    .$class->forum['last_poster_name']  ."', ".
											  "last_post='"           .$class->forum['last_post']         ."', ".
		...
		if ($ibforums->member['id'])
		{
			...
			$DB->query("UPDATE acf_members SET ".$pcount.$mgroup.
		...
		{
			$DB->query("SELECT trid FROM acf_tracker WHERE topic_id='".$this->topic['tid']."' AND member_id='".$ibforums->member['id']."'");
			...
			if ( ! $DB->get_num_rows() )
			{
				...
				$DB->query("INSERT INTO acf_tracker ({$db_string['FIELD_NAMES']}) VALUES ({$db_string['FIELD_VALUES']})");
			}
		}
	...
	function show_form($class) {
		...
		$DB->query("SELECT post, author_id, author_name, post_date, pid FROM acf_posts WHERE pid='".$ibforums->input['p']."'");
