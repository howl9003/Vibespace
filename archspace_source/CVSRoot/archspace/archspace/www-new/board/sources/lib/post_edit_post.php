...
class post_functions extends Post {
	...
	function post_functions($class) {
		...
		$DB->query("SELECT * FROM acf_topics WHERE tid=".intval($ibforums->input['t']));
		...
		$DB->query("SELECT * FROM acf_posts WHERE pid=".intval($ibforums->input['p']));
		...
		{
			$DB->query("SELECT member_id, member_name, mid, edit_post, edit_topic FROM acf_moderators WHERE forum_id=".$class->forum['id']." AND (member_id=".$ibforums->member['id']." OR (is_group=1 AND group_id='".$ibforums->member['mgroup']."'))");
	...
	function complete_edit($class) {
		...
		if ($this->orig_post['new_topic'] == 1)
		{
			...
			{
				$DB->query("UPDATE acf_topics SET icon_id='".$this->post['icon_id']."' WHERE tid=".$this->topic['tid']);
			}
		}
		...
		if ( $this->edit_title == 1 )
		{
			...
			if ( $ibforums->input['TopicTitle'] != "" )
			{
				if ( ($ibforums->input['TopicTitle'] != $this->topic['title']) or ($ibforums->input['TopicDesc'] != $this->topic['description'])  )
				{
					...
					$DB->query("UPDATE acf_topics SET $dbs WHERE tid=".$this->topic['tid']);
					
					if ($this->topic['tid'] == $class->forum['last_id'])
					{
						$DB->query("UPDATE acf_forums SET last_title='{$ibforums->input['TopicTitle']}' WHERE id='".$class->forum['id']."'");
					}
		...
		$DB->query("UPDATE acf_posts SET $db_string WHERE pid='".$this->post['pid']."'");
