...
class Moderate {
    ...
    function Moderate()
    {
        ...
        if ($ibforums->member['id'])
        {
        	...
        	else if ($ibforums->member['is_mod'])
        	{
        		...
        		$DB->query("SELECT * FROM acf_moderators WHERE $qe (member_id='".$ibforums->member['id']."' OR (is_group=1 AND group_id='".$ibforums->member['mgroup']."'))");
	...
	function topic_mmod()
	{
        ...
        $DB->query("SELECT * FROM acf_topic_mmod WHERE mm_id={$this->mm_id}");
        ...
        $DB->query("UPDATE acf_topics SET title=CONCAT('$pre', title, '$end') WHERE tid IN(".implode( ",", $this->tids ).")");
        ...
        if ( $this->mm_data['topic_move'] )
        {
        	...
        	$DB->query("SELECT id, name, subwrap, sub_can_post FROM acf_forums WHERE id=".$this->mm_data['topic_move']);
        	
        	if ( $r = $DB->fetch_row() )
        	{
        		if ( $r['subwrap'] == 1 AND $r['sub_can_post'] != 1 )
        		{
        			$DB->query("UPDATE acf_topic_mmod SET topic_move=0 WHERE mm_id=".$this->mm_id);
        		}
        		else
        		{
	...
	function do_ip()
	{
		...
		else if ($ibforums->input['iptool'] == 'members')
		{
			...
			$DB->query("SELECT count(id) as max FROM acf_members WHERE $sql");
			...
			$DB->query("SELECT name, id, ip_address, posts, joined FROM acf_members WHERE $sql ORDER BY joined DESC LIMIT {$this->start_val},50");
		...
		else
		{
			...
			$DB->query("SELECT id, read_perms FROM acf_forums");
			...
			$DB->query("SELECT pid FROM acf_posts WHERE queued <> 1 AND forum_id IN($forums) AND $sql");
			...
			$DB->query("INSERT INTO acf_search_results ({$str['FIELD_NAMES']}) VALUES ({$str['FIELD_VALUES']})");
	...
	function complete_checked_move()
	{
		...
		$DB->query("SELECT id, subwrap, sub_can_post, name FROM acf_forums WHERE id IN(".$source_id.",".$dest_id.")");
	...
	function start_checked_move()
	{
		...
		$DB->query("SELECT title, tid FROM acf_topics WHERE forum_id=".$this->forum['id']." AND tid IN(".implode(",", $this->tids).")");
	...
	function alter_topics($mod_action="", $sql="")
	{
		...
		$DB->query("UPDATE acf_topics SET $sql WHERE forum_id=".$this->forum['id']." AND tid IN(".implode(",",$this->tids).")");
	...
	function show_topics()
	{
		...
		$DB->query("SELECT COUNT(tid) as max FROM acf_topics WHERE forum_id='".$this->forum['id']."' and approved='1'");
		$total_possible = $DB->fetch_row();
		...
		$query = "SELECT * from acf_topics WHERE forum_id='".$this->forum['id']."' and approved=1 ORDER BY pinned DESC, last_post DESC LIMIT $first,".$ibforums->vars['display_max_topics'];
		...
		$DB->query("SELECT mm_id, mm_title FROM acf_topic_mmod WHERE mm_id IN(".implode( ",", explode( ",", $this->forum['topic_mm_id'] ) ).")");
	...
	function show_forums()
	{
		...
		{
			$DB->query("SELECT forum_id FROM acf_moderators WHERE member_id={$ibforums->member['id']} OR group_id={$ibforums->member['mgroup']}");
		...
		$DB->query("SELECT COUNT(pid) as qued, forum_id FROM acf_posts
		...
		$DB->query("SELECT COUNT(tid) as qued, forum_id FROM acf_topics
		...
		$DB->query("SELECT f.*, c.id as cat_id, c.name as cat_name
		            FROM acf_forums f
 		             LEFT JOIN acf_categories c ON (c.id=f.category)
 		            WHERE f.id IN ($fids)
	...
	function approve_all()
	{
		...
		$DB->query("UPDATE acf_posts SET queued=0 WHERE topic_id='".$ibforums->input['tid']."'");
		
		$DB->query("SELECT COUNT(pid) as posts FROM acf_posts WHERE new_topic <> 1 and topic_id='".$ibforums->input['tid']."'");
		$count = $DB->fetch_row();
		
		$DB->query("UPDATE acf_topics SET posts=".$count['posts']." WHERE tid='".$ibforums->input['tid']."'");
		
		// Update the posters ..er.. post count.
			
		$DB->query("SELECT author_id FROM acf_posts WHERE topic_id='".$ibforums->input['tid']."'");
		...
		if ( count($mems) > 0 )
		{
			if ( $this->forum['inc_postcount'] )
			{
				...
				$DB->query("SELECT * FROM acf_groups");
				...
				$loopy_loo = $DB->query("SELECT id, mgroup, posts FROM acf_members WHERE id IN ($mstring)");
				...
				while ( $member = $DB->fetch_row($loopy_loo) )
				{
					...
					$newbie = $DB->query("UPDATE acf_members SET ".$mgroup."posts=posts+1 WHERE id={$member['id']}");
				}
			}
		}
		...
		$DB->query("SELECT author_id, author_name, post_date FROM acf_posts WHERE topic_id='".$ibforums->input['tid']."' AND queued <> 1 ORDER BY pid DESC LIMIT 0,1");
		...
		if ($last = $DB->fetch_row())
		{
			...
			$DB->query("UPDATE acf_topics SET $db_string WHERE tid='".$ibforums->input['tid']."'");
		}
	...
	function mod_domodposts()
	{
		...
		if ( count($approve_ids) > 0 )
		{
			...
			$DB->query("UPDATE acf_topics SET posts=posts+$pid_count WHERE tid='".$ibforums->input['tid']."'");
			
			$DB->query("UPDATE acf_posts SET queued=0 WHERE pid IN ($pids)");
			
			// Update the posters ..er.. post count.
			
			$DB->query("SELECT author_id FROM acf_posts WHERE queued <> 1 and pid IN ($pids)");
		...
		if ( count($delete_ids) > 0 )
		{
			...
			$DB->query("DELETE FROM acf_posts WHERE pid IN ($pids)");
	...
	function mod_topicview()
	{
		...
		$DB->query("SELECT tid, title FROM acf_topics WHERE tid='".$ibforums->input['tid']."'");
		...
		$DB->query( "SELECT p.*, ".
				    ...
				    "FROM acf_posts p, acf_members m, acf_groups g, acf_topics t ".
	...
	function mod_posts()
	{
		...
		$DB->query("SELECT t.*, COUNT(p.pid) as reply_count FROM acf_topics t, acf_posts p WHERE p.queued=1 AND p.new_topic <> 1 AND t.tid=p.topic_id AND t.forum_id='".$this->forum['id']."' GROUP BY t.tid ORDER BY p.post_date ASC");
	...
	function domod_topics()
	{
		...
		if ( count($approve_ids) > 0 )
		{
			...
			$DB->query("UPDATE acf_topics SET approved=1 WHERE tid IN ($tids)");
			
			$DB->query("UPDATE acf_posts SET queued=0 WHERE topic_id IN ($tids)");
			
			// Update the posters ..er.. post count.
			
			$DB->query("SELECT starter_id FROM acf_topics WHERE tid IN ($tids)");
		...
		if ( count($delete_ids) > 0 )
		{
			...
			$DB->query("DELETE FROM acf_topics WHERE tid IN ($tids)");
			
			$DB->query("DELETE FROM acf_posts WHERE topic_id IN ($tids)");
	...
	function mod_topics()
	{
		...
		$DB->query("SELECT COUNT(tid) as tcount FROM acf_topics WHERE approved=0 and forum_id='".$this->forum['id']."'");
		$count = $DB->fetch_row();
	...
	function prune_juice()
	{
		...
		if ($ibforums->input['check'] == 1)
		{
			...
			$DB->query("SELECT COUNT(tid) as tcount FROM acf_topics WHERE approved=1 and forum_id='".$this->forum['id']."'");
			$tcount = $DB->fetch_row();
			
			$db_query = "SELECT COUNT(*) as count FROM acf_topics WHERE approved=1 and forum_id='".$this->forum['id']."'";
			...
			{
				$DB->query("SELECT id FROM acf_members WHERE name='".$ibforums->input['member']."'");
	...
	function find_user_two()
	{
		...
		$DB->query("SELECT id, name FROM acf_members WHERE name LIKE '".$ibforums->input['name']."%' LIMIT 0,100");
	...
	function edit_user()
	{
		...
		$DB->query("SELECT m.*, g.* FROM acf_members m, acf_groups g WHERE m.id='".$ibforums->input['memberid']."' AND m.mgroup=g.g_id");
	...
	function complete_user_edit()
	{
		...
		if ($ibforums->input['photo'] == 1)
		{
			...
			$DB->query("SELECT id FROM acf_member_extra WHERE id={$member['id']}");
		
			if ( $DB->get_num_rows() )
			{
				$DB->query("UPDATE acf_member_extra SET photo_location='', photo_type='', photo_dimensions='' WHERE id={$member['id']}");
			}
			else
			{
				$DB->query("INSERT INTO acf_member_extra SET photo_location='', photo_type='', photo_dimensions='', id={$member['id']}");
			}
		...
		$DB->query("UPDATE acf_members SET $db_string WHERE id='".$ibforums->input['memberid']."'");
	...
	function splash()
	{
		...
		$DB->query("SELECT COUNT(tid) as count FROM acf_topics WHERE approved <> 1 and forum_id='".$this->forum['id']."'");
		$row = $DB->fetch_row();
		...
		$DB->query("SELECT COUNT(pid) as pcount FROM acf_posts WHERE queued=1 and new_topic <> 1 and forum_id='".$this->forum['id']."'");
		$row = $DB->fetch_row();
	...
	function do_move() {
		...
		$DB->query("SELECT subwrap, id, sub_can_post FROM acf_forums WHERE id='$moveto'");
...
//+---------------------------------------------------------------------------------------------
	...
	function moderate_log($title = 'unknown') {
		...
		$DB->query("INSERT INTO acf_moderator_logs (" .$db_string['FIELD_NAMES']. ") VALUES (". $db_string['FIELD_VALUES'] .")");
	...
	function load_forum($fid="")
	{
		...
		$DB->query("SELECT * FROM acf_forums WHERE id=$fid");
