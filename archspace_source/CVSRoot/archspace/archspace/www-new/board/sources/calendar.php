...
class calendar {
	...
	function edit()
	{
        ...
        $DB->query("SELECT * FROM acf_calendar_events WHERE eventid=$eventid");
	...
	function do_edit($del=0)
	{
        ...
        {
        	$DB->query("DELETE FROM acf_calendar_events WHERE eventid='$eventid'");
        ...
        else
        {
			...
			$DB->query("UPDATE acf_calendar_events SET $db_string WHERE eventid='$eventid'");
	...
	function show_month() {
        ...
        if ( $ibforums->vars['show_bday_calendar'] )
        {
			...
			$DB->query("SELECT bday_day from acf_members WHERE bday_month=".$this->chosen_month);
        ...
        $DB->query("SELECT * FROM acf_calendar_events
    ...
    function add_new_event() {
		...
		$DB->query("INSERT INTO acf_calendar_events (" .$db_string['FIELD_NAMES']. ") VALUES (". $db_string['FIELD_VALUES'] .")");
    ...
    function show_day()
    {
        ...
        $eq = $DB->query("SELECT * FROM acf_calendar_events WHERE month='$month' AND mday='$day' AND year='$year'
        				  OR (event_ranged=1 AND ( unix_stamp < $timenow AND end_unix_stamp > $timenow ) )");
    ...
    function make_event_html($event)
    {
        ...
        $DB->query("SELECT m.id, m.name, m.mgroup, m.posts, m.joined, m.avatar, m.avatar_size, ".
                   "g.g_id, g.g_title FROM acf_members m, acf_groups g WHERE m.id='".$event['userid']."' AND g.g_id=m.mgroup");
    ...
    function make_birthday_html($month, $day, $switch=0) {
        ...
        $DB->query("SELECT id, name, bday_year from acf_members WHERE bday_month='".$month."' and bday_day='$day'");
