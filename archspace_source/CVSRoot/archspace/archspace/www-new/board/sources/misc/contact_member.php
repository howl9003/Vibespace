...
class Contact {
	...
	function show_integ()
	{
		...
		$DB->query("SELECT name, id, integ_msg from acf_members WHERE id='".$ibforums->input['MID']."'");
	...
	function send_report()
	{
		...
		$DB->query("SELECT title FROM acf_topics WHERE tid='$tid'");
		...
		$DB->query("SELECT m.name, m.email, mod.member_id FROM acf_moderators mod, acf_members m WHERE mod.forum_id='$fid' and mod.member_id=m.id");
		...
		else
		{
			...
			$DB->query("SELECT m.id, m.name, m.email FROM acf_members m, acf_groups g WHERE g.g_access_cp=1 AND m.mgroup=g.g_id");
    ...
    function check_access($fid, $tid)
    {
		...
		$DB->query("SELECT t.title as topic_title, f.*, c.id as cat_id, c.name as cat_name from acf_forums f, acf_categories c, acf_topics t WHERE f.id=".$fid." and c.id=f.category and t.tid=$tid");
	...
	function show_msn() {
		...
		$DB->query("SELECT name, id, msnname from acf_members WHERE id='".$ibforums->input['MID']."'");
	...
	function show_yahoo() {
		...
		$DB->query("SELECT name, id, yahoo from acf_members WHERE id='".$ibforums->input['MID']."'");
	...
	function show_aim() {
		...
		$DB->query("SELECT name, id, aim_name from acf_members WHERE id='".$ibforums->input['MID']."'");
	...
	function show_icq() {
		...
		$DB->query("SELECT name, id, icq_number from acf_members WHERE id='".$ibforums->input['MID']."'");
	...
	function mail_member_form($errors="", $extra = "")
	{
		...
		$DB->query("SELECT name, id, email, hide_email from acf_members WHERE id=".$ibforums->input['MID']);
	...
	function mail_member_send()
	{
		...
		$DB->query("SELECT name, id, email, hide_email from acf_members WHERE id=".$ibforums->input['to']);
		...
		$DB->query("INSERT INTO acf_email_logs ({$db_string['FIELD_NAMES']}) VALUES({$db_string['FIELD_VALUES']})");	
	...
	function _allow_to_email($member_id, $email_limit)
	{
		...
		if ( $flood )
		{
			$DB->query("SELECT * FROM acf_email_logs WHERE from_member_id=$member_id ORDER BY email_date DESC LIMIT 0,1");
		...
		if ( $limit )
		{
			...
			$DB->query("SELECT count(email_id) as cnt FROM acf_email_logs WHERE from_member_id=$member_id AND email_date > $time_range");
