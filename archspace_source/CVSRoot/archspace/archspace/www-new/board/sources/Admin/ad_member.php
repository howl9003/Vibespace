...
|   Time: Wed, 12 May 2004 05:31:36 GMT
|   Release: 46e092d368135b069b566ab7f12ec108
...
class ad_forums {
	...
	function do_bulk_mail() {
		...
		$ids = array();
 		...
 		$DB->query("SELECT COUNT(id) as total FROM acf_members WHERE mgroup IN($group_str)".$where);
		...
		$DB->query("SELECT * FROM acf_stats");
		$stats = $DB->fetch_row();
		...
		$DB->query("SELECT email FROM acf_members WHERE mgroup IN($group_str)".$where);
	...
	function bulk_mail_form($preview=0, $title='Preview', $content="") {
		...
		$DB->query("SELECT g_id, g_title FROM acf_groups WHERE g_id <> ".$INFO['guest_group']." ORDER BY g_title");
	...
	function temp_ban_start()
	{
		...
		$DB->query("SELECT * FROM acf_members WHERE id='".$IN['mid']."'");
	...
	function unsuspend()
	{
		...
		if ($IN['mid'] == 'all')
		{
			$DB->query("UPDATE acf_members SET temp_ban=''");
		...
		else
		{
			...
			$DB->query("UPDATE acf_members SET temp_ban='' WHERE id=$mid");
			
			$DB->query("SELECT name FROM acf_members WHERE id=$mid");
	...
	function temp_ban_complete()
	{
		...
		$DB->query("UPDATE acf_members SET temp_ban='$new_ban' WHERE id={$IN['mid']}");
	...
	function change_name_complete()
	{
		...
		$DB->query("SELECT name, email FROM acf_members WHERE id='".$IN['mid']."'");
		...
		$DB->query("SELECT id FROM acf_members WHERE LOWER(name)='".strtolower($new_name)."'");
		...
		$DB->query("UPDATE acf_members SET name='$new_name' WHERE id='$mid'");
		$DB->query("UPDATE acf_contacts SET contact_name='$new_name' WHERE contact_id='$mid'");
		$DB->query("UPDATE acf_forums SET last_poster_name='$new_name' WHERE last_poster_id='$mid'");
		$DB->query("UPDATE acf_moderator_logs SET member_name='$new_name' WHERE member_id='$mid'");
		$DB->query("UPDATE acf_moderators SET member_name='$new_name' WHERE member_id='$mid'");
		$DB->query("UPDATE acf_posts SET author_name='$new_name' WHERE author_id='$mid'");
		$DB->query("UPDATE acf_sessions SET member_name='$new_name' WHERE member_id='$mid'");
		$DB->query("UPDATE acf_topics SET starter_name='$new_name' WHERE starter_id='$mid'");
		$DB->query("UPDATE acf_topics SET last_poster_name='$new_name' WHERE last_poster_id='$mid'");
		...
		$DB->query("SELECT id, name FROM acf_members WHERE mgroup <> '".$INFO['auth_group']."' ORDER BY id DESC LIMIT 0,1");
		$r = $DB->fetch_row();
		$stats['LAST_MEM_NAME'] = $r['name'];
		$stats['LAST_MEM_ID']   = $r['id'];
		
		$db_string = $DB->compile_db_update_string( $stats );
		$DB->query("UPDATE acf_stats SET $db_string");
	...
	function change_name_start($message="") {
		...
		$DB->query("SELECT name FROM acf_members WHERE id='".$IN['mid']."'");
	...
	function domod() {
		...
		if ($IN['type'] == 'approve')
		{
			...
			$main = $DB->query("SELECT m.id, m.email, m.mgroup, v.* FROM acf_validating v
								 LEFT JOIN acf_members m ON (v.member_id=m.id)
			...
			while( $row = $DB->fetch_row( $main ) )
			{
				...
				$update = $DB->query("UPDATE acf_members SET mgroup=".$row['real_group']." WHERE id=".$row['id']);
			...
			$DB->query("DELETE FROM acf_validating WHERE member_id IN(".implode( ",",$ids ).")");
			
			$DB->query("SELECT id, name FROM acf_members WHERE mgroup <> ".$INFO['auth_group']." ORDER BY id DESC LIMIT 0,1");
			$r = $DB->fetch_row();

			$DB->query("UPDATE acf_stats SET MEM_COUNT=MEM_COUNT+".count($ids).", LAST_MEM_NAME='{$r['name']}', LAST_MEM_ID='{$r['id']}'");
		...
		{
			$DB->query("DELETE FROM acf_members WHERE id IN(".implode( ",",$ids ).")");
			
			$DB->query("DELETE FROM acf_member_extra WHERE id IN(".implode( ",",$ids ).")");
			
			// Delete member messages...
		
			$DB->query("DELETE from acf_messages WHERE member_id IN(".implode( ",",$ids ).")");
			$DB->query("DELETE from acf_contacts WHERE member_id IN(".implode( ",",$ids ).") or contact_id IN(".implode( ",",$ids ).")");
			
			$DB->query("DELETE FROM acf_validating WHERE member_id IN(".implode( ",",$ids ).")");
			
			$DB->query("DELETE from acf_pfields_content WHERE member_id IN(".implode( ",",$ids ).")");
			
			$DB->query("DELETE from acf_warn_logs WHERE wlog_mid IN(".implode( ",",$ids ).")");
			
			// Convert their posts and topics into guest postings..
		
			$DB->query("UPDATE acf_posts SET author_id='0' WHERE author_id IN(".implode( ",",$ids ).")");
		
			$DB->query("UPDATE acf_topics SET starter_id='0' WHERE starter_id IN(".implode( ",",$ids ).")");
	...
	function view_mod() {
		...
		$DB->query("SELECT COUNT(vid) as mcount FROM acf_validating WHERE lost_pass <> 1");
	...
	function titles() {
		...
		$DB->query("SELECT macro_id, img_dir FROM acf_skins WHERE default_set=1");
		
		$mid = $DB->fetch_row();
		
		$DB->query("SELECT macro_replace AS A_STAR FROM acf_macro WHERE macro_set={$mid['macro_id']} AND macro_value='A_STAR'");
		...
		$DB->query("SELECT * FROM acf_titles ORDER BY posts");
	...
	function add_rank() {
		...
		$DB->query("INSERT INTO acf_titles (" .$db_string['FIELD_NAMES']. ") VALUES (". $db_string['FIELD_VALUES'] .")");
	...
	function delete_rank() {
		...
		$DB->query("DELETE FROM acf_titles WHERE id='".$IN['id']."'");
	...
	function edit_rank() {
		...
		$DB->query("UPDATE acf_titles SET $db_string WHERE id='".$IN['id']."'");
	...
	function rank_setup($mode='edit') {
		...
		if ($mode == 'edit')
		{
			...
			$DB->query("SELECT * from acf_titles WHERE id='".$IN['id']."'");
			$rank = $DB->fetch_row();
	...
	function delete_form() {
		...
		$DB->query("SELECT g_id, g_title FROM acf_groups ORDER BY g_title");
	...
	function prune_confirm() {
		...
		$query = "SELECT COUNT(id) as mcount FROM acf_members WHERE";
		...
		if ($count['mcount'] < 101)
		{
			$DB->query("SELECT id, name FROM acf_members WHERE $additional_query");
	...
	function doprune() {
		...
		$DB->query("SELECT id FROM acf_members WHERE ".$query);
		...
		$DB->query("UPDATE acf_posts SET author_id='0' WHERE author_id IN(".$id_string.")");
		
		$DB->query("UPDATE acf_topics SET starter_id='0' WHERE starter_id IN(".$id_string.")");
		
		// Delete member...
		
		$DB->query("DELETE from acf_members WHERE id IN(".$id_string.")");
		
		$DB->query("DELETE from acf_pfields_content WHERE member_id IN(".$id_string.")");
		
		// Delete member messages...
		
		$DB->query("DELETE from acf_messages WHERE member_id IN (".$id_string.")");
		
		// Delete member subscriptions.
		
		$DB->query("DELETE from acf_tracker WHERE member_id IN (".$id_string.")");
		
		// Delete from validating..
		
		$DB->query("DELETE FROM acf_validating WHERE member_id IN (".$id_string.")");
		
		// Set the stats DB straight.
		
		$DB->query("SELECT id, name FROM acf_members WHERE mgroup <> '".$INFO['auth_group']."' ORDER BY joined DESC LIMIT 0,1");
		
		$memb = $DB->fetch_row();
		
		$DB->query("SELECT COUNT(id) as members from acf_members WHERE mgroup <> '".$INFO['auth_group']."'");
		$r = $DB->fetch_row();
		// Remove "guest" account...
		$r['members']--;
		$r['members'] < 1 ? 0 : $r['members'];
		
		$DB->query("UPDATE acf_stats SET ".
			             "MEM_COUNT={$r['members']}, ".
	...
	function delete_lookup_form() {
		...
		$DB->query("SELECT id, name FROM acf_members WHERE name LIKE '".$IN['USER_NAME']."%'");
	...
	function dodelete() {
		...
		$DB->query("SELECT * FROM acf_members WHERE id='".$IN['MEMBER_ID']."'");
		$mem = $DB->fetch_row();
		...
		$DB->query("UPDATE acf_posts SET author_id='0' WHERE author_id='".$IN['MEMBER_ID']."'");
		
		$DB->query("UPDATE acf_topics SET starter_id='0' WHERE starter_id='".$IN['MEMBER_ID']."'");
		
		// Delete member...
		
		$DB->query("DELETE from acf_members WHERE id='".$IN['MEMBER_ID']."'");
		$DB->query("DELETE from acf_pfields_content WHERE member_id='".$IN['MEMBER_ID']."'");
		$DB->query("DELETE from acf_member_extra WHERE id='".$IN['MEMBER_ID']."'");
		
		// Delete member messages...
		
		$DB->query("DELETE from acf_messages WHERE member_id='".$IN['MEMBER_ID']."'");
		$DB->query("DELETE from acf_contacts WHERE member_id='".$IN['MEMBER_ID']."' or contact_id='".$IN['MEMBER_ID']."'");
		
		// Delete member subscriptions.
		
		$DB->query("DELETE from acf_tracker WHERE member_id='".$IN['MEMBER_ID']."'");
		$DB->query("DELETE from acf_forum_tracker WHERE member_id='".$IN['MEMBER_ID']."'");
		$DB->query("DELETE from acf_warn_logs WHERE wlog_mid='".$IN['MEMBER_ID']."'");
		
		// Delete from validating..
		
		$DB->query("DELETE FROM acf_validating WHERE member_id='".$IN['MEMBER_ID']."'");
	...
	function add_form() {
		...
		$DB->query("SELECT * from acf_pfields_content WHERE member_id='".$IN['MEMBER_ID']."'");
		...
		$DB->query("SELECT * from acf_pfields_data WHERE fshowreg=1 ORDER BY forder");
	...
	function do_add()
	{
		...
		$DB->query("SELECT id FROM acf_members WHERE LOWER(name)='".$IN['name']."'");
		...
		$DB->query("SELECT * from acf_pfields_data");
		...
		$DB->query("SELECT MAX(id) as new_id FROM acf_members");
		$r = $DB->fetch_row();
		...
		$DB->query("INSERT INTO acf_members (" .$db_string['FIELD_NAMES']. ") VALUES (". $db_string['FIELD_VALUES'] .")");
		...
		if ( count($custom_fields) > 0 )
		{
			...
			$DB->query("INSERT INTO acf_pfields_content (".$db_string['FIELD_NAMES'].") VALUES(".$db_string['FIELD_VALUES'].")");
	...
	function do_advanced_search($basic=0) {
		...
		$query = "SELECT m.id, m.email, m.name, m.mgroup, m.ip_address, m.posts, m.temp_ban, g.g_title
		          FROM acf_members m
		           LEFT JOIN acf_groups g ON (g.g_id=m.mgroup)
		...
		$DB->query("SELECT COUNT(m.id) as count FROM acf_members m WHERE $rq");
	...
	function do_edit_form() {
		...
		$DB->query("SELECT ldir, lname FROM acf_languages");
		...
		while ( $l = $DB->fetch_row() )
		{
 		...
 		$DB->query("SELECT uid, sid, sname, default_set, hidden FROM acf_skins");
		...
		$DB->query("SELECT * from acf_pfields_data ORDER BY forder");
		...
		$DB->query("SELECT * FROM acf_forum_perms");
	...
	function do_edit()
	{
		...
		$DB->query("UPDATE acf_members SET $db_string".$password." WHERE id='".$IN['mid']."'");
		...
		if ( $IN['remove_photo'] )
		{
			$DB->query("SELECT id FROM acf_member_extra WHERE id={$IN['mid']}");
		
			if ( $DB->get_num_rows() )
			{
				$DB->query("UPDATE acf_member_extra SET photo_location='', photo_type='', photo_dimensions='' WHERE id={$IN['mid']}");
			}
			else
			{
				$DB->query("INSERT INTO acf_member_extra SET photo_location='', photo_type='', photo_dimensions='', id={$IN['mid']}");
			}
		...
		if ( count($custom_fields) > 0 )
		{
			...
			$DB->query("SELECT member_id FROM acf_pfields_content WHERE member_id='".$IN['mid']."'");
			$test = $DB->fetch_row();
			...
			if ( $test['member_id'] )
			{
				...
				$DB->query("UPDATE acf_pfields_content SET $db_string WHERE member_id='".$IN['mid']."'");
			}
			else
			{
