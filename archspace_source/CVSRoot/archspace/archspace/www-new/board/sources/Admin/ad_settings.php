...
class ad_settings {
	...
	function fulltext()
	{
		...
		else
		{
			...
			$DB->query("SHOW CREATE TABLE acf_posts");
	...
	function do_fulltext()
	{
		...
		else
		{
			...
			if ( $this->mysql_version >= 32323 )
			{
				...
				$DB->query("SELECT COUNT(*) as cnt from acf_posts");
				...
				else
				{
					...
					$DB->query("alter table acf_topics add fulltext(title)");
					
					$DB->query("alter table acf_posts add fulltext(post)");
	...
	function spider()
	{
		...
		$DB->query("SELECT sname, sid FROM acf_skins ORDER BY sname");
	...
	function do_guidelines()
	{
		...
		$DB->query("SELECT * FROM acf_cache_store WHERE cs_key='boardrules'");

		if ( $row = $DB->fetch_row() )
		{
			$DB->query("UPDATE acf_cache_store SET cs_value='".addslashes($glines)."' WHERE cs_key='boardrules'");
		}
		else
		{
			...
			$DB->query("INSERT INTO acf_cache_store (".$db_string['FIELD_NAMES'].") VALUES(".$db_string['FIELD_VALUES'].")");
		}
	...
	function docount()
	{
		...
		if ($IN['posts'])
		{
			$DB->query("SELECT COUNT(pid) as posts FROM acf_posts WHERE queued <> 1");
			$r = $DB->fetch_row();
			$stats['TOTAL_REPLIES'] = $r['posts'];
			$stats['TOTAL_REPLIES'] < 1 ? 0 : $stats['TOTAL_REPLIES'];
			
			$DB->query("SELECT COUNT(tid) as topics FROM acf_topics WHERE approved = 1");
			$r = $DB->fetch_row();
	...
	function doedit_emoticons()
	{
		...
		$DB->query("UPDATE acf_emoticons SET $db_string WHERE id='".$IN['id']."'");
	...
	function edit_emoticons()
	{
		...
		$DB->query("SELECT * FROM acf_emoticons WHERE id='".$IN['id']."'");
	...
	function remove_emoticons()
	{
		...
		$DB->query("DELETE FROM acf_emoticons WHERE id='".$IN['id']."'");
	...
	function add_emoticons()
	{
		...
		$DB->query("INSERT INTO acf_emoticons (".$db_string['FIELD_NAMES'].") VALUES(".$db_string['FIELD_VALUES'].")");
	...
	function emoticons()
	{
		...
		$DB->query("SELECT * from acf_emoticons");
	...
	function doedit_badword()
	{
		...
		$DB->query("UPDATE acf_badwords SET $db_string WHERE wid='".$IN['id']."'");
	...
	function edit_badword()
	{
		...
		$DB->query("SELECT * FROM acf_badwords WHERE wid='".$IN['id']."'");
	...
	function remove_badword()
	{
		...
		$DB->query("DELETE FROM acf_badwords WHERE wid='".$IN['id']."'");
	...
	function add_badword()
	{
		...
		$DB->query("INSERT INTO acf_badwords (".$db_string['FIELD_NAMES'].") VALUES(".$db_string['FIELD_VALUES'].")");
	...
	function badword()
	{
		...
		$DB->query("SELECT * from acf_badwords");
	...
	function news()
	{
		...
		$DB->query("SELECT id, name FROM acf_forums ORDER BY name");
...
In mysql type:
<br />
<pre>\g alter table acf_topics add fulltext(title);</pre>
<br />
If you are not using 'acf_' as your table extension, adjust that query to suit. This query can take a while
...
In mysql type:
<br />
<pre>\g alter table acf_posts add fulltext(post);</pre>
<br />
If you are not using 'acf_' as your table extension, adjust that query to suit. This query can take a while
