...
class ad_mod {
	...
	function do_delete()
	{
		...
		$DB->query("SELECT * FROM acf_moderators WHERE mid='".$IN['mid']."'");
		$mod = $DB->fetch_row();
		...
		$DB->query("DELETE FROM acf_moderators WHERE mid='".$IN['mid']."'");
	...
	function do_edit()
	{
		...
		$DB->query("SELECT member_name FROM acf_moderators WHERE mid='".$IN['mid']."'");
		$mod = $DB->fetch_row();
		...
		$DB->query("UPDATE acf_moderators SET $db_string WHERE mid='".$IN['mid']."'");
	...
	function add_mod()
	{
		...
		$DB->query("SELECT id FROM acf_forums WHERE id IN(".$IN['fid'].")");
		...
		if ($IN['mod_type'] == 'group')
		{
			...
			$DB->query("SELECT g_id, g_title FROM acf_groups WHERE g_id='".$IN['gid']."'");
			...
			$DB->query("SELECT * FROM acf_moderators WHERE forum_id IN(".$IN['fid'].") and group_id={$IN['gid']}");
		...
		else
		{
			...
			$DB->query("SELECT id, name from acf_members WHERE id='".$IN['mem']."'");
			...
			$DB->query("SELECT * FROM acf_moderators WHERE forum_id IN(".$IN['fid'].") and member_id={$IN['mem']}");
		...
		foreach ($forum_ids as $cartman)
		{
			...
			$DB->query("INSERT INTO acf_moderators (" .$kenny['FIELD_NAMES']. ") VALUES (". $kenny['FIELD_VALUES'] .")");
		}
	...
	function mod_form( $type='add' ) {
		...
		if ($type == 'add')
		{
			...
			$DB->query("SELECT name FROM acf_forums WHERE id IN(".$IN['fid'].")");
			...
			if ($IN['mod_type'] == 'group')
			{
				$DB->query("SELECT g_id, g_title FROM acf_groups WHERE g_id='".$IN['mod_group']."'");
			...
			else
			{
				...
				{
					$DB->query("SELECT name, id FROM acf_members WHERE id='".$IN['MEMBER_ID']."'");
	...
	function show_list() {
		...
		$DB->query("SELECT * from acf_moderators");
		while ($r = $DB->fetch_row())
		{
			$mods[] = $r;
		}
