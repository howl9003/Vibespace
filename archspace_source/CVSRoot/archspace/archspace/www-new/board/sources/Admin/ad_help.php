...
class ad_settings {
	...
	function doedit()
	{
		...
		$DB->query("UPDATE acf_faq SET $db_string WHERE id='".$IN['id']."'");
	...
	function show_form($type='new')
	{
		...
		if ($type != 'new')
		{
			...
			$DB->query("SELECT * FROM acf_faq WHERE id='".$IN['id']."'");
	...
	function remove()
	{
		...
		$DB->query("DELETE FROM acf_faq WHERE id='".$IN['id']."'");
	...
	function doadd()
	{
		...
		$DB->query("INSERT INTO acf_faq (".$db_string['FIELD_NAMES'].") VALUES(".$db_string['FIELD_VALUES'].")");
	...
	function list_files()
	{
		...
		$DB->query("SELECT * from acf_faq ORDER BY id ASC");
