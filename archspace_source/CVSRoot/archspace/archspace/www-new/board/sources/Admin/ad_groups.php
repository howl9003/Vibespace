...
class ad_groups {
	...
	function delete_mask()
	{
		...
		$DB->query("DELETE FROM acf_forum_perms WHERE perm_id='".$IN['id']."'");
		...
		$get = $DB->query("SELECT id, read_perms, reply_perms, start_perms, upload_perms FROM acf_forums");
		...
		while( $f = $DB->fetch_row($get) )
		{
			...
			if ( count($d_arr) > 0 )
			{
				...
				if ( strlen($d_str) > 5)
				{
					$save = $DB->query("UPDATE acf_forums SET $d_str WHERE id={$f['id']}");
				}
			}
		}
	...
	function add_new_perm()
	{
		...
		$DB->query("INSERT INTO acf_forum_perms SET perm_name='".$IN['new_perm_name']."'");
	...
	function preview_forums()
	{
		...
		$DB->query("SELECT * FROM acf_forum_perms WHERE perm_id='".$IN['id']."'");
	...
	function remove_mask()
	{
		...
		$DB->query("SELECT id, name, org_perm_id FROM acf_members WHERE id=".intval($IN['id']));
		...
		if ( $IN['pid'] == 'all' )
		{
			$DB->query("UPDATE acf_members SET org_perm_id=0 WHERE id=".intval($IN['id']));
		}
		else
		{
			...
			else
			{
				...
				$DB->query("UPDATE acf_members SET org_perm_id='".implode(",",$new_arr)."' WHERE id=".intval($IN['id']));
			}	
	...
	function view_perm_users()
	{
		...
		$outer = $DB->query("SELECT id, name, email, posts, org_perm_id FROM acf_members WHERE (org_perm_id IS NOT NULL AND org_perm_id <> 0) ORDER BY name");
			...
			foreach( explode( ",", $r['org_perm_id'] ) as $pid )
			{
				if ( $pid == $IN['id'] )
				{
					if ( count($exp_pid) > 1 )
					{
						...
						$DB->query("SELECT * FROM acf_forum_perms WHERE perm_id IN ({$r['org_perm_id']}) AND perm_id <> {$IN['id']}");
	...
	function permsplash()
	{
		...
		$DB->query("SELECT COUNT(id) as count, org_perm_id FROM acf_members WHERE (org_perm_id IS NOT NULL AND org_perm_id <> 0) GROUP by org_perm_id");
		...
		$DB->query("SELECT g_id, g_title, g_perm_id FROM acf_groups");
	...
	function edit_name_perm()
	{
		...
		$DB->query("UPDATE acf_forum_perms SET perm_name='{$IN['perm_name']}' WHERE perm_id='".$IN['id']."'");
	...
	function do_forum_perms()
	{
		...
		$forum_q = $DB->query("SELECT id, read_perms, start_perms, reply_perms, upload_perms FROM acf_forums ORDER BY position ASC");
		...
		while ( $row = $DB->fetch_row( $forum_q ) )
		{
			...
			if (! $new_q = $DB->query("UPDATE acf_forums SET read_perms='$read', reply_perms='$reply', start_perms='$start', upload_perms='$upload' WHERE id='".$row['id']."'") )
			{
	...
	function delete_form()
	{
		...
		$DB->query("SELECT COUNT(id) as users FROM acf_members WHERE mgroup='".$IN['id']."'");
		...
		$DB->query("SELECT g_title FROM acf_groups WHERE g_id='".$IN['id']."'");
		$group = $DB->fetch_row();
		
		//+-------------------------------
		
		$DB->query("SELECT g_id, g_title FROM acf_groups WHERE g_id <> '".$IN['id']."'");
	...
	function do_delete()
	{
		...
		$DB->query("SELECT g_id FROM acf_groups WHERE g_id IN(".$IN['id'].",".$IN['to_id'].")");
		...
		$DB->query("UPDATE acf_members SET mgroup='".$IN['to_id']."' WHERE mgroup='".$IN['id']."'");
		
		$DB->query("DELETE FROM acf_groups WHERE g_id='".$IN['id']."'");
		
		// Look for promotions in case we have members to be promoted to this group...
		
		$prq = $DB->query("SELECT g_id FROM acf_groups WHERE g_promotion LIKE '{$IN['id']}&%'");
		
		while ( $row = $DB->fetch_row($prq) )
		{
			$nq = $DB->query("UPDATE acf_groups SET g_promotion='-1&-1' WHERE g_id='".$row['g_id']."'");
		}
		
		// Remove from moderators table
		
		$DB->query("DELETE FROM acf_moderators WHERE is_group=1 AND group_id=".$IN['id']);
	...
	function save_group($type='edit')
	{
		...
		if ($type == 'edit')
		{
			...
			$DB->query("UPDATE acf_groups SET $rstring WHERE g_id='".$IN['id']."'");
			
			// Update the title of the group held in the mod table incase it changed.
			
			$DB->query("UPDATE acf_moderators SET group_name='".trim($IN['g_title'])."' WHERE group_id='".$IN['id']."'");
		...
		else
		{
			...
			$DB->query("INSERT INTO acf_groups (" .$rstring['FIELD_NAMES']. ") VALUES (". $rstring['FIELD_VALUES'] .")");
	...
	function group_form($type='edit')
	{
		...
		if ($IN['id'] != "")
		{
			$DB->query("SELECT * FROM acf_groups WHERE g_id='".$IN['id']."'");
			$group = $DB->fetch_row();
			
			$query = "SELECT g_id, g_title FROM acf_groups WHERE g_id <> {$IN['id']} ORDER BY g_title";
		}
		else
		{
			$group = array();
			
			$query = "SELECT g_id, g_title FROM acf_groups ORDER BY g_title";
		}
	...
	function main_screen()
	{
		...
		$DB->query("SELECT acf_groups.g_id, acf_groups.g_access_cp, acf_groups.g_is_supmod, acf_groups.g_title,acf_groups.prefix, acf_groups.suffix, COUNT(acf_members.id) as count FROM acf_groups "
		          ."LEFT JOIN acf_members ON (acf_members.mgroup = acf_groups.g_id) "
		          ."GROUP BY acf_groups.g_id ORDER BY acf_groups.g_title");
