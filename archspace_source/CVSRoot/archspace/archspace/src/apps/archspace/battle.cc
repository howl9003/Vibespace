	...
	CBattleFleet::init_common()
{
	...
	switch( mCommander->get_racial_ability() ){
		...
		case CAdmiral::RA_LONE_WOLF:
			mBerserkModifier += 5;
			NewEffect = new CFleetEffect( CFleetEffect::FE_MOBILITY, mCommander->get_level()* 5 / 2, CFleetEffect::AT_PROPORTIONAL );
			add_static_effect( NewEffect );
			NewEffect = new CFleetEffect( CFleetEffect::FE_STEALTH,  mCommander->get_level()* 5 / 2, CFleetEffect::AT_PROPORTIONAL );
			add_static_effect( NewEffect );
			if( mCommander->get_level() >= 5 )
		...
		case CAdmiral::RA_CLONAL_DOUBLE :
			NewEffect = new CFleetEffect( CFleetEffect::FE_DR, mCommander->get_level() * 3 / 2, CFleetEffect::AT_PROPORTIONAL );
			add_static_effect( NewEffect );
			break;
		...
		case CAdmiral::RA_ARTIFACT_COOLING_ENGINE :
			NewEffect = new CFleetEffect( CFleetEffect::FE_COOLING_TIME, -mCommander->get_level() * 9 / 4, CFleetEffect::AT_PROPORTIONAL );
			add_static_effect( NewEffect );
			break;
		case CAdmiral::RA_LYING_DORMANT :
			mAttribute += WEAK_CLOAKING;
			NewEffect = new CFleetEffect( CFleetEffect::FE_STEALTH, mCommander->get_level() * 7 / 2, CFleetEffect::AT_PROPORTIONAL );
			add_static_effect( NewEffect );
		...
		case CAdmiral::RA_PATTERN_BROADCASTER :
			...
			add_static_effect( NewEffect );*/
			NewEffect = new CFleetEffect( CFleetEffect::FE_DR_AGAINST_MISSILE, mCommander->get_level() * 5, CFleetEffect::AT_PROPORTIONAL );
			add_static_effect( NewEffect );
			break;
		case CAdmiral::RA_FAMOUS_PRIVATEER :
			...
			NewEffect = new CFleetEffect( CFleetEffect::FE_STEALTH, 10 + mCommander->get_level() * 3, CFleetEffect::AT_PROPORTIONAL );
			add_static_effect( NewEffect );
			if( mCommander->get_level() >= 7 ){
				mAttribute += WEAK_CLOAKING;
			}
		...
		case CAdmiral::RA_BLITZKRIEG :
			NewEffect = new CFleetEffect( CFleetEffect::FE_DAMAGE, mCommander->get_level() * 5 / 4, CFleetEffect::AT_PROPORTIONAL );
			add_static_effect( NewEffect );
			NewEffect = new CFleetEffect( CFleetEffect::FE_CRITICAL_HIT, mCommander->get_level() / 2, CFleetEffect::AT_ABSOLUTE );
			add_static_effect( NewEffect );
			break;
