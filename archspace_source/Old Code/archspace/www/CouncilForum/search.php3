...
/***************************************************************************
    ...
    $Id: search.php,v 1.12 2001/01/22 18:30:54 bartvb Exp $
...
<TR>
	...
	<TD BGCOLOR="<?php echo $color2?>" WIDTH="50%">
		<select name="forum">
<?
		$query = "SELECT forum_name, forum_id FROM forums WHERE cat_id=$COUNCIL_ID";
		if(!$result = mysql_query($query,$db))
		{
			die("<font size=+1>An Error Occured</font><hr>Council Forum was unable to query the forums database");
		}
...
$query = "SELECT u.user_id,f.forum_id,p.topic_id, u.username, p.post_time,t.topic_title,f.forum_name FROM posts p, users u, forums f,topics t";
if(isset($term)&&$term!="")
{
...
if(isset($forum) && $forum!="all")
{
	if(isset($addquery)) {
	   ...
	   $subquery .=" AND p.forum_id=$forum AND f.forum_id=$forum";
	}
	else {
	   $addquery .=" p.forum_id=$forum AND f.forum_id=$forum";
	   $subquery .=" p.forum_id=$forum AND f.forum_id=$forum";
	}
}
if(isset($username)&&$username!="")
{
   if(!$result = mysql_query("SELECT user_id FROM users WHERE username='$username'",$db))
     {
	die("<font size=+1>An Error Occured</font><hr>phpBB was unable to query the forums database");
     }
   $row = @mysql_fetch_array($result);
   if(!$row)
     die("That user does not exist.  Please go back and search again.");
   $userid = $row[user_id];
   if(isset($addquery)) {
      $addquery.=" AND p.poster_id=$userid AND u.username='$username'";
      $subquery.=" AND p.poster_id=$userid AND u.username='$username'";
   }
   else {
      $addquery.=" p.poster_id=$userid AND u.username='$username'";
      $subquery.=" p.poster_id=$userid AND u.username='$username'";
   }
}	
if(isset($addquery)) {
   switch ($searchboth) { 
   ...
   $query .= " p.topic_id = t.topic_id AND p.forum_id = f.forum_id AND p.poster_id = u.user_id AND f.forum_type != 1";
...
//		echo "<TD ALIGN=\"CENTER\" WIDTH=\"25%\"><a href=\"bb_profile.$phpEx?mode=view&user=$row[user_id]\">$row[username]</a></TD>";
		echo "<TD ALIGN=\"CENTER\" WIDTH=\"25%\">$row[username]</TD>";
