...
{
	die("Error - The forum/topic you selected does not exist. Please go back and try again.");
}
...
//     $posterdata = array("user_id" => -1, "username" => $l_anonymous, "user_posts" => "0", "user_rank" => -1);
   ...
   $posterdata = get_game_name_from_postid($myrow[post_id], $db);

   echo "<TD valign=top><FONT FACE=\"$FontFace\" COLOR=\"$textcolor\"><b>$posterdata[poster_name]</b></FONT>";
// $posterdata[username]
...
/*
   if($posterdata[user_id] != -1)
   {
	...
	die("Error connecting to the database!");
      ...
      echo "<BR><FONT FACE=\"$FontFace\" SIZE=\"$FontSize1\" COLOR=\"$textcolor\"><B>" . stripslashes($rank) . "</B></font>";
*/

      if($rank_image != '')
	echo "<BR><IMG SRC=\"$url_images/$rank_image\" BORDER=\"0\">";
//      echo "<BR><FONT FACE=\"$FontFace\" SIZE=\"$FontSize1\" COLOR=\"$textcolor\">$l_joined: $posterdata[user_regdate]</FONT>";
...
*/
 echo "<HR>\n";
   ...
   $message = eregi_replace("\[addsig]$", "<BR>_________________<BR>" . stripslashes(bbencode($sig)), $message);
...
<TR>
	<TD>
		<a href="newtopic.<?php echo $phpEx?>?forum=<?php echo $forum?>"><IMG SRC="<?php echo $NEW_TOPIC_IMAGE?>" BORDER="0"></a>&nbsp;&nbsp;
<?php
		if($lock_state != 1) {
?>
