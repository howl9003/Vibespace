...
/***************************************************************************
    ...
    $Id: bb_profile.php,v 1.48 2001/02/08 23:17:31 bartvb Exp $
...
?>
	...
	<TR ALIGN="LEFT">
		...
		<TD  BGCOLOR="<?php echo $color2?>"><FONT FACE="<?php echo $FontFace?>" SIZE="<?php echo $FontSize2?>" COLOR="<?php echo $textcolor?>"><?php echo $userdata[username]?></FONT>
	        <font size=-2>(<a href="search.<?php echo $phpEx?>?term=&addterms=any&forum=all&username=<?php echo $userdata[username]?>&sortby=p.post_time&searchboth=both&submit=Search"><?php echo $l_viewpostuser?></a>)
			  &nbsp;&nbsp;(<a href="sendpmsg.<?php echo $phpEx?>?tousername=<?php echo urlencode($userdata[username])?>"><?php echo $l_sendpmsg?></a>)</font></TD>
	</TR>
	<TR ALIGN="LEFT">
	...
	case 'edit':
	   if ($submit || $user_logged_in) {
		 ...
		 if(!$result = mysql_query($sql, $db)) {
		    error_die("Could not update userinfo in database.");
		 }
