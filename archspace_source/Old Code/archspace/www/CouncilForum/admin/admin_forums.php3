...
/***************************************************************************
    ...
    $Id: admin_forums.php,v 1.34 2001/01/16 05:25:58 thefinn Exp $
 ***************************************************************************/
 ...
 case 'editforum':
   if($save) {
	 ...
	 if(isset($rem_mods) && $total > 1) {
	    while(list($null, $mod) = each($rem_mods)) {
	 ...
	 echo "</tr><TR><TD><TABLE width=\"100%\" cellspacing=\"0\" cellpadding=\"0\"><TR>";
	 echo "<td align=\"center\" width=\"100%\" bgcolor=\"$color2\"><font face=\"$FontFace\" size=\"$FontSize1\" color=\"$textcolor\"><P><BR>&nbsp;&nbsp;Forum removed from database along with all its posts.<P>Click <a href=\"$url_admin\">here</a> to return to the Administration Panel.<P>Click <a href=\"$url_phpbb\">here</a> to return to the forum index.</font><P><BR><P></TD>";
	 echo "</TR></table></TD></TR></TABLE>";
      }
   }
   if($submit && !$save) {
   ...
   if($submit) {
	...
	die("An Error Occurred<HR>Could not contact the database. Please check your config file.<BR>".mysql_error()."<BR>$sql");
	 ...
	 if(!mysql_query($user_query, $db))
      ...
      echo "</tr><TR><TD><TABLE width=\"100%\" cellspacing=\"0\" cellpadding=\"0\"><TR>";
      echo "<td align=\"center\" width=\"100%\" bgcolor=\"$color2\"><font face=\"$FontFace\" size=\"$FontSize1\" color=\"$textcolor\"><P><BR>&nbsp;&nbsp;Click <a href=\"$url_admin/index.$phpEx\">here</a> to return to the Administration Panel.<P>Click <a href=\"$url_phpbb/viewforum.$phpEx?forum=$forum\">here</a> to view the forum  you just created.</font><P><BR><P></TD>";
      echo "</TR></table></TD></TR></TABLE>";
   }
   else {
