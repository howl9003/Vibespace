...
if ($BBStatus eq "OFF") {
&StandardHTML("죄송합니다. 작업으로인해 게시판 사용을 할 수 없습니다. 잠시후에 다시 접속해주시기 바랍니다. <A HREF=\"mailto:$BBEmail\">$BBEmail</A>");
exit;
}
...
sub Intro
{
	...
	if (-e "$NonCGIPath/Archives/archives.pl")
	{
		...
		if ($archives[0] ne "")
		{
			...
			$ArchiveLine = qq(<FORM ACTION="$CGIURL/forumdisplay.cgi" METHOD="POST">
...
<SELECT NAME="number">$ArchiveLine2</SELECT><INPUT TYPE="SUBMIT" NAME="Submit" VALUE="Go!">
	...
	{
		$Legend = "<img src=\"$NonCGIURL/on.gif\"  border=0 alt=\"On\">&nbsp;&nbsp; 마지막 접속 이후 새로운 글 있음<BR><img src=\"$NonCGIURL/off.gif\" border=0 alt=\"off\">&nbsp;&nbsp; 마지막 접속 이후 새로운 글 없음";
		...
		$LastLoginTime = ("<FONT SIZE=2>최근 $FormatDate $FormatTime 에 접속하셨습니다</FONT>");
	}
	else
	{
		$LastLoginTime = "";
...
<B><FONT SIZE="2" FACE="$FontFace" COLOR="$TableStripTextColor">주제</FONT></B>
</td>);
			$TheColSpan = "5";
		}
		...
		{
			$TopRow = qq(<td NOWRAP valign=bottom align=center>
<b><FONT SIZE="2" FACE="$FontFace" COLOR="$TableStripTextColor">글 개수</FONT></B>
</td>);
			$TheColSpan = "5";
		}
		else
...
<FONT SIZE="2" FACE="$FontFace" COLOR="$TableStripTextColor">주제</FONT></B>
</td><td NOWRAP valign=bottom align=center><B>
<FONT SIZE="2" FACE="$FontFace" COLOR="$TableStripTextColor">글 개수</FONT></B>
</td>);
			$TheColSpan = "6";
		}
...
<FONT SIZE='2' FACE="$FontFace">$Moderator</FONT></td></tr>);
	...
	{  ## CATEGORIES ONLY VIEW
		...
		foreach $catline(@CATS)
		{
			...
			{
				print<<CatInfo;
<TR $CategoryColor><TD><FONT SIZE="$TextSize" FACE="$FontFace"><A HREF="$CGIURL/Ultimate.cgi?action=intro&category=$CatNumber&BypassCookie=true">$CatName</A></font></TD><TD align=center><FONT SIZE="$TextSize" FACE="$FontFace">$ForumCount</font></TD></tr>
CatInfo
...
#	$RegLinkHere = qq(<A HREF="$CGIURL/ubbmisc.cgi?action=editbio&Browser=$Browser&DaysPrune=$DaysPrune&LastLogin=$in{'LastLogin'}"><ACRONYM TITLE="사용자 정보를 변경할 수 있습니다.">정보변경</ACRONYM></A> | <A HREF="$CGIURL/Ultimate.cgi?action=agree"><ACRONYM TITLE="등록은 무료입니다!">등록</ACRONYM></A> |);
}

if ($AllowSearch eq "FALSE") {
$SearchOption = "";
}  else {
$SearchOption = qq(| <A HREF="$CGIURL/search.cgi?action=intro"><ACRONYM TITLE="키워드, 날짜, 이름으로 찾을 수 있습니다.">찾기</ACRONYM></A>);
}
...
<TD>
<CENTER>
<A HREF="$CGIURL/Ultimate.cgi"><IMG SRC="$NonCGIURL/$BBTitle"  BORDER=0></A>
</CENTER>
</td>
<td>
<CENTER>
<B><FONT SIZE="3" FACE="$FontFace" COLOR="$BBTitleFontColor">$BBName</FONT></B>
<br><FONT SIZE="2" FACE="$FontFace">
$RegLinkHere <A HREF="$CGIURL/ubbmisc.cgi?action=setprefs"><ACRONYM TITLE="환경설정">환경설정</ACRONYM></A> | <A HREF="$NonCGIURL/faq.html" target=_blank><ACRONYM TITLE="자주 질문하는 것들">FAQ</ACRONYM></A> $SearchOption 
$DigestWording
</FONT>
</CENTER>
</td></TR>
...
if ($category ne "") {
	$CategoryLine = qq(<b><A HREF="$CGIURL/Ultimate.cgi?action=intro&BypassCookie=true"><FONT SIZE=2>모든 포럼 카테고리 보기</FONT></A></B>);
	$CategoryName = &GetCatName($category);
	$ForumHeader = "<FONT SIZE=2>카테고리: $CategoryName</FONT>";
...
<tr>
<td><FONT SIZE="2" FACE="$FontFace"><A HREF="$CGIURL/search.cgi?action=getdaily">활성화 주제 보기(모든 포럼)</A></FONT>
</td>
</tr>
<tr>
...
<FONT SIZE="2" FACE="$FontFace" COLOR="$TableStripTextColor"><B>최근 글</B></FONT>
</td>
<td valign=bottom>
<FONT SIZE="2" FACE="$FontFace" COLOR="$TableStripTextColor"><B>관리자</B></FONT>
</td></tr>
TableTop1
}
...
<FONT SIZE="2" FACE="$FontFace" COLOR="$TableStripTextColor"><B>포럼 카테고리</B></FONT>
</td>
<td valign=bottom align=center>
<FONT SIZE="2" FACE="$FontFace" COLOR="$TableStripTextColor"><B>포럼개수</B></FONT>
</td>
</tr>
TableTop2
}
...
<td valign=top align=left>
<FONT SIZE="1" FACE="$FontFace">&nbsp; &nbsp; &nbsp; &nbsp; &nbsp;All times are $TimeZone.</font></td></tr>
<tr><td>
<FONT SIZE="1" FACE="$FontFace">$Legend</font> &nbsp;
</td></tr>
</table>
</center>
<center>
...
#&StandardHTML("지금은 등록을 받지 않고 있습니다.");
&StandardHTML("등록은 <A Href=\"http://www.archmage.co.kr/regi.html\">아크메이지 사이트</A>에서 하셔야 합니다.");
exit;
}
print <<Agreement;
<HTML>
...
<tr $TableColorStrip>
<TD align=center>
<FONT SIZE="$TextSize" COLOR="$TableStripTextColor" FACE="$FontFace"> <B>게시판 사용규칙과 회사원칙</B></font></td></tr>
<TR $AltColumnColor1><TD><FONT SIZE="$TextSize" FACE="$FontFace">
이 게시판의 사용은 완전히 무료입니다. 만약 아래의 규칙에 따르기를 동의하신다면 아래 "동의" 버튼을 누르십시오. 만약 동의하지 않으신다면 "동의하지 않음" 을 눌러주십시오.</font></td></tr>
...
sub Register {
if ($SuspendRegister eq "true") {
#&StandardHTML("죄송합니다. 현재 등록을 허용하고있지 않습니다.");
&StandardHTML("등록은 <A Href=\"http://www.archmage.co.kr/regi.html\">아크메이지 사이트</A>에서 하셔야 합니다.");
exit;
}

if (($UseEmail eq "ON") && ($EmailVerify eq "ON")) {
$EmailVerifyHTML = "<tr $AltColumnColor2><TD  colspan=2><FONT SIZE=\"$TextSize\" FACE=\"$FontFace\" COLOR=\"$LinkColor\">경고: 기입한 이메일 주소로 암호를 알려드립니다. 따라서 허위로(혹은 잘못된) 이메일 주소를 기입하게되면 글을 쓰거나 관련글을 올릴 수 없습니다.</FONT></td></tr>";

$PasswordWording1 = "아이디는 25자(한글은 12자)까지 가능하며 문자와 숫자로만 사용해 주시기 바랍니다.";
$PasswordWording2 = "";
...
<FONT SIZE="$TextSize" FACE="$FontFace">
게시판의 모든 기능을 사용하기 위해서 가입을 해야합니다.  
<BR><BR>
$PasswordWording1
 
<BR><BR>
알림: 본 페이지에서 요구하는 모든 정보는 게시판을 사용하는 다른 사용자들에게 공개됩니다. 따라서 공개가 꺼려지는 항목은 빈칸으로 남겨두시기 바랍니다. $PasswordWording2 * 표시가 있는 항목은 필히 기입을 해야합니다.</FONT>
</td></tr>
<tr $AltColumnColor1>
<FORM NAME="Register" METHOD=POST ACTION="$CGIURL/ubbmisc.cgi">

<TD><FONT SIZE="$TextSize" FACE="$FontFace"><B>아이디*</B></FONT></TD>
<TD><INPUT TYPE="TEXT" NAME="UserName" VALUE="" SIZE=25 MAXLENGTH=25>
 </TD>
</TR>
$EmailVerifyHTML
<TR $AltColumnColor1>
	<TD><FONT SIZE="$TextSize" FACE="$FontFace"><B>이메일*</B></FONT></TD>
	<TD><INPUT TYPE="TEXT" NAME="Email" VALUE="" SIZE=30 MAXLENGTH=150>
 </TD>
</TR>
<!--
<TR $AltColumnColor2>
	<TD><FONT SIZE="$TextSize" FACE="$FontFace"><B>도시</B></FONT></TD>
...
<TR $AltColumnColor2>
	...
	<TD><SELECT NAME="Location" SIZE=1>
 	...
 	<OPTION SELECTED VALUE="서울특별시">서울특별시
...
<!--
<TR $AltColumnColor1>
	<TD><FONT SIZE="$TextSize" FACE="$FontFace"><B>직업</B></FONT></TD>
...
<TR $AltColumnColor1>
	...
	<OPTION VALUE="EmployedSalaryHourlyFull">채용직원
	<OPTION VALUE="EmployedPartTime">채용직원(시간제)
	<OPTION VALUE="SelfEmployed">자영업
	<OPTION VALUE="Homemaker">주부
	<OPTION VALUE="College">대학생
	<OPTION VALUE="HighSchool">고등학생
	<OPTION VALUE="Student">초&amp;중학생
	<OPTION VALUE="Retired">휴&amp;퇴직자
	<OPTION VALUE="Unemployed">미취업자
	</SELECT>
	</TD>
</TR>

<TR $AltColumnColor2>
	<TD><FONT SIZE="$TextSize" FACE="$FontFace"><B>홈페이지</B></FONT></TD>
	<TD><INPUT TYPE="TEXT" NAME="URL" VALUE="http://" SIZE=30 MAXLENGTH=100>
 </TD></tr>
<TR $AltColumnColor1>
	<TD ><FONT SIZE="$TextSize" FACE="$FontFace"><B>관심분야</B></FONT></TD>
	<TD><INPUT TYPE="TEXT" NAME="Interests" VALUE="" SIZE=30 MAXLENGTH=200>
 </TD></tr>
 
<TR $AltColumnColor2>
	<TD><FONT SIZE="$TextSize" FACE="$FontFace"><B>ICQ 번호</B></FONT></TD>
	<TD><INPUT TYPE="TEXT" NAME="ICQnum" VALUE="" SIZE=7 MAXLENGTH=20>
 </TD></tr>
 

 <TR $AltColumnColor1>
	<TD valign=top><FONT SIZE="$TextSize" FACE="$FontFace"><B>서명</B></font><BR><FONT SIZE="1" FACE="$FontFace">서명은 글을 쓸때 맨 아래에 추가되는 것을 말합니다. 알림: 현재 서명 기능은 사용할 수 $SigStatus니다</U>. (게시판 관리자의 설정) <A HREF="$NonCGIURL/ubbcode.html">UBB Code</A> 는 사용할 수 있으나 이미지 태그는 허용하지 않습니다. (HTML 사용 불가)</B></FONT></TD>
	<TD><TEXTAREA NAME="Signature" ROWS=3 COLS=25 WRAP=PHYSICAL>
</TEXTAREA>
 </TD></tr>
 
 <TR $AltColumnColor2>
	<TD><FONT SIZE="$TextSize" FACE="$FontFace"><B>공지사항을 이메일로 받아보기 원합니까? (Allow Bulletin Board Administrators and Moderators To Send You Email Notices?)</B></FONT></TD>
	<TD><CENTER><FONT SIZE="$TextSize" FACE="$FontFace"><B><INPUT TYPE="RADIO" NAME="AllowMassMail" VALUE="yes" CHECKED> 예  &nbsp;&nbsp;&nbsp;<INPUT TYPE="RADIO" NAME="AllowMassMail" VALUE="no"> 아니오</B></FONT></CENTER>
 </TD></tr>
...
환경설정:</b> 쿠키를 지원하는 브라우저를 사용한다면 기본 설정을 저장할 수 있습니다. 쿠키를 지원하지 않거나 쿠키를 허용하지 않는다면 환경설정은 저장되지 않습니다.
</font><BR><BR></td></tr>

<tr $AltColumnColor1>
<TD valign=top><FONT SIZE="$TextSize" FACE="$FontFace"><B>아이디와 암호를 저장합니다. (주의: 게임방이나 학교 같이 공용으로 사용하는 컴퓨터에서는 사용하지 마시기바랍니다)</B></FONT></TD>
<TD valign=top><CENTER><FONT SIZE="$TextSize" FACE="$FontFace"><INPUT TYPE="RADIO" NAME="NameStorage" VALUE="yes" CHECKED>
 예 <INPUT TYPE="RADIO" NAME="NameStorage" VALUE="no"> 아니오</font>
</CENTER> </TD>
</TR>

<TR $AltColumnColor2>
	<TD valign=top><FONT SIZE="$TextSize" FACE="$FontFace"><B>기본 주제 보기 설정:</B></FONT></TD>
	<TD valign=top><CENTER><SELECT NAME="DaysPrune">
	<OPTION value="1" $Select1>최근 주제 보기
	<OPTION value="2" $Select2>지난 2일전 주제부터 보기
	<OPTION value="5" $Select5>지난 5일전 주제부터 보기
	<OPTION value="10" $Select10>지난 10일전 주제부터 보기
	<OPTION value="20" $Select20>지난 20일전 주제부터 보기
	<OPTION value="30" $Select30>지난 30일전 주제부터 보기
	<OPTION value="45" $Select45>지난 45일전 주제부터 보기
	<OPTION value="60" $Select60>지난 60일전 주제부터 보기
	<OPTION value="75" $Select75>지난 75일전 주제부터 보기
	<OPTION value="100" $Select100>지난 100일전 주제부터 보기
	<OPTION value="365" $Select365>지난 1년전 주제부터 보기
	</SELECT>
</CENTER>
	
 </TD></tr>
<tr $AltColumnColor1>
<TD valign=top><FONT SIZE="$TextSize" FACE="$FontFace"><B>글을 올릴때 이메일 주소를 보이겠습니까?</B></FONT></TD>
<TD valign=top><CENTER><FONT SIZE="$TextSize" FACE="$FontFace"><INPUT TYPE="RADIO" NAME="EmailView" VALUE="yes" CHECKED>
 예 <INPUT TYPE="RADIO" NAME="EmailView" VALUE="no"> 아니오</font>
</CENTER> </TD>
</TR>
</TABLE>
$BorderBottom
