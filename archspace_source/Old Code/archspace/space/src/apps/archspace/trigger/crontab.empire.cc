...
CCronTabEmpireRelation::handler()
{
	...
	if (EMPIRE->is_dead() == false)
	{
		...
		for (int i=0 ; i<PLAYER_TABLE->length() ; i++)
		{
			...
			Player->change_empire_relation(-2 + Player->get_relation_lost_per_day_by_rank());
					...
					if (Council->get_speaker_id() == Player->get_game_id())
					{
						...
						for (int i=0 ; i<CouncilRelationList->length() ; i++ )
						{
							...
							if (Relation->get_relation() != CRelation::RELATION_WAR &&
								Relation->get_relation() != CRelation::RELATION_TOTAL_WAR ) Count++;
						}
						Count = number(Count);
						for( int i = 0; i < CouncilRelationList->length(); i++ )
						{
				...
				else if (DemandRoll <= 60)		// make truce
				{
					if (Council->get_speaker_id() == Player->get_game_id())
					{
						...
						for( int i = 0; i < CouncilRelationList->length(); i++ ){
							...
							if( Relation->get_relation() == CRelation::RELATION_WAR || Relation->get_relation() == CRelation::RELATION_TOTAL_WAR ) Count++;
						}
						Count = number(Count);
						for( int i = 0; i < CouncilRelationList->length(); i++ ){
							CCouncilRelation *
