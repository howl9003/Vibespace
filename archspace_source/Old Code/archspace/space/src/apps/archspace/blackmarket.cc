...
CBlackMarket::add_new_item( CFleet* aFleet, CShipDesign* aDesign )
{
	...
	newBid->set_winner_id( 0 );	// no winner yet.
	newBid->set_price( aFleet->get_build_cost() * aFleet->get_current_ship() );
	time_t currentTime = time( 0 );
	newBid->set_open_time( currentTime );
...
CBlackMarket::create_new_fleet()
{
	int classWeekTable[16][10] = {
			...
			{ 22, 11, 13, 10, 12, 10,  8,  7, 7, 0 },
			{ 21, 10, 12, 10, 12, 10,  8,  7, 7, 3 },
			{ 20,  9, 12, 10, 12, 10,  9,  8, 7, 3 },
			{ 19,  8, 12, 10, 12, 10,  9,  9, 8, 3 },
			{ 18,  7, 12, 10, 12, 10, 10, 10, 8, 3 }
		};
	int classIndex = number( 100 );
	//int week = GAME->get_game_time() / (60*60*24*7);
	int week = GAME->get_game_time();
	week = week/60;
	week = week/60;
	week = week/24;
	week = week/7;
	if( week > 15 )	// 0<= week <= 15
	{
		week = 15;
	}
	for(int i=0 ; i<10 ; i++)
	{
		classIndex -= classWeekTable[week][i];	
		if( classIndex <= 0 )
		{
			classIndex = i+1;
			break;
	...
	for(int i=0 ; i<COMPONENT_TABLE->length() ; i++)
	{
		...
		if(component->get_level() <= levelIndex )
		{
			if( component->get_category()==CComponent::CC_WEAPON )
			{
				weaponList->add_component( component );
			}
			if( component->get_category()==CComponent::CC_DEVICE )
			{
	...
	for (int i=0 ; i<shipSize->get_weapon() ; i++)
	{
		...
		shipDesign->set_weapon_number(i, shipSize->get_slot()/weapon->get_space());
	}
	...
	int weekDice[10] = { 3, 6, 4, 2, 2, 2, 3, 2, 2, 0 };
	int weekRoll[10] = { 1, 2, 2, 2, 2, 2, 3, 3, 3, -1 };

	int ships = base[classIndex-1] 
				+ number( baseDice[classIndex-1] );
	for(int i=0 ; i<week/weekRoll[classIndex-1] ; i++)
	{
		ships += number( weekDice[classIndex-1] );
	}
	if(ships > 40)	//set max cap
	{
		ships = 40;
	}
	CFleet*
...
{
	int projectNumber = number( SECRET_PROJECT_TABLE->length() )-1;
	CProject*
		project = (CProject*)SECRET_PROJECT_TABLE->get( projectNumber );
	if( add_new_item( project ) == true )
	{
	}
	else
	{
...
CBlackMarket::expire(CBid *aBid)
{
	...
	if (aBid->get_winner_id() != 0)
	{
		...
		switch (aBid->get_type())
		{
			...
			{
				winner->buy_secret_project( aBid->get_item() );
				CProject*
					project = SECRET_PROJECT_TABLE->get_by_id( aBid->get_item() );
				CString
					news;
				news.clear();
				news.format(GETTEXT("You won the bid and achived %1$s."),
							project->get_name());
				winner->time_news( news );
...
CBlackMarketHTML::get_tech_list_html(CPlayer *aPlayer)
{
	...
	for( int i=0 ; i<bidList->length() ; i++ )
	{
		...
		if( bid->get_type() == CBid::ITEM_TECH )
		{
			...
			bool
				canBePurchased = true;
			if( tech->get_prerequisite1() != 0 )
			{
				if( knownTechList->get_by_id( tech->get_prerequisite1() ) == NULL )
				{
					canBePurchased = false;
				}
				prerequisite1 = TECH_TABLE->get_by_id( tech->get_prerequisite1() );
				if( tech->get_prerequisite2() != 0 )
				{
					if( knownTechList->get_by_id( tech->get_prerequisite2() ) == NULL )
					{
						canBePurchased = false;
					}
					prerequisite2 = TECH_TABLE->get_by_id( tech->get_prerequisite2() );
				}
				else
				{
					prerequisite2 = NULL;
				...
				if( knownTechList->get_by_id( tech->get_id() ) != NULL )
				{
					canBePurchased = false;
				}
			}
			...
			if( knownTechList->get_by_id( tech->get_id() ) != NULL )
			{
				html.format("<TD CLASS=\"tabletxt\" ALIGN=\"CENTER\" style=\"color:#0000FF\">%s(%d)</TD>", tech->get_name(), tech->get_level() );
			}
			else
			{
				html.format("<TD CLASS=\"tabletxt\" ALIGN=\"CENTER\">%s(%d)</TD>", tech->get_name(), tech->get_level() );
			}
			...
			if( prerequisite1 != NULL )
			{
				if( knownTechList->get_by_id( tech->get_prerequisite1() ) != NULL )
				{
					html.format( "%s", prerequisite1->get_name() );
				}
				else
				{
					html.format( "<FONT COLOR=\"#FF0000\">%s</FONT>", prerequisite1->get_name() );
				}
			}
			else
			{
			...
			if( prerequisite2 != NULL )
			{
				if( knownTechList->get_by_id( tech->get_prerequisite2() ) != NULL )
				{
					html.format( ", %s", prerequisite2->get_name() );
				}
				else
				{
					//html.format( ", %s", prerequisite2->get_name() );
					html.format( ", <FONT COLOR=\"#FF0000\">%s</FONT>", prerequisite2->get_name() );
				}
			}
			html.format("</TD>");
...
CBlackMarketHTML::get_project_list_html(CPlayer* aPlayer)
{
	...
	html.format( "<TH CLASS=\"tabletxt\" WIDTH=\"144\" BGCOLOR=\"#171717\">%s</TH>\n", GETTEXT("Project Name") );
	...
	for (int i=0 ; i<bidList->length() ; i++)
	{
		...
		html += "<TR>\n";
		if (PurchasedProjectList->get_by_id(bid->get_item()) == NULL &&
		...
		{
			html.format("<TD CLASS=\"maintext\" ALIGN=\"center\" style=\"color:#0000FF\">%s</TD>", project->get_name());
		}
		...
		html.format("<TD CLASS=\"maintext\" ALIGN=\"center\">%s %s</TD>",
					dec2unit(bid->get_expire_time()), GETTEXT("min(s)"));
		if (bid->get_winner_id() == aPlayer->get_game_id())
		{
			html.format("<TD CLASS=\"maintext\" ALIGN=\"center\" style=\"color:#0000FF\">%s</TD>", dec2unit(bid->get_price()));
		}
