...
CArchspacePageStation::registration()
{
	...
	insert_sorted(new CPagePersonalEnding());
...
pth_mutex_t CArchspaceStoreCenter::mPacketMutex = PTH_MUTEX_INIT;
pth_t CArchspaceStoreCenter::mSQLThread;
pth_t CArchspaceStoreCenter::mPacketThread;
...
void
CArchspaceStoreCenter::lock_packet()
{
	pth_mutex_acquire(&mPacketMutex, FALSE, NULL);
}

void
CArchspaceStoreCenter::unlock_packet()
{
	pth_mutex_release(&mPacketMutex);
	pth_nap((pth_time_t){0, 5});
}

void
CArchspaceStoreCenter::push()
{
	lock_packet();
	CStoreCenter::push();
	unlock_packet();
...
bool
CArchspaceStoreCenter::query(const char *aTable, const char *aQuery)
{
	bool Ret;
	lock_sql();
	Ret = CStoreCenter::query(aTable, aQuery);
	unlock_sql();
...
void*
CArchspaceStoreCenter::send(void *aArg)
{
	CArchspaceStoreCenter *StoreCenter = (CArchspaceStoreCenter*)aArg;

	time_t t = time(0);	//telecard 2001/05/15

	while(CArchspace::get_server_loop())
	{
		//SLOG("Write SQL Therad");
		t = time(0);
		lock_sql();
		lock_packet();
		StoreCenter->CStoreCenter::send();	
		unlock_packet();
		unlock_sql();
		//start telecard 2001/05/15
		if(time(0) - t > 3)
		{
			SLOG("%d second elapsed for CArchspaceStoreCenter::send() send", time(0)-t );
		}
		pth_nap((pth_time_t){time(0)-t+1, 0});
		//end telecard
	}
...
CArchspaceStoreCenter::write(void *aArg)
{
	...
	while(CArchspace::get_server_loop())
	{
//		SLOG("Write Packet Therad");
		lock_packet();
		Ret = StoreCenter->CStoreCenter::write();
		if (Ret == -1)
		{
			SLOG("Could not write packet to store center %d, %s",
					errno, strerror(errno));
			exit(-1);
		}
		unlock_packet();

		if (Ret == 1) pth_nap((pth_time_t){0, 500});
		else pth_nap((pth_time_t){0, 50});
	}
...
{
	mSQLThread = pth_spawn(PTH_ATTR_DEFAULT, CArchspaceStoreCenter::send, STORE_CENTER);
	if (mSQLThread == NULL) 
...
{
	mPacketThread = pth_spawn(PTH_ATTR_DEFAULT, CArchspaceStoreCenter::write, STORE_CENTER);
	if (mPacketThread == NULL) 
		SLOG("Could not spawn database write thread");

	SLOG("Spawn database write thread");
}
...
void
CArchspaceStoreCenter::kill_database_packet_thread()
{
	pth_cancel(mPacketThread);
}

bool
CArchspaceDatabase::initialize(int aArgc, char** aArgv)
{
	bool
		Res;
	...
	char
		*SystemLog =
			mConfiguration.get_string("Database",
					"SystemLogFile", "./systemlog");
	strcpy(SYSTEM_LOGFILE, SystemLog);

	SLOG("start to initalize database");
	...
	{
		SLOG("could not get information to open MySQL Server");
		SLOG("Host:%s, User:%s, Password:%s, Database:%s",
	...
	if (!DBConnection)
	{
		SLOG("Could not get DBConnection in database");
		return false;
	} 
	...
	while(Count++ < 30)
	{
		...
		if (Ret < 0) pth_nap((pth_time_t){1, 0});
		else break;
	}
	if (Ret < 0) return false;

	SLOG("Open a database connection %d %s", Ret, DBConnection);
