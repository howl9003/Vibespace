...
CDiplomaticMessage::record_html(bool aIsForAdmin)
{
	...
	HTML.format("<IMG SRC=\"http://%s/image/as_game/council/%s\">",
	...
	if (aIsForAdmin == false)
	{
		HTML.format("<A HREF=\"read_diplomatic_message_unanswered.as?MESSAGE_ID=%d\">",
					mID);
		HTML.format("<IMG SRC=\"http://%s/image/as_game/bu_read.gif\" BORDER=0>",
					(char *)CGame::mImageServerURL);
		HTML += "</A>\n";
	}
	else
	{
		HTML += "<FORM METHOD=post ACTION=\"player_message_read.as\">\n";
		HTML.format("<INPUT TYPE=image SRC=\"http://%s/image/as_game/bu_read.gif\">\n",
					(char *)CGame::mImageServerURL);
	...
	if (aIsForAdmin == true)
	{
		HTML.format("<A HREF=\"player_message_delete_confirm.as?MESSAGE_ID=%d\">",
					get_id());
		HTML.format("<IMG SRC=\"http://%s/image/as_game/bu_delete.gif\" BORDER=0></A>",
					(char *)CGame::mImageServerURL);
	}
	else
	{
		HTML += "<FORM METHOD=post ACTION=\"player_message_delete.as\">\n";
		HTML.format("<INPUT TYPE=image SRC=\"http://%s/image/as_game/bu_delete.gif\">\n",
					(char *)CGame::mImageServerURL);
		HTML.format("<INPUT TYPE=hidden NAME=PLAYER_GAME_ID VALUE=%d>\n", mReceiverID);
		HTML.format("<INPUT TYPE=hidden NAME=MESSAGE_ID VALUE=%d>\n", get_id());
		HTML += "</FORM>\n";
