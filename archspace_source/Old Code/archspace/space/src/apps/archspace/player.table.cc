...
CPlayerTable::load(CMySQL &aMySQL)
{
	...
	aMySQL.query( "SELECT tick FROM player ORDER BY tick DESC LIMIT 1" );
	aMySQL.use_result();

	if (aMySQL.fetch_row())
	{
		int
			MaxTick = atoi(aMySQL.row(0));
		aMySQL.free_result();

		int
			LastTick = 0;
		while (LastTick != MaxTick)
		{
			CString
				Query;
			...
			Query.format( "SELECT "
							...
							"news_research, "
							"news_population "
						"FROM player WHERE tick > %d ORDER BY tick", LastTick);
...
CPlayerTable::add_player(CPlayer* aPlayer)
{
	...
	unsigned int
		Sum = 0;
	for (int i=0 ; aPlayer->get_real_name()[i] ; i++) 
	{
		Sum += (unsigned int)(aPlayer->get_real_name()[i]);
	}
...
CPlayerTable::remove_player(int aGameID)
{
	...
	unsigned int 
		Sum = 0;
	for(int i=0; Player->get_real_name()[i]; i++) 
	{
		Sum += (unsigned int)(Player->get_real_name()[i]);
	}
	Index = MAX_HASHTABLE - (Sum % MAX_HASHTABLE) - 1;
	Pos = mNameIndex[Index].find_sorted_key(Player->get_real_name());
	if (Pos >= 0)
	{
