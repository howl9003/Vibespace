...
//	system_log("start page handler %s", get_name());
	...
	for (int i=0 ; i<FORTRESS_LIST->length() ; i++)
	{
		...
		if (Fortress->get_owner_id() == aPlayer->get_game_id())
		{
...
#undef SPACE_MINING_MODULE
	...
	LosingProductionPointByBuildingUpkeep += BuildingUpkeep.get(BUILDING_FACTORY) + BuildingUpkeep.get(BUILDING_MILITARY_BASE) + BuildingUpkeep.get(BUILDING_RESEARCH_LAB);
	...
	TempInt = (long long int)IncomeByPlanet.get(RESOURCE_MILITARY) + (long long int)MilitaryPointByFortress + (long long int)MilitaryPointByEffect;
	if (TempInt > (long long int)MAX_PLAYER_PP)
	{
		TotalMilitaryPointPerTurn = MAX_PLAYER_PP;
	}
	else
	{
	...
	ITEM("FACTORY_UPKEEP",
			(char *)format(GETTEXT("%1$s PP"),
							dec2unit(BuildingUpkeep.get(BUILDING_FACTORY))));
	ITEM("MILITARY_BASE_UPKEEP",
			(char *)format(GETTEXT("%1$s PP"),
							dec2unit(BuildingUpkeep.get(BUILDING_MILITARY_BASE))));
	ITEM("RESEARCH_LAB_UPKEEP",
			(char *)format(GETTEXT("%1$s PP"),
							dec2unit(BuildingUpkeep.get(BUILDING_RESEARCH_LAB))));
