...
//	system_log("start page handler %s", get_name());
	...
	CString
		ImageURL;
	ImageURL.clear();
	ImageURL.format("http://%s/image/as_game/race/", (char *)CGame::mImageServerURL);
	...
	Buf = ImageURL + Race + "/main_symbol.gif";
	...
	ITEM("HONOR", aPlayer->get_honor_description());
	...
	for (int i=0 ; i<PERSONAL_ENDING_TABLE->length() ; i++)
	{
		...
		if (ProjectList->get_by_id(EndingProject->get_id()) != NULL)
		{
			...
			EndingLink.format("<INPUT TYPE=image SRC=\"http://%s/image/as_game/personal_ending%d>",
	...
	if (UnreadDiplomaticMessages != 0)
	{
		...
		ITEM("UNREAD_DIPLOMATIC_MESSAGE_NUMBER", format(GETTEXT("You have %1$d unread diplomatic message(s)."),
					UnreadDiplomaticMessages));
	}
	else
	{
	...
	if(UnreadCouncilMessage != 0)
	{
		if (Speaker != NULL)
		{
			...
			{
				ITEM("UNREAD_COUNCIL_MESSAGE_NUMBER", format(GETTEXT("You have %1$d unread council message(s)."),
							UnreadCouncilMessage));
			}
			else
			{
