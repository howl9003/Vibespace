...
bool
CPageEmpireInvadeEmpireCapitalPlanetResult::handler(CPlayer *aPlayer)
{
//	system_log( "start page handler %s", get_name() );
	...
	CBattle
		Battle(CBattle::WAR_EMPIRE_CAPITAL_PLANET, aPlayer, EMPIRE, (void *)EMPIRE_CAPITAL_PLANET);

	CAdmiralList
		*AttackerAdmiralList = aPlayer->get_admiral_list(),
		*DefenderAdmiralList = EMPIRE_CAPITAL_PLANET->get_admiral_list();
	CFleetList
		*AttackerFleetList = aPlayer->get_fleet_list(),
		*DefenderFleetList = EMPIRE_CAPITAL_PLANET->get_fleet_list();
...
#undef SHIP_SIZE_CRUISER
	...
	Report.format(GETTEXT("Your fleet(s) arrived at %1$s."),
					EMPIRE_CAPITAL_PLANET->get_name());
	Report += "<BR>\n";
	...
	if (Battle.init_battle_fleet(&OffensePlan, AttackerFleetList, AttackerAdmiralList, &DefensePlan, DefenderFleetList, DefenderAdmiralList))
	{
		...
		if (Battle.attacker_win())
		{
			...
			EMPIRE_CAPITAL_PLANET->set_owner_id(aPlayer->get_game_id());
			...
			ADMIN_TOOL->add_invade_empire_capital_planet_log(
					(char *)format("The player %s has succeeded in invasion against the empire capital planet.",
									aPlayer->get_nick()));
		}
		else
		{
			...
			Buf.format(GETTEXT("%1$s invaded %2$s and lost."),
					aPlayer->get_nick(), EMPIRE_CAPITAL_PLANET->get_name());
			Buf += "<BR>\n";

			aPlayer->time_news((char *)Buf);

			ADMIN_TOOL->add_invade_empire_capital_planet_log(
					(char *)format("The player %s has failed in invasion against the empire capital planet.",
									aPlayer->get_nick()));
		}
...
//	system_log( "end page handler %s", get_name() );
	...
	return output("empire/invade_empire_capital_planet_result.html");
}
