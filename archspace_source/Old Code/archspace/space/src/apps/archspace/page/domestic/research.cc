...
//	system_log("start page handler %s", get_name());
	...
	int ResearchLab = aPlayer->get_research_lab()*10;
	...
	if (Target)
	{
		int Progress = (int)((aPlayer->get_research()*10) /
								aPlayer->get_research_cost(Target));

		CString ProgressBar;
		for(int i=0; i<10; i++)
		{
			ProgressBar.format("<IMG SRC=\"http://%s/image/as_game/domestic",
		...
		ITEM("PROGRESS_BAR", " ");
		ITEM("PROGRESS", GETTEXT("No one knows the progress in free research"));
	}
