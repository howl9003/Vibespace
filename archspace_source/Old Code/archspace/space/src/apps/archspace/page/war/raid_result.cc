		...
		if (Relation)
		{
			switch(Relation->get_relation())
			{
				...
				case CRelation::RELATION_NONE:
					aPlayer->change_honor(-2); break;
				case CRelation::RELATION_PEACE:
					aPlayer->change_honor(-4); break;
				case CRelation::RELATION_ALLY:
					aPlayer->change_honor(-8); break;
				default:
					break;
			}
		} else aPlayer->change_honor(-2);
	}
	else
	{
		CCouncilRelation *
			Relation = (CCouncilRelation *)TargetPlayer->get_council()->get_relation(aPlayer->get_council());
