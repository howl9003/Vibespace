...
#define MAGISTRATE_ATTACK_CHANCE		.80
...
#define EMPIRE_ATTACK_MAXIMUIM_RELATION		-100
#define EMPIRE_ATTACK_CHANCE			.70
#define EMPIRE_ATTACK_MINIMUIM_POWER		150001
#define EMPIRE_ATTACK_MAXIMUIM_POWER		MAX_POWER
#define EMPIRE_ATTACK_MEDIAN_FLEET_POWER	80000
	...
	if (EMPIRE->is_dead() == false)
	{
		...
		for (int i=0 ; i<MagistrateList->length() ; i++)
		{
			...
			while (index < PLAYER_TABLE->length())
			{
				...
				if (number(MAX_GENERAL_INT+ TargetPlayer->get_empire_relation() -MAGISTRATE_ATTACK_MAXIMUIM_RELATION) > MAX_GENERAL_INT*MAGISTRATE_ATTACK_CHANCE) continue;
...
CCronTabEmpireCounterattackEmpirePlanet::handler()
{
	...
	if (EMPIRE->is_dead() == false)
	{
		...
		while (index < PLAYER_TABLE->length())
		{
			...
			if (number(MAX_GENERAL_INT+ TargetPlayer->get_empire_relation() -EMPIRE_ATTACK_MAXIMUIM_RELATION) > MAX_GENERAL_INT*EMPIRE_ATTACK_CHANCE) continue;
