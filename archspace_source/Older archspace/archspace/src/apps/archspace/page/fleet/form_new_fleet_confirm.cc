...
//	system_log( "start page handler %s", get_name());
	...
	if (Fleet)
	{
		ITEM("ERROR_MESSAGE", GETTEXT("That fleet ID is already in use."));
		return output ("fleet/form_new_fleet_error.html");
	}

	QUERY("NEW_FLEET_NAME", NewFleetNameData);
	if (!NewFleetNameData)
	{
		ITEM("ERROR_MESSAGE",
	...
	CString NewFleetName = NewFleetNameData;
	NewFleetName.strip_all_slashes();

	if(!is_valid_name((char*)NewFleetName))
	{
		ITEM("ERROR_MESSAGE",
