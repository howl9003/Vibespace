...
bool
CPageBlackMarketLeasersOfficeBidResult::handler(CPlayer *aPlayer)
{
...
*/
	...
	if ((long long int)aPlayer->get_production() <
		(long long int)newPrice * (long long int)aPlayer->get_planet_list()->length())
	{
		ITEM("ERROR_MESSAGE",
				(char *)format(GETTEXT("We are sorry, you don't have (%1$s * %2$s) PP."),
							dec2unit(newPrice), dec2unit(aPlayer->get_planet_list()->length())));
		return output("black_market/leasers_office_error.html");
	}
	...
	NumberOfPlanetString = dec2unit(PlanetList->length());
	TotalPriceString = dec2unit(newPrice * PlanetList->length());
	...
	return output("black_market/leasers_office_bid_result.html");
}
