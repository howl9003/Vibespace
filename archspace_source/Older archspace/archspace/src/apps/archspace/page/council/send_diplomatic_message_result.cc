...
//	system_log("start page handler %s", get_name());
	...
	{
		if (Council->get_power()*100/PlayerCouncil->get_power() > COUNCIL_ATTACK_RESTRICTION_POWER_MAX)
		{
			ITEM("ERROR_MESSAGE",
		...
		if (Council->get_power()*100/PlayerCouncil->get_power() < COUNCIL_ATTACK_RESTRICTION_POWER_MIN)
		{
			ITEM("ERROR_MESSAGE",
		...
		for (int i=0; i < aRelationList->length(); i++)
		{
		    ...
		    if (aCouncilRelation->get_relation() == CRelation::RELATION_TOTAL_WAR)
		    {
                  ...
                  {
                     ITEM("ERROR_MESSAGE",
					(char *)format(GETTEXT("We can't declare total war against the council %1$s since we currently havea total war declared."),
									Council->get_nick()));
					return output("council/send_diplomatic_message_error.html");
