...
CPlayer::declare_war(CPlayer *aPlayer)
{
	...
	HonorLoss += (int)((aPlayer->get_honor()-get_honor())/6);
	...
	if (mAbility.has(ABILITY_PACIFIST)) HonorLoss += 10;
...
CPlayer::war_target_list_html()
{
	...
	for (int i=0 ; i<mRelationList.length() ; i++)
	{
		...
		if (Relation->get_relation() != CRelation::RELATION_WAR &&
			...
			Relation->get_relation() != CRelation::RELATION_BOUNTY) continue;
...
{	
	...
	map<int, CBounty*> *aBountyList = mBountyList.get_current_bounties();
	
	map<int, CBounty*>::iterator current;
	CPlayer *aPlayer;
	for (current = aBountyList->begin(); current != aBountyList->end(); current++)
	{
		aPlayer = current->second->get_target_player();
		if (!aPlayer)
		{
		...
		aResult.format("<tr><td><a href=\"../info/player_search_result.as?PLAYER_ID=%d\">%s (#%d)</a></td><td ALIGN=\"CENTER\">%s</td></tr>",aPlayer->get_game_id(),aPlayer->get_name(),aPlayer->get_game_id(),dec2unit(current->second->get_empire_points()));
	}
...
{
	std::vector<COfferedBounty*> *aBountyVector = OFFERED_BOUNTY_TABLE->get_available_bounties(this);
	...
	for (int i = aBountyVector->size() - 1; i >= 0; i--)
	{
		if (aBountyVector->at(i) == NULL) return "There was a funky error";
		aPlayer = aBountyVector->at(i)->get_player();
