...
CBattleFleet::init_common()
{
	...
	switch( mCommander->get_racial_ability() ){
		...
		case CAdmiral::RA_PATTERN_BROADCASTER :
			...
			add_static_effect( NewEffect );
			NewEffect = new CFleetEffect( CFleetEffect::FE_DR_AGAINST_MISSILE, 10+mCommander->get_level()*2, CFleetEffect::AT_PROPORTIONAL );
			add_static_effect( NewEffect );
			break;
		...
		case CAdmiral::RA_RETREAT_SHIELD :
			...
			if( mCommander->get_level() <= 5 ){
				...
				mAttribute += COMPLETE_CLOAKING;
				NewEffect = new CFleetEffect( CFleetEffect::FE_SHIELD_SOLIDITY, 4, CFleetEffect::AT_ABSOLUTE);
 				add_static_effect( NewEffect );
			} else if( mCommander->get_level() == 20 ){
				mAttribute += COMPLETE_CLOAKING;
...
CBattleFleet::damage_first_active( int aDam, bool aPSI )
{
	...
	float
		MoraleDrop = aDam*100.0/mTotalHP,
...
/*			if( is_psi_race() && aTurret->has_effect( CFleetEffect::WE_PSI_EMPOWER ) )
			...
			*aTotalDamage += Dam;
			...
			float
				MoraleDrop = RealDam*100.0/aEnemy->get_total_hp();
			
			// SLOG ("MORALE TEST: RealDam: %d , Enemy_hp: %d , MoraleDrop: %f", RealDam, aEnemy->get_total_hp(), MoraleDrop);
