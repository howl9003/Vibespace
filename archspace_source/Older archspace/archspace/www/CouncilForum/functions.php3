...
// AS auth server ¿¡ Á¢¼ÓÇØ¼­ AS_STRING °¡Á®¿È
...
Function as_string_parsing($string)
{
  ...
  $COUNCIL_NAME = urldecode($COUNCIL_NAME);
...
function get_whosonline($IP, $username, $forum, $db) {
	...
	mysql_free_result($resultlogtab);
...
//      $sql = "SELECT p.post_time, u.username FROM posts p, users u WHERE p.topic_id = '$id' AND p.poster_id = u.user_id ORDER BY post_time DESC LIMIT 1";
      $sql = "SELECT p.post_time, t.topic_poster_name FROM posts p, topics t WHERE t.topic_id = '$id' ORDER BY post_time DESC LIMIT 1";
      break;
    case 'user':
...
//     $val = sprintf("%s <br> %s %s", $myrow[post_time], $l_by, $myrow[username]);
     $val = sprintf("%s <br> %s %s", $myrow[post_time], $l_by, $myrow[topic_poster_name]);
   else
     $val = $myrow[post_time];
...
// get game name of article
function get_game_name_from_postid($post_id, $db)
{
...
global $l_jumpto, $l_selectforum, $l_go;
	...
	echo "</SELECT>\n<INPUT TYPE=\"IMAGE\" SRC=\"$GO_IMAGE\" BORDER=0>\n</FORM>";
}
