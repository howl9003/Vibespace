...
if ($IN['login'] != 'yes') {
	...
	else
	{
		...
		$DB->query("SELECT * FROM acf_admin_sessions WHERE ID='".$IN['adsess']."'");
		$row = $DB->fetch_row();
		...
		else
		{
			...
			$DB->query("SELECT * FROM acf_members WHERE id='".$row['MEMBER_ID']."'");
			$MEMBER = $DB->fetch_row();
			...
			else
			{
				...
				else
				{
					...
					$DB->query("SELECT * FROM acf_groups WHERE g_id='".$MEMBER['mgroup']."'");
...
else 
{
	...
	$DB->query("SELECT name, password, id, mgroup FROM acf_members WHERE LOWER(name)='".$username."'");
	$mem = $DB->fetch_row();
	...
	{
		$DB->query("SELECT * FROM acf_groups WHERE g_id='".$mem['mgroup']."'");
		...
		else
		{
			...
			$DB->query("INSERT INTO acf_admin_sessions (".$db_string['FIELD_NAMES'].") VALUES (".$db_string['FIELD_VALUES'].")");
...
if ($session_validated == 1 )
{
	...
	$DB->query("UPDATE acf_admin_sessions SET RUNNING_TIME='".time()."', LOCATION='".$IN['act']."' WHERE MEMBER_ID='".$MEMBER['id']."' AND ID='".$IN['AD_SESS']."'");
...
function do_login($message="") {
	...
	$DB->query("DELETE FROM acf_admin_sessions WHERE RUNNING_TIME < $cut_off_stamp");
	...
	if ( $mid > 0 )
	{
		$DB->query("SELECT m.id, m.name, m.mgroup, g.g_access_cp FROM acf_members m, acf_groups g WHERE m.id=$mid AND g.g_id=m.mgroup AND g.g_access_cp=1");
