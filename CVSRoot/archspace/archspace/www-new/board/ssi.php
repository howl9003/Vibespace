...
function do_syndication()
{
	...
	// ADDEDDDDDD!!!!!!!!!!!!!
	fatal_error("Currently disabled.");
	...
	$DB->query("SELECT g_id, g_perm_id FROM acf_groups WHERE g_id={$ibforums->vars['guest_group']}");
...
function do_news()
{
	...
	$DB->query("SELECT m.name as member_name, m.id as member_id,m.title as member_title, m.avatar, m.avatar_size, m.posts, t.*, p.*, g.g_dohtml, f.use_html
	            FROM acf_topics t
	            	LEFT JOIN acf_posts p ON (p.new_topic=1 AND p.topic_id=t.tid)
	            	LEFT JOIN acf_members m ON (m.id=t.starter_id)
...
function do_active()
{
	...
	$DB->query("SELECT s.member_id, s.member_name, s.login_type, g.suffix, g.prefix FROM acf_sessions s, acf_groups g WHERE running_time > '$time' AND g.g_id=s.member_group ORDER BY running_time DESC");
