...
class index_page {
	...
	function index_page()
	{
		...
		if ( $IN['save'] == 1 )
		{
			$DB->query("UPDATE acf_cache_store SET cs_value='".addslashes($_POST['notes'])."' WHERE cs_key='adminnotes'");
		}
		
		$text = "You can use this section to keep notes for yourself and other admins, etc.";
		
		$DB->query("SELECT * FROM acf_cache_store WHERE cs_key='adminnotes'");
		
		if ( ! $notes = $DB->fetch_row() )
		{
			$DB->query("INSERT INTO acf_cache_store (cs_key,cs_value) VALUES ('adminnotes', '$text')");
		...
		$DB->query("SELECT COUNT(*) as reg FROM acf_validating WHERE lost_pass <> 1");
		$reg = $DB->fetch_row();
		
		if ($reg['reg'] < 1 ) $reg['reg'] = 0;
		
		$DB->query("SELECT COUNT(*) as coppa FROM acf_validating WHERE coppa_user=1");
		$coppa = $DB->fetch_row();
		...
		$DB->query("SELECT MEMBER_NAME, LOCATION, LOG_IN_TIME, RUNNING_TIME, IP_ADDRESS FROM acf_admin_sessions WHERE RUNNING_TIME > $t_time");
		...
		if ($MEMBER['mgroup'] == $INFO['admin_group'])
		{
			...
			$DB->query("SELECT m.*, mem.id, mem.name FROM acf_admin_logs m, acf_members mem
		...
		if ( $INFO['spider_sense'] )
		{
			...
			$DB->query("SELECT * FROM acf_spider_logs ORDER BY entry_date DESC LIMIT 0,10");
