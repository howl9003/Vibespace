...
class ad_settings {
	...
	function import()
	{
		...
		$DB->query("SELECT MAX(set_id) as max FROM acf_macro_name");
		...
		foreach( $final_keys as $k => $v)
		{
			...
			$DB->query("INSERT INTO acf_macro ({$str['FIELD_NAMES']}) VALUES({$str['FIELD_VALUES']})");
		}
		...
		$DB->query("INSERT INTO acf_macro_name ({$str['FIELD_NAMES']}) VALUES({$str['FIELD_VALUES']})");
	...
	function edit_set_name()
	{
		...
		$DB->query("SELECT * FROM acf_macro WHERE macro_set='".$IN['id']."'");
		...
		$DB->query("UPDATE acf_macro_name SET set_name='{$IN['setname']}' WHERE set_id='".$IN['id']."'");
	...
	function macro_remove()
	{
		...
		$DB->query("SELECT * FROM acf_macro WHERE macro_id='".$IN['mid']."'");
		
		$row = $DB->fetch_row();
		
		$DB->query("DELETE FROM acf_macro WHERE macro_id='".$IN['mid']."'");
	...
	function macro_edit()
	{
		...
		$DB->query("UPDATE acf_macro SET macro_value='$key', macro_replace='$val' WHERE macro_id='".$IN['mid']."'");
	...
	function macro_add()
	{
		...
		$DB->query("SELECT * FROM acf_macro WHERE macro_set='".$IN['id']."' LIMIT 0,1");
	...
	function macro_form($type='edit')
	{
		...
		if ($type == 'edit')
		{
			...
			$DB->query("SELECT m.*, mn.set_name from acf_macro m, acf_macro_name mn WHERE macro_id='".$IN['mid']."' AND mn.set_id=m.macro_set");
		...
		else
		{
			...
			$DB->query("SELECT set_name from acf_macro_name WHERE set_id='".$IN['id']."'");
	...
	function export()
	{
		...
		$DB->query("SELECT macro_replace, macro_value FROM acf_macro WHERE macro_set='".$IN['id']."'");
	...
	function add_macro_set()
	{
		...
		$DB->query("SELECT * FROM acf_macro_name WHERE set_id='".$IN['id']."'");
		...
		$q1 = $DB->query("SELECT macro_value, macro_replace FROM acf_macro WHERE macro_set='{$IN['id']}'");
		...
		while( $row = $DB->fetch_row($q1) )
		{
			...
			$q2 = $DB->query("INSERT INTO acf_macro ({$str['FIELD_NAMES']}) VALUES({$str['FIELD_VALUES']})");
			//$q2 = $DB->query("INSERT INTO acf_macro SET macro_value='{$row['macro_value']}',macro_replace='{$row['macro_replace']}', macro_set='$new_id'");
		}
		
		$DB->query("INSERT INTO acf_macro_name SET set_id='$new_id', set_name='{$mac['set_name']}'");
	...
	function remove()
	{
		...
		$DB->query("DELETE FROM acf_macro WHERE macro_set='".$IN['id']."'");
		
		$DB->query("DELETE FROM acf_macro_name WHERE set_id='".$IN['id']."'");
	...
	function show_macros()
	{
		...
		$DB->query("SELECT m.*, ms.* from acf_macro m, acf_macro_name ms WHERE m.macro_set='".$IN['id']."' AND ms.set_id=m.macro_set LIMIT 0,1");
		...
		$DB->query("SELECT img_dir from acf_skins WHERE macro_id='".$IN['id']."'");
	...
	function list_current()
	{
		...
		$DB->query("SELECT DISTINCT(s.macro_id), i.macro_set, ms.set_name, s.sname from acf_macro i, acf_macro_name ms, acf_skins s WHERE s.macro_id=i.macro_set  AND ms.set_id=i.macro_set ORDER BY ms.set_name ASC");
		...
		if ( count($used_ids) > 0 )
		{
			...
			$DB->query("SELECT set_id, set_name FROM acf_macro_name WHERE set_id NOT IN(".implode(",",$used_ids).")");
