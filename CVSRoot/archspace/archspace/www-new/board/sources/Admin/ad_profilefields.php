...
class ad_fields {
	...
	function delete_form()
	{
		...
		$DB->query("SELECT ftitle, fid FROM acf_pfields_data WHERE fid='".$IN['id']."'");
	...
	function do_delete()
	{
		...
		$DB->query("ALTER TABLE acf_pfields_content DROP field_{$row['fid']}");
		
		$DB->query("DELETE FROM acf_pfields_data WHERE fid='".$IN['id']."'");
	...
	function main_save($type='edit')
	{
		...
		if ($type == 'edit')
		{
			...
			$DB->query("UPDATE acf_pfields_data SET $rstring WHERE fid='".$IN['id']."'");
		...
		else
		{
			...
			$DB->query("INSERT INTO acf_pfields_data (" .$rstring['FIELD_NAMES']. ") VALUES (". $rstring['FIELD_VALUES'] .")");
			
			$new_id = $DB->get_insert_id();
			
			$DB->query("ALTER TABLE acf_pfields_content ADD field_$new_id text default ''");
			
			$DB->query("OPTIMIZE TABLE acf_pfields_content");
	...
	function main_form($type='edit')
	{
		...
		if ($IN['id'] != "")
		{
			$DB->query("SELECT * FROM acf_pfields_data WHERE fid='".$IN['id']."'");
			$fields = $DB->fetch_row();
		}
		else
		{
