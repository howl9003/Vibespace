...
class ad_forums {
	...
	function skin_edit() {
		...
		$DB->query("SELECT id, name, skin_id FROM acf_forums WHERE id='".$IN['f']."'");
		...
		$DB->query("SELECT sid, sname FROM acf_skins");
	...
	function do_skin_edit() {
		...
		if ($IN['fsid'] == 'n')
		{
			$DB->query("UPDATE acf_forums SET skin_id='-1' WHERE id='".$IN['f']."'");
			$ADMIN->rebuild_config( array( 'forum_skin_'.$IN['f'] => "" ) );
		}
		else
		{
			$DB->query("UPDATE acf_forums SET skin_id='".$IN['fsid']."' WHERE id='".$IN['f']."'");
	...
	function subdeleteform() {
		...
		$DB->query("SELECT c.id, c.name, f.id as forum_id, f.subwrap, f.name as forum_name, f.parent_id, f.category FROM acf_categories c, acf_forums f WHERE c.id > 0 ORDER BY c.position, f.position");
	...
	function dosubdelete() {
		...
		$DB->query("UPDATE acf_forums SET category='$cat', parent_id='$parent' WHERE parent_id='".$IN['f']."'");
		
		$DB->query("DELETE FROM acf_forums WHERE id='".$IN['f']."'");
		
		// Delete any moderators, if any..
		
		$DB->query("DELETE FROM acf_moderators WHERE forum_id='".$IN['f']."'");
	...
	function show_rules() {
		...
		$DB->query("SELECT id, name, show_rules, rules_title, rules_text FROM acf_forums WHERE id='".$IN['f']."'");
	...
	function do_rules() {
		...
		$DB->query("UPDATE acf_forums SET $dbs WHERE id='".$IN['f']."'");
	...
	function recount($f_override="") {
		...
		$DB->query("SELECT name FROM acf_forums WHERE id='".$IN['f']."'");
		$forum = $DB->fetch_row();
		...
		$DB->query("SELECT COUNT(tid) as count FROM acf_topics WHERE approved=1 and forum_id='".$IN['f']."'");
		$topics = $DB->fetch_row();
		
		// Get the posts..
		
		$DB->query("SELECT COUNT(pid) as count FROM acf_posts WHERE queued <> 1 and forum_id='".$IN['f']."'");
		$posts = $DB->fetch_row();
		
		// Get the forum last poster..
		
		$DB->query("SELECT tid, title, last_poster_id, last_poster_name, last_post FROM acf_topics WHERE approved=1 and forum_id='".$IN['f']."' ORDER BY last_post DESC LIMIT 0,1");
		...
		$DB->query("UPDATE acf_forums SET $db_string WHERE id='".$IN['f']."'");
	...
	function empty_form() {
		...
		$DB->query("SELECT id, name FROM acf_forums WHERE id='".$IN['f']."'");
	...
	function do_empty() {
		...
		$DB->query("SELECT id, posts, topics FROM acf_forums WHERE id='".$IN['f']."'");
		...
		$DB->query("DELETE FROM acf_topics WHERE forum_id='".$IN['f']."'");
		
		// Move posts...
		
		$DB->query("DELETE FROM acf_posts WHERE forum_id='".$IN['f']."'");
		
		// Move polls...
		
		$DB->query("DELETE FROM acf_polls WHERE forum_id='".$IN['f']."'");
		
		// Move voters...
		
		$DB->query("DELETE FROM acf_voters WHERE forum_id='".$IN['f']."'");
		
		// Clean up the stats
		
		$DB->query("UPDATE acf_forums SET posts='0', topics='0', last_post='', last_poster_id='', last_poster_name='', last_title='', last_id='' WHERE id='".$IN['f']."'");
		
		$DB->query("UPDATE acf_stats SET TOTAL_TOPICS=TOTAL_TOPICS-".$forum['topics'].", TOTAL_REPLIES=TOTAL_REPLIES-".$forum['posts']);
	...
	function reorder_form() {
		...
		$DB->query("SELECT * from acf_categories WHERE id > 0 ORDER BY position ASC");
		while ($r = $DB->fetch_row())
		{
			$cats[$r['id']] = $r;
		}
		
		$DB->query("SELECT * from acf_forums ORDER BY position ASC");
		while ($r = $DB->fetch_row())
		{
	...
	function do_reorder() {
		...
		$cat_query = $DB->query("SELECT id from acf_forums");
		
		while ( $r = $DB->fetch_row($cat_query) )
		{
			$order_query = $DB->query("UPDATE acf_forums SET position='".$IN[ 'POS_' . $r['id'] ]."' WHERE id='".$r['id']."'");
		}
	...
	function delete_form() {
		...
		$DB->query("SELECT id, name FROM acf_forums ORDER BY position");
	...
	function do_delete() {
		...
		$DB->query("SELECT * FROM acf_forums WHERE id='".$IN['f']."'");
		$forum = $DB->fetch_row();
		...
		$DB->query("UPDATE acf_topics SET forum_id='".$IN['MOVE_ID']."' WHERE forum_id='".$IN['f']."'");
		
		// Move posts...
		
		$DB->query("UPDATE acf_posts SET forum_id='".$IN['MOVE_ID']."' WHERE forum_id='".$IN['f']."'");
		
		// Move polls...
		
		$DB->query("UPDATE acf_polls SET forum_id='".$IN['MOVE_ID']."' WHERE forum_id='".$IN['f']."'");
		
		// Move voters...
		
		$DB->query("UPDATE acf_voters SET forum_id='".$IN['MOVE_ID']."' WHERE forum_id='".$IN['f']."'");
		...
		{
			$DB->query("SELECT id FROM acf_forums WHERE parent_id='{$forum['parent_id']}'");
			...
			if ( ! $DB->get_num_rows() )
			{
				...
				$DB->query("UPDATE acf_forums SET subwrap=0 WHERE id='{$forum['parent_id']}'");
			}
		}
	...
	function new_form() {
		...
		$DB->query("SELECT c.id, c.name, f.id as forum_id, f.subwrap, f.name as forum_name, f.subwrap, f.parent_id, f.category FROM acf_categories c, acf_forums f WHERE c.id > 0 ORDER BY c.position, f.position");
		...
		else
		{
			...
			$DB->query("SELECT * from acf_categories WHERE id > 0");
	...
	function do_new() {
		...
		$DB->query("SELECT MAX(id) as top_forum FROM acf_forums");
		$row = $DB->fetch_row();
		...
		else
		{
			...
			$DB->query("SELECT category FROM acf_forums WHERE id='$parent'");
		...
		$DB->query("INSERT INTO acf_forums (".$db_string['FIELD_NAMES'].") VALUES (".$db_string['FIELD_VALUES'].")");
		
		if ($parent != -1)
		{
			$DB->query("UPDATE acf_forums SET subwrap=1 WHERE id='$parent'");
		}
	...
	function edit_form() {
		...
		$DB->query("SELECT c.id, c.name, f.subwrap, f.id as forum_id, f.name as forum_name, f.subwrap, f.parent_id, f.category FROM acf_categories c, acf_forums f WHERE c.id > 0 ORDER BY c.position, f.position");
	...
	function do_edit() {
		...
		{
			$DB->query("SELECT id FROM acf_forums WHERE parent_id='{$old_details['parent_id']}'");
			...
			if ( ! $DB->get_num_rows() )
			{
				...
				$DB->query("UPDATE acf_forums SET subwrap=0 WHERE id='{$old_details['parent_id']}'");
			}
		}
	...
	function subedit() {
		...
		$DB->query("SELECT * from acf_categories WHERE id > 0 ORDER BY position");
		...
		$DB->query("SELECT * from acf_forums WHERE subwrap='1' AND id='".$IN['f']."'");
	...
	function perm_edit_form() {
		...
		$DB->query("SELECT id,name FROM acf_categories ORDER BY position");
