...
class ad_skinfix {
	...
	function tool_compare()
	{
		...
		{
			$DB->query("SELECT updated, suid, group_name, func_name, section_content FROM acf_skin_templates WHERE group_name IN('".implode( "','", $look_up_group )."') AND
	...
	function run_tool()
	{
		...
		$DB->query("SELECT * FROM acf_skin_templates WHERE set_id=$skin");
		...
		$DB->query("SELECT * FROM acf_tmpl_names WHERE skid=$skin");
	...
	function list_current($msg="")
	{
		...
		$DB->query("SELECT t.skid, t.skname from acf_tmpl_names t ORDER BY t.skname ASC");
	...
	function _update_section($skin_file, $func_name, $func_data, $func_content)
	{
		...
		$DB->query("DELETE FROM acf_skin_templates WHERE set_id={$this->skin_id} AND group_name='$skin_file' AND func_name='$func_name'");
		...
		$DB->query("INSERT INTO acf_skin_templates ({$dbs['FIELD_NAMES']}) VALUES({$dbs['FIELD_VALUES']})");
	...
	function _update_php_file($group)
	{
		...
		$DB->query("SELECT * FROM acf_skin_templates WHERE set_id={$this->skin_id} AND group_name='$group'");
