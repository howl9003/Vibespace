...
class ad_plugins {
	...
	function chat_refresh_online()
	{
		...
		if ( $data = @file( $server_url ) )
		{
			...
			if ( count($data) > 0 )
			{
				$DB->query("SELECT m.id, m.name, g.g_id, g.prefix, g.suffix FROM acf_members m
			...
			$DB->query("UPDATE acf_cache_store SET cs_value='".addslashes($final)."', cs_extra='{$hits_left}&{$time}' WHERE cs_key='chatstat'");
		}
	...
	function chat_config()
	{
		...
		$DB->query("SELECT * FROM acf_cache_store WHERE cs_key='chatstat'");
		
		if ( ! $row = $DB->fetch_row() )
		{
			$DB->query("INSERT INTO acf_cache_store VALUES('chatstat', '', '0&0')");
		}
