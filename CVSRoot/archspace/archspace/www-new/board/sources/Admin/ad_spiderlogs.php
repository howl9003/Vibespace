...
class ad_spiderlogs {
	...
	function view()
	{
		...
		if ($IN['search_string'] == "")
		{
			$DB->query("SELECT COUNT(sid) as count FROM acf_spider_logs WHERE bot='$botty'");
			...
			$DB->query("SELECT * FROM acf_spider_logs WHERE bot='$botty' ORDER BY entry_date DESC LIMIT $start, 20");
		...
		else
		{
			...
			$DB->query("SELECT COUNT(sid) as count FROM acf_spider_logs WHERE query_string LIKE '%{$IN['search_string']}%'");
			...
			$DB->query("SELECT * FROM acf_spider_logs WHERE query_string LIKE '%{$IN['search_string']}%' ORDER BY entry_date DESC LIMIT $start, 20");
	...
	function remove()
	{
		...
		$DB->query("DELETE FROM acf_spider_logs WHERE bot='$botty'");
	...
	function list_current()
	{
		...
		$DB->query("SELECT count(*) as cnt, bot, entry_date, query_string FROM `acf_spider_logs` group by bot order by entry_date DESC");
