...
class ad_member_tools {
	...
	function learn_ip()
	{
		...
		$DB->query("SELECT id, name, email, posts, ip_address, joined FROM acf_members WHERE ip_address{$query} ORDER BY joined DESC LIMIT 250");
		...
		$DB->query("SELECT m.id, m.name, m.email, m.posts, m.joined, p.vote_date, p.ip_address, p.tid
					FROM acf_voters p
					 LEFT JOIN acf_members m ON ( p.member_id=m.id)
		...
		$DB->query("SELECT m.id, m.name, m.email, m.posts, m.joined, p.email_date, p.from_ip_address
					FROM acf_email_logs p
					 LEFT JOIN acf_members m ON ( p.from_member_id=m.id)
	...
	function show_ips()
	{
		...
		if ( $IN['member_id'] )
		{
			...
			$DB->query("SELECT id, name, email, ip_address FROM acf_members WHERE id=$id");
		...
		else
		{
			...
			$DB->query("SELECT id, name, email, ip_address FROM acf_members WHERE name='$name'");
		...
		$DB->query("SELECT count(distinct(ip_address)) as cnt FROM acf_posts WHERE author_id={$member['id']}");
		...
		$DB->query("SELECT count(ip_address) as ip, ip_address, pid, topic_id, post_date FROM acf_posts WHERE author_id={$member['id']} GROUP BY ip_address ORDER BY ip DESC LIMIT $st, $end");
		...
		if ( count($ips) > 0 )
		{
			$DB->query("SELECT id, name, ip_address FROM acf_members WHERE ip_address IN (".implode(",",$ips).") AND id != {$member['id']}");
	...
	function show_index($msg="", $membername="")
	{
		...
		{
			$DB->query("SELECT id, name FROM acf_members WHERE lower(name) LIKE '{$membername}%'");
