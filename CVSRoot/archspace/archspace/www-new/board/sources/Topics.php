...
class Topics {
    ...
    function Topics()
    {
        ...
        if ( ! $ibforums->topic_cache['tid'] )
        {
			$DB->query("SELECT t.*, f.topic_mm_id, f.name as forum_name, f.quick_reply, f.id as forum_id, f.read_perms, f.reply_perms, f.parent_id, f.use_html,
						...
						FROM acf_topics t, acf_forums f , acf_categories c where t.tid=".$ibforums->input['t']." and f.id = t.forum_id and f.category=c.id");
        ...
        if ( isset($ibforums->input['view']) )
        {
        	...
        	{
        		$DB->query("SELECT * from acf_topics WHERE forum_id=".$this->forum['id']." AND approved=1 AND state <> 'link' AND last_post > ".$this->topic['last_post']." "
			...
			else if ($ibforums->input['view'] == 'getnewpost')
			{
				...
				$DB->query("SELECT pid, post_date FROM acf_posts WHERE queued <> 1 AND topic_id='".$this->topic['tid']."' AND post_date > '".$last_time."' ORDER BY post_date LIMIT 1");
				...
				if ( $post = $DB->fetch_row() )
				{
					...
					$DB->query("SELECT COUNT(pid) as posts FROM acf_posts WHERE topic_id='".$this->topic['tid']."' AND pid <= '".$post['pid']."'");
			...
			else if ($ibforums->input['view'] == 'findpost')
			{
				...
				if ( $pid > 0 )
				{
					$DB->query("SELECT COUNT(pid) as posts FROM acf_posts WHERE topic_id='".$this->topic['tid']."' AND pid <= '".$pid."'");
        ...
        $DB->query("UPDATE acf_topics SET views=views+1 WHERE tid='".$this->topic['tid']."'");
        ...
        if ($this->forum['parent_id'] > 0)
        {
        	...
        	$DB->query("SELECT f.id as forum_id, f.name as forum_name, c.id, c.name FROM acf_forums f, acf_categories c WHERE f.id=".$this->forum['parent_id']." AND c.id=f.category");
        ...
        $DB->query("SELECT id, title, pips, posts from acf_titles ORDER BY posts DESC");
		...
		if ( $ibforums->vars['custom_profile_topic'] == 1 )
		{
			...
			$DB->query("SELECT fid, ftype, fhide, fcontent FROM acf_pfields_data");
			...
			$join_profile_query = "LEFT JOIN acf_pfields_content pc ON (pc.member_id=p.author_id)";
			$join_get_fields    = ", pc.*";
		}
		...
		$DB->query( "SELECT p.*,
				    ...
				    FROM acf_posts p
				      LEFT JOIN acf_members m ON (p.author_id=m.id)
		...
		if ( ! $DB->get_num_rows() )
		{
			if ($first >= $ibforums->vars['display_max_posts'])
			{
				...
				$newq = $DB->query("SELECT COUNT(pid) as pcount FROM acf_posts p, acf_members m WHERE p.topic_id=".$this->topic['tid']." and p.queued !=1 AND p.author_id=m.id");
				...
				if ($pcount['pcount'] > 1)
				{
					$DB->query("UPDATE acf_topics SET posts=".$pcount['pcount']." WHERE tid='".$this->topic['tid']."'");
				}
		...
		{	
			...
			$DB->query("SELECT s.member_id, s.member_name, s.login_type, s.location, g.suffix, g.prefix, g.g_perm_id, m.org_perm_id
					    FROM acf_sessions s
					     LEFT JOIN acf_groups g ON (g.g_id=s.member_group)
					     LEFT JOIN acf_members m on (s.member_id=m.id)
	...
	function multi_moderation()
	{
		...
		$DB->query("SELECT mm_id, mm_title FROM acf_topic_mmod WHERE mm_id IN(".implode( ",", explode( ",", $this->forum['topic_mm_id'] ) ).") ORDER BY mm_title");
	...
	function parse_poll()
	{
        ...
        $DB->query("SELECT member_id from acf_voters WHERE member_id=".$ibforums->member['id']." and tid=".$this->topic['tid']);
        $voter = $DB->fetch_row();
	...
	function return_last_post()
	{
		...
		$DB->query("SELECT pid FROM acf_posts WHERE queued <> 1 AND topic_id='".$this->topic['tid']."' ORDER BY pid DESC LIMIT 1");
		$post = $DB->fetch_row();
