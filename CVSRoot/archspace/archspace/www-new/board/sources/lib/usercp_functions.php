...
class usercp_functions {
	...
	function do_photo()
	{
		...
		if ( $ibforums->input['remove'] )
		{
			...
			$DB->query("SELECT id FROM acf_member_extra WHERE id={$ibforums->member['id']}");
		
			if ( $DB->get_num_rows() )
			{
				$DB->query("UPDATE acf_member_extra SET photo_location='', photo_type='', photo_dimensions='' WHERE id={$ibforums->member['id']}");
			}
			else
			{
				$DB->query("INSERT INTO acf_member_extra SET photo_location='', photo_type='', photo_dimensions='', id={$ibforums->member['id']}");
			}
		...
		if ( $DB->get_num_rows() )
		{
			$DB->query("UPDATE acf_member_extra SET photo_location='$final_location', photo_type='$final_type', photo_dimensions='$final_dimension' WHERE id={$ibforums->member['id']}");
		}
		else
		{
			$DB->query("INSERT INTO acf_member_extra SET photo_location='$final_location', photo_type='$final_type', photo_dimensions='$final_dimension', id={$ibforums->member['id']}");
		}
	...
	function do_skin_langs()
	{
		...
		if ($ibforums->vars['allow_skins'] == 1)
		{
			...
			$DB->query("SELECT sid FROM acf_skins WHERE hidden <> 1 AND sid='".$ibforums->input['u_skin']."'");
		...
		$DB->query("UPDATE acf_members SET $db_string WHERE id='".$this->class->member['id']."'");
	...
	function set_internal_avatar()
	{
		...
		$DB->query("UPDATE acf_members SET avatar='$final_string' WHERE id='".$ibforums->member['id']."'");
	...
	function do_avatar()
	{
		...
		if ( $ibforums->input['remove'] )
		{
			...
			$DB->query("UPDATE acf_members SET avatar='noavatar', avatar_size='' WHERE id='".$this->class->member['id']."'");
		...
		$DB->query("UPDATE acf_members SET avatar='$real_choice', avatar_size='$real_dims' WHERE id='".$this->class->member['id']."'");
	...
	function do_profile()
	{
		...
		$DB->query("UPDATE acf_members SET $set_string WHERE id='".$this->class->member['id']."'");
		...
		if ( count($custom_fields) > 0 )
		{
			...
			$DB->query("SELECT member_id FROM acf_pfields_content WHERE member_id='".$ibforums->member['id']."'");
			$test = $DB->fetch_row();
			...
			if ( $test['member_id'] )
			{
				...
				$DB->query("UPDATE acf_pfields_content SET $db_string WHERE member_id='".$ibforums->member['id']."'");
			}
			else
			{
	...
	function do_signature() {
		...
		$DB->query("UPDATE acf_members SET signature='".$ibforums->input['Post']."' WHERE id ='".$this->class->member['id']."'");
