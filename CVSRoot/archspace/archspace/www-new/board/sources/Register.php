...
class Register {
	...
	function revalidate_two()
	{
		...
		$DB->query("SELECT * FROM acf_members WHERE LOWER(name)='".strtolower($ibforums->input['username'])."'");
		...
		$DB->query("SELECT * FROM acf_validating WHERE member_id=".intval($member['id']));
	...
	function lost_password_start($errors="")
	{
		...
		if ($ibforums->vars['bot_antispam'])
		{
			...
			$DB->query("DELETE FROM acf_reg_antispam WHERE ctime < '$r_date'");
			...
			$DB->query("INSERT INTO acf_reg_antispam ({$str['FIELD_NAMES']}) VALUES ({$str['FIELD_VALUES']})");
		}
    ...
    function lost_password_end()
    {
    	...
    	if ($ibforums->vars['bot_antispam'])
		{
			...
			$DB->query("SELECT * FROM acf_reg_antispam WHERE regid='".trim(addslashes($ibforums->input['regid']))."'");
		...
		$DB->query("SELECT name, id, email, mgroup FROM acf_members WHERE LOWER(name)='$member_name'");
		...
		else
		{
			...
			$DB->query("INSERT INTO acf_validating ({$db_str['FIELD_NAMES']}) VALUES({$db_str['FIELD_VALUES']})");
    ...
    function show_reg_form($errors = "") {
		...
		$DB->query("SELECT * from acf_pfields_data WHERE fedit=1 AND fshowreg=1 ORDER BY forder");
	...
	function create_account()
	{
		...
		$DB->query("SELECT * from acf_pfields_data WHERE fedit=1");
		...
		$DB->query("SELECT id FROM acf_members WHERE LOWER(name)='".strtolower($in_username)."'");
		...
		{
			$DB->query("SELECT id FROM acf_members WHERE email='".$in_email."'");
		...
		if ($ibforums->vars['bot_antispam'])
		{
			...
			$DB->query("DELETE FROM acf_reg_antispam WHERE regid='".trim(addslashes($ibforums->input['regid']))."'");
		}
		...
		$DB->query("INSERT INTO acf_member_extra (id) VALUES ($member_id)");
		...
		$DB->query("DELETE FROM acf_pfields_content WHERE member_id=".$member['id']);
	...
	function validate_user() {
		...
		$DB->query("SELECT * FROM acf_members WHERE id=$in_user_id");
		...
		$DB->query("SELECT * FROM acf_validating WHERE member_id=$in_user_id");
		...
		else
		{
			...
			if ($in_type == 'reg')
			{
				...
				$DB->query("UPDATE acf_members SET mgroup=".intval($validate['real_group'])." WHERE id=".intval($member['id']));
				...
				$DB->query("DELETE FROM acf_validating WHERE vid='".$validate['vid']."' OR (member_id={$member['id']} AND new_reg=1)");
			...
			else if ($in_type == 'lostpass')
			{
				...
				$DB->query("UPDATE acf_members SET password='$new_pass' WHERE id=".intval($member['id']));
				...
				$DB->query("DELETE FROM acf_validating WHERE vid='".$validate['vid']."' OR (member_id={$member['id']} AND lost_pass=1)");
			...
			else if ($in_type == 'newemail')
			{
				...
				$DB->query("DELETE FROM acf_validating WHERE vid='".$validate['vid']."' OR (member_id={$member['id']} AND email_chg=1)");
	...
	function show_manual_form($type='reg') {
		...
		if ( $type == 'lostpass' )
		{
			...
			{ 
				...
				$DB->query("SELECT id, name, password, email FROM acf_members WHERE id=$in_user_id");
				...
				$DB->query("SELECT * FROM acf_validating WHERE member_id=$in_user_id and vid='$in_validate_key' and lost_pass=1");
	...
	function show_image()
	{
		...
		$DB->query("SELECT * FROM acf_reg_antispam WHERE regid='".trim(addslashes($ibforums->input['rc']))."'");
	...
	function bash_dead_validations()
	{
		...
		if ( intval($ibforums->vars['validate_day_prune']) > 0 )
		{
			...
			if ( count($mids) > 0 )
			{
				$DB->query("DELETE FROM acf_members WHERE id IN(".implode(",",$mids).")");
				$DB->query("DELETE FROM acf_member_extra WHERE id IN(".implode(",",$mids).")");
				$DB->query("DELETE FROM acf_pfields_content WHERE member_id IN(".implode(",",$mids).")");
				$DB->query("DELETE FROM acf_validating WHERE vid IN(".implode(",",$vids).")");
