...
class Moderate {
    ...
    function Moderate()
    {
        ...
        if ($ibforums->input['t'])
        {
        	...
        	{
        		$DB->query("SELECT tid, title, description, posts, state, starter_id, pinned, forum_id, last_post from acf_topics WHERE tid='".$ibforums->input['t']."'");
        ...
        $DB->query("SELECT f.*, c.name as cat_name, c.id as cat_id from acf_forums f, acf_categories c WHERE f.id=".$ibforums->input['f']." and c.id=f.category");
        ...
        if ($ibforums->member['id'])
        {
        	...
        	{
        		$DB->query("SELECT * FROM acf_moderators WHERE forum_id=".$this->forum['id']." AND (member_id=".$ibforums->member['id']." OR (is_group=1 AND group_id=".$ibforums->member['mgroup']."))");
	...
	function topic_history() {
		...
		$DB->query("SELECT * FROM acf_topics WHERE tid='$tid'");
		...
		$DB->query("SELECT * FROM acf_moderator_logs WHERE topic_id='$tid' ORDER BY ctime DESC");
	...
	function split_complete() {
		...
		//------------------------------------------
 		...
 		$DB->query("SELECT count(pid) as cnt FROM acf_posts WHERE topic_id={$this->topic['tid']}");
		...
		{
			$DB->query("SELECT id, subwrap, sub_can_post FROM acf_forums WHERE id='".$ibforums->input['fid']."'");
		...
		$DB->query("INSERT INTO acf_topics (" .$db_string['FIELD_NAMES']. ") VALUES (". $db_string['FIELD_VALUES'] .")");
		...
		$DB->query("UPDATE acf_posts SET forum_id='".$ibforums->input['fid']."', topic_id='$new_topic_id' WHERE pid IN($pid_string)");
		
		//----------------------------------------------------
		// NEW TOPIC: Get the last / first post in the "new" topic
		//----------------------------------------------------
		
		$DB->query("SELECT author_id, author_name, post_date FROM acf_posts WHERE topic_id='$new_topic_id' ORDER BY post_date DESC LIMIT 1");
		
		$last_post = $DB->fetch_row();
		
		$DB->query("SELECT pid, author_id, author_name, post_date FROM acf_posts WHERE topic_id='$new_topic_id' ORDER BY post_date ASC LIMIT 1");
		
		$first_post = $DB->fetch_row();
		
		// Get the number of posts in this "new" topic
		
		$DB->query("SELECT count(pid) as posts FROM acf_posts WHERE queued <> 1 AND topic_id='$new_topic_id'");
		...
		$DB->query("UPDATE acf_topics SET $db_string WHERE tid='$new_topic_id'");
		
		//----------------------------------------------------
		// NEW TOPIC: Reset the new_topic bit
		//----------------------------------------------------
		
		$DB->query("UPDATE acf_posts SET new_topic=0 WHERE topic_id='$new_topic_id'");
		
		$DB->query("UPDATE acf_posts SET new_topic=1 WHERE topic_id='$new_topic_id' AND pid='".$first_post['pid']."'");
		...
		$DB->query("SELECT author_id, author_name, post_date FROM acf_posts WHERE topic_id='".$this->topic['tid']."' ORDER BY post_date DESC LIMIT 1");
		
		$last_post = $DB->fetch_row();
		
		$DB->query("SELECT pid, author_id, author_name, post_date FROM acf_posts WHERE topic_id='".$this->topic['tid']."' ORDER BY post_date ASC LIMIT 1");
		
		$first_post = $DB->fetch_row();
		
		// Get the number of posts in this "new" topic
		
		$DB->query("SELECT count(pid) as posts FROM acf_posts WHERE queued <> 1 AND topic_id='".$this->topic['tid']."'");
		...
		$DB->query("UPDATE acf_topics SET $db_string WHERE tid='".$this->topic['tid']."'");
		
		//----------------------------------------------------
		// OLD TOPIC: Reset the new_topic bit
		//----------------------------------------------------
		
		$DB->query("UPDATE acf_posts SET new_topic=0 WHERE topic_id='".$this->topic['tid']."'");
		
		$DB->query("UPDATE acf_posts SET new_topic=1 WHERE topic_id='".$this->topic['tid']."' AND pid='".$first_post['pid']."'");
	...
	function merge_complete() {
		...
		$DB->query("SELECT tid, title, forum_id, last_post, last_poster_id, last_poster_name, posts, views FROM acf_topics WHERE tid='$old_id'");
		...
		else
		{
			...
			{
				$DB->query("SELECT mid FROM acf_moderators WHERE forum_id=".$old_topic['forum_id']." AND (member_id='".$ibforums->member['id']."' OR (is_group=1 AND group_id='".$ibforums->member['mgroup']."'))");
		...
		$DB->query("UPDATE acf_posts SET forum_id='".$this->topic['forum_id']."', topic_id='".$this->topic['tid']."' WHERE topic_id='".$old_topic['tid']."'");
		
		$DB->query("DELETE FROM acf_polls WHERE tid='".$old_topic['tid']."'");
		
		$DB->query("DELETE FROM acf_voters WHERE tid='".$old_topic['tid']."'");
		
		$DB->query("DELETE FROM acf_tracker WHERE topic_id='".$old_topic['tid']."'");
		
		$DB->query("DELETE FROM acf_topics WHERE tid='".$old_topic['tid']."'");
		...
		$DB->query("UPDATE acf_topics SET $str,views=views+{$old_topic['views']} WHERE tid='".$this->topic['tid']."'");
		...
		$DB->query("SELECT pid, author_name, author_id, post_date FROM acf_posts WHERE topic_id='".$this->topic['tid']."' ORDER BY post_date ASC LIMIT 1");
		
		if ( $first_post = $DB->fetch_row() )
		{
			$DB->query("UPDATE acf_posts SET new_topic=1 WHERE pid='".$first_post['pid']."'");
		}
	...
	function unsubscribe_all_form() {
		...
		$DB->query("SELECT COUNT(trid) as subbed FROM acf_tracker WHERE topic_id='".$this->topic['tid']."'");
	...
	function unsubscribe_all() {
		...
		$DB->query("DELETE FROM acf_tracker WHERE topic_id='".$this->topic['tid']."'");
	...
	function poll_delete_form() {
		...
		$DB->query("SELECT * FROM acf_polls WHERE tid='".$this->topic['tid']."'");
	...
	function poll_delete_do() {
		...
		$DB->query("DELETE FROM acf_polls WHERE tid='".$this->topic['tid']."'");
		
		// Remove from poll votes
		
		$DB->query("DELETE FROM acf_voters WHERE tid='".$this->topic['tid']."'");
		
		// Update topic
		
		$DB->query("UPDATE acf_topics SET poll_state='', last_vote='', total_votes='' WHERE tid='".$this->topic['tid']."'");
	...
	function poll_edit_do() {
		...
		$DB->query("UPDATE acf_topics SET poll_state='$poll_state' WHERE tid='".$this->topic['tid']."'");
	...
	function do_move() {
		...
		$DB->query("SELECT id, subwrap, sub_can_post, name, redirect_on FROM acf_forums WHERE id IN(".$ibforums->input['sf'].",".$ibforums->input['move_id'].")");
		...
		$DB->query("SELECT * FROM acf_topics WHERE tid='".$ibforums->input['tid']."'");
	...
	function delete_post() {
		...
		$DB->query("SELECT pid,attach_file, author_id, attach_id, post_date, new_topic from acf_posts WHERE forum_id='".$this->forum['id']."' AND topic_id='".$this->topic['tid']."' and pid='".$ibforums->input['p']."'");
		...
		$DB->query("DELETE from acf_posts WHERE topic_id='".$this->topic['tid']."' and pid='".$post['pid']."'");
		
		//---------------------------------------
		// Update the stats
		//---------------------------------------
		
		$DB->query("UPDATE acf_stats SET TOTAL_REPLIES=TOTAL_REPLIES-1");
		
		//---------------------------------------
		// Get the latest post details
		//---------------------------------------
		
		$DB->query("SELECT post_date, author_id, author_name from acf_posts WHERE topic_id='".$this->topic['tid']."' and queued <> 1 ORDER BY pid DESC");
		$last_post = $DB->fetch_row();
			
		$DB->query("UPDATE acf_topics SET last_post='"        .$last_post['post_date']   ."', ".
										 "last_poster_id='"   .$last_post['author_id']   ."', ".
										 "last_poster_name='" .$last_post['author_name'] ."', ".
		...
		if ($this->forum['last_id'] == $this->topic['tid'])
		{
			$DB->query("SELECT title, tid, last_post, last_poster_id, last_poster_name "
					  ."FROM acf_topics WHERE forum_id='".$this->forum['id']."' AND approved=1 "
			...
			$DB->query("UPDATE acf_forums SET ".$db_string.",posts=posts-1 WHERE id='".$this->forum['id']."'");
		}
	...
	function delete_topic()
	{
		...
		$DB->query("SELECT tid, forum_id FROM acf_topics WHERE state='link' AND moved_to='".$this->topic['tid'].'&'.$this->forum['id']."'");
		
		if ( $linked_topic = $DB->fetch_row() )
		{
			$DB->query("DELETE FROM acf_topics WHERE tid='".$linked_topic['tid']."'");
	...
	function do_edit()
	{
		...
		$DB->query("UPDATE acf_topics SET title='$topic_title', description='$topic_desc' WHERE tid='".$this->topic['tid']."'");
		
		if ($this->topic['tid'] == $this->forum['last_id'])
		{
			$DB->query("UPDATE acf_forums SET last_title='$topic_title' WHERE id='".$this->forum['id']."'");
		}
...
//+---------------------------------------------------------------------------------------------
	...
	function recount($fid="") {
		...
		$DB->query("SELECT COUNT(tid) as count FROM acf_topics WHERE approved=1 and forum_id='".$fid."'");
		$topics = $DB->fetch_row();
		
		// Get the posts..
		
		$DB->query("SELECT COUNT(pid) as count FROM acf_posts WHERE queued <> 1 and forum_id='".$fid."'");
		$posts = $DB->fetch_row();
		
		// Get the forum last poster..
		
		$DB->query("SELECT tid, title, last_poster_id, last_poster_name, last_post FROM acf_topics WHERE approved=1 and forum_id='".$fid."' ORDER BY last_post DESC LIMIT 0,1");
		$last_post = $DB->fetch_row();
		...
		$DB->query("UPDATE acf_forums SET $db_string WHERE id='".$fid."'");
