...
class Login {
    ...
    function Login()
    {
    	...
    	switch($ibforums->input['CODE']) {
    		...
    		case '03':
          // log out of portal!!!
          setcookie("ID_STRING", "", time() - 3600, "/", "68.11.248.40");
          setcookie("AS_STRING", "", time() - 3600, "/", "68.11.248.40");
          setcookie("ARCHSPACE_HOST", "", time() - 3600, "/", "68.11.248.40");
          setcookie("ARCHSPACE_PORT", "", time() - 3600, "/", "68.11.248.40");
          setcookie("NAME", "", time() - 3600, "/", "68.11.248.40");
 	...
 	function auto_login()
 	{
 		...
 		if ( ! $ibforums->member['id'] )
 		{
			...
			If ($mid and $pid)
			{
				$DB->query("SELECT * FROM acf_members WHERE id=$mid AND password='$pid'");
 	...
 	function markboard()
 	{
		...
		$DB->query("UPDATE acf_members SET last_visit='".time()."', last_activity='".time()."' WHERE id='".$ibforums->member['id']."'");
    ...
    function markforum()
    {
        ...
        $DB->query("SELECT id, name, subwrap, parent_id FROM acf_forums WHERE id=".$ibforums->input['f']);
        ...
        {	
        	...
        	$DB->query("SELECT id FROM acf_forums WHERE parent_id={$ibforums->input['f']}");
    ...
    function do_log_in() {
		...
		$DB->query("SELECT id, name, mgroup, password, ip_address FROM acf_members WHERE LOWER(name)='$username'");
		...
		if ( $DB->get_num_rows() )
		{
			...
			{
				$DB->query("UPDATE acf_members SET ip_address='{$ibforums->input['IP_ADDRESS']}' WHERE id={$member['id']}");
			}
			...
			if ($poss_session_id)
			{
				...
				$DB->query("DELETE FROM acf_sessions WHERE ip_address='".$ibforums->input['IP_ADDRESS']."' AND id <> '$session_id'");
				...
				$db_query = "UPDATE acf_sessions SET $db_string WHERE id='".$session_id."'";
			}
			else
			{
				...
				$DB->query("DELETE FROM acf_sessions WHERE ip_address='".$ibforums->input['IP_ADDRESS']."'");
				...
				$db_query = "INSERT INTO acf_sessions (" .$db_string['FIELD_NAMES']. ") VALUES (". $db_string['FIELD_VALUES'] .")";
			}
			...
			$DB->query("DELETE FROM acf_validating WHERE member_id={$ibforums->member['id']} AND lost_pass=1");
			...
			//-----------------------------------
			// ArchCave Portal Login and Error!
			//-----------------------------------
			include "libstd.php";
      include "libportal.php";
      $message = enter_portal($_POST["UserName"], $password,  "localhost 6000");

      if ($message != "ok")
      {
          if ($message == "")
          {
              $message = "Portal Login Error: Unknown";
          }
          //$message = $ibforums->lang[ $message ];
        	//$message = preg_replace( "/<#NAME#>/", "<b>{$ibforums->input[UserName]}</b>", $message );
      		$this->output .= $this->login_html->errors("ArchCave Portal Error - ".$message);//$message);
          $this->output .= $this->login_html->ShowForm( $ibforums->lang['please_log_in'], $HTTP_REFERER );
	...
	function do_log_out()
	{
		...
		$DB->query("UPDATE acf_sessions SET ".
				     "member_name='',".
				     "member_id='0',".
				     "login_type='0' ".
