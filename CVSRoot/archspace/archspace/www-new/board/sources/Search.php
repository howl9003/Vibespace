...
class Search {
	...
	//-----------------------------------------------------
 	...
 	function get_all_user() {
		...
		if ($ibforums->member['g_search_flood'] > 0)
		{
			...
			$DB->query("SELECT id FROM acf_search_results WHERE (member_id='".$ibforums->member['id']."' OR ip_address='".$ibforums->input['IP_ADDRESS']."') AND search_date > '$flood_time'");
		...
		$DB->query("SELECT pid FROM acf_posts WHERE queued <> 1 AND forum_id IN($forums) AND author_id=$mid");
 	...
 	function get_new_posts()
 	{
		...
		$DB->query("SELECT t.tid FROM acf_topics t
 	...
 	function get_last_ten()
 	{
		...
		$DB->query("SELECT p.*, t.*, f.id as forum_id, f.name as forum_name FROM acf_forums f, acf_posts p, acf_topics t WHERE p.queued <> 1 AND p.forum_id IN($forums) AND p.author_id='".$ibforums->member['id']."' AND t.tid=p.topic_id AND f.id=p.forum_id ORDER BY p.post_date DESC LIMIT 0,10");
 	...
 	function get_replies()
 	{
		...
		$DB->query("SELECT tid FROM acf_topics WHERE starter_id='".$ibforums->member['id']."'
	...
	function show_results()
	{
		...
		$DB->query("DELETE FROM acf_search_results WHERE search_date < '$t_time'");
		...
		$DB->query("SELECT * FROM acf_search_results WHERE id='{$this->unique_id}'");
		$sr = $DB->fetch_row();
		...
		if ($this->result_type == 'topics')
		{
			if ($this->search_in == 'titles')
			{
				...
				$DB->query("SELECT t.*, f.id as forum_id, f.name as forum_name
							FROM acf_topics t, acf_forums f
			...
			else
			{
				...
				if ($posts != ",")
				{
					$DB->query("SELECT topic_id FROM acf_posts WHERE pid IN(0{$posts}0)");
				...
				$DB->query("SELECT t.*, f.id as forum_id, f.name as forum_name
							FROM acf_topics t
							 LEFT JOIN acf_forums f ON (f.id=t.forum_id)
		...
		else //--------------------------------------------
		{
			...
			if ($this->search_in == 'titles')
			{
				...
				$DB->query("SELECT t.*, p.pid, p.author_id, p.author_name, p.post_date, p.post, f.id as forum_id, f.name as forum_name
				            FROM acf_topics t
				              LEFT JOIN acf_posts p ON (t.tid=p.topic_id AND p.new_topic=1)
				              LEFT JOIN acf_forums f ON (f.id=t.forum_id)
			...
			else
			{
				if ($topics != ",")
				{
					$DB->query("SELECT pid FROM acf_posts WHERE topic_id IN(0{$topics}0) AND new_topic=1");
				...
				$DB->query("SELECT t.*, p.pid, p.author_id, p.author_name, p.post_date, p.post, f.id as forum_id, f.name as forum_name, f.use_html, g.g_dohtml
							FROM acf_posts p
							  LEFT JOIN acf_topics t ON (t.tid=p.topic_id)
							  LEFT JOIN acf_forums f ON (f.id=p.forum_id)
							  LEFT JOIN acf_members m ON (m.id=p.author_id)
							  LEFT JOIN acf_groups g ON (m.mgroup=g.g_id)
	...
	function get_active() {
		...
		if (! isset($ibforums->input['searchid']) )
		{
			...
			$query = "SELECT DISTINCT(t.tid)
					  FROM acf_posts p
					    LEFT JOIN acf_topics t ON ( t.approved=1 and p.topic_id=t.tid)
		...
		else 
		{
			...
			$DB->query("SELECT * FROM acf_search_results WHERE id='$unique_id'");
			$sr = $DB->fetch_row();
    ...
    function get_searchable_forums()
    {
    	...
    	if ( is_array( $HTTP_POST_VARS['forums'] )  )
    	{
    		...
    		if ( in_array( 'all', $HTTP_POST_VARS['forums'] ) )
    		{
    			...
    			$sql_query = "SELECT c.state, f.id, f.read_perms, f.password FROM acf_forums f, acf_categories c WHERE c.id=f.category AND c.state <> 0";
    		...
    		else
    		{
				...
				if ( count( $cats ) )
				{
					$sql_query = "SELECT id, read_perms, password, subwrap from acf_forums WHERE category IN(".implode(",",$cats).")";
					$boolean   = "OR";
				}
				else
				{
					$sql_query = "SELECT id, read_perms, password, subwrap from acf_forums";
					$boolean   = "WHERE";
				}
		...
		else
		{
			...
			{
				$DB->query("SELECT c.state, f.id, f.read_perms, f.password FROM acf_forums f, acf_categories c WHERE c.id=f.category AND c.state <> 0");
			...
			else
			{
				if ( $ibforums->input['forums'] != "" )
				{
					...
					if ( preg_match( "/^c_/", $l ) )
					{
						...
						if ($c)
						{
							$DB->query("SELECT id, read_perms, password FROM acf_forums WHERE category=$c");
					...
					else
					{
						...
						if ($f)
						{
							...
							$DB->query("SELECT id, read_perms, password FROM acf_forums WHERE id=$f".$qe);
