...
extern module AP_MODULE_DECLARE_DATA as_module;
...
//	log(aRequest, "try to connect %s:%d", aServer, aPort);
	...
	if ((Address.s_addr = inet_addr(aServer)) == -1)
	{
		...
		//memcpy(&Address.s_addr, ((struct in_addr *)(HostEntry->h_addr))->s_addr, HostEntry->h_length);
		//Address = (struct in_addr *)(HostEntry->h_addr);
	}
	...
	while(*Data && (Pair=ap_getword(aRequest->pool, &Data, ';')))
	{
		...
		if (strlen(Value))
		{
			...
			Cookie = ap_psprintf(aRequest->pool,
				...
				"domain=192.168.0.2",
				Name, Value, apr_day_snames[Tms->tm_wday],
