...
CSpy::perform(CPlayer *aSpyPlayer, CPlayer *aTargetPlayer, bool *aSuccess)
{
	*aSuccess = false;
	...
	{
		Result = GETTEXT("Your PP is not enough to perform this special operation.");
		return (char *)Result;
	}
...
#undef STELLAR_BOMBARDMENT
	...
	{
			Result.format(GETTEXT("You can't perform %1$s because you love peace."),
									(char *)mName);
			return (char *)Result;
	}
	...
	*aSuccess = true;
	...
	Result = GETTEXT("Your spy was successful. The report is like below :");
	Result += "<BR><BR>\n";
...
CSpy::general_information_gathering(CPlayer *aSpyPlayer, CPlayer *aTargetPlayer)
{
	...
	if (InfoSet.has(8))
	{
		...
		if (AnyResult) Result += "<BR>\n";
		Result.format(GETTEXT("Commander(s) In The Pool : %1$s"),
	...
	aTargetPlayer->time_news(
			GETTEXT("Your some general information has been drained by someone!"));
...
CSpy::detailed_information_gathering(CPlayer *aSpyPlayer, CPlayer *aTargetPlayer)
{
	...
	if (DetailedInfo == 2)
	{
		...
		{
			Result.format(GETTEXT("%1$s doesn't own any project."),
	...
	aTargetPlayer->time_news(
			GETTEXT("Your some general information and a detailed information have been drained by someone!"));
...
CSpy::steal_secret_info(CPlayer *aSpyPlayer, CPlayer *aTargetPlayer)
{
	...
	switch (DetailedInfo)
	{
		case 1 :
		{
			...
			aTargetPlayer->time_news(
					GETTEXT("Your some detailed information has been drained by someone!"));
		...
		case 9 :
		{
			Result.format("%s :<BR>\n", GETTEXT("An used ship design which is selected randomly"));
...
CSpy::computer_virus_infiltration(CPlayer *aSpyPlayer, CPlayer *aTargetPlayer)
{
	...
	int
		ResearchDiff = aTargetPlayer->get_research() * number(10) / 100;
	if (ResearchDiff == 0)
	{
	...
	Result.format(GETTEXT("Your spy was successful, and %1$s lost some accumulated RP."),
					aTargetPlayer->get_nick());

	aTargetPlayer->time_news(
			GETTEXT("You lost some accumulated RP by someone's spy!"));
...
CSpy::devastating_network_worm(CPlayer *aSpyPlayer, CPlayer *aTargetPlayer)
{
	...
	else
	{
		...
		Result.format(GETTEXT("%1$s has lost some accumulated RP."),
						aTargetPlayer->get_nick());

		News += GETTEXT("some accumulated RP");
		Comma = true;
	}
	...
	else
	{
		...
		Result += "<BR>\n";
		Result.format(GETTEXT("%1$s has lost some accumulated invested PP on ship production."),
						aTargetPlayer->get_nick());

		if (Comma) News += ", ";
		News += GETTEXT("some invested PP on ship production");
		Comma = true;
	}
	...
	else
	{
		...
		if (Comma) News += ", ";
		News += GETTEXT("some invested PP on research");
		Comma = true;
	}
	...
	for (int i=0 ; i<PlanetList->length() ; i++)
	{
		...
		else
		{
			...
			Result += "<BR>\n";
			Result.format(GETTEXT("%1$s has lost some accumulated invested PP on %2$s."),
						aTargetPlayer->get_nick(), Planet->get_name());

			if (Comma) News += ", ";
			News.format(GETTEXT("some invested PP on %1$s"), Planet->get_name());
			Comma = true;
		}
	}
...
CSpy::sabotage(CPlayer *aSpyPlayer, CPlayer *aTargetPlayer)
{
	...
	else
	{
		...
		Result.format(GETTEXT("%1$s has lost some factories on %2$s."),
						aTargetPlayer->get_nick(), Planet->get_name());

		News.format(GETTEXT("some factories on %1$s"),
					Planet->get_name());
		Comma = true;
	}
	...
	else
	{
		...
		if (Comma) News += ", ";
		News.format(GETTEXT("some accumulated PP on %1$s"), Planet->get_name());
		Comma = true;
	}
...
CSpy::incite_riot(CPlayer *aSpyPlayer, CPlayer *aTargetPlayer)
{
	...
	else
	{
		...
		if (Comma) News += ", ";
		News.format(GETTEXT("some research labs. on %1$s"),
					Planet->get_name());
		Comma = true;
	}
...
CSpy::artificial_disease(CPlayer *aSpyPlayer, CPlayer *aTargetPlayer)
{
	...
	for (int i=0 ; i<PlanetList->length() ; i++)
	{
		...
		else
		{
			...
			if (i != 0) Result += "<BR>\n";
			Result.format(GETTEXT("%1$s has lost some people on planet %2$s."), aTargetPlayer->get_nick(), Planet->get_name());
...
CSpy::red_death(CPlayer *aSpyPlayer, CPlayer *aTargetPlayer)
{
	...
	for (int i=0 ; i<PlanetList->length() ; i++)
	{
		...
		{
			if (i != 0) Result += "<BR>\n";
			Result.format(GETTEXT("%1$s lost much people on planet %2$s."),
							aTargetPlayer->get_nick(), Planet->get_name());
			...
			{
				aTargetPlayer->time_news(
						(char *)format(GETTEXT("You lost much people on planet %1$s by someone's spy, and the planet has an obstinate microbe now."),
										Planet->get_name()));
			}
			else
			{
				aTargetPlayer->time_news(
						(char *)format(GETTEXT("You lost much people on planet %1$s by someone's spy."),
										Planet->get_name()));
			}
		}
	}
...
CSpy::strike_base(CPlayer *aSpyPlayer, CPlayer *aTargetPlayer)
{
	...
	else
	{
		...
		Result.format(GETTEXT("%1$s has lost some military bases on %2$s."),
						aTargetPlayer->get_nick(), Planet->get_name());

		News.format(GETTEXT("some military bases on %1$s"),
						Planet->get_name());
		Comma = true;
	}
	...
	if (number(2) == 1)
	{
		...
		if (Dock->length() != 0)
		{
			...
			Result += "<BR>\n";
			Result.format(GETTEXT("Some ships in %1$s's ship pool were destroyed."),
							aTargetPlayer->get_nick());

			if (Comma) News = ", ";
			News += GETTEXT("some ships in your ship pool");
		}
	}
...
CSpy::meteor_strike(CPlayer *aSpyPlayer, CPlayer *aTargetPlayer)
{
	...
	{
		Result.format(GETTEXT("%1$s has lost some people on planet %2$s."),
						aTargetPlayer->get_nick(), Planet->get_name());

		News += GETTEXT("some people");
		Comma = true;
	}
	else
	{
...
CSpy::emp_storm(CPlayer *aSpyPlayer, CPlayer *aTargetPlayer)
{
	...
	Result.format(GETTEXT("Now the target player %1$s's random planet is paralyzed."),
					aTargetPlayer->get_nick());

	aTargetPlayer->time_news(
			(char *)format(GETTEXT("Your planet %1$s is now paralyzed by someone's spy."),
							ApplyPlanet->get_name()));
