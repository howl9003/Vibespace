...
bool
CComponent::is_for_race(int aRace)
{
...
CComponentScript::get(CComponentTable *aComponentTable)
{
	...
	while( (component = get_array(i++)) )
	{
		...
		*/
		...
		/* race */
		something = get_section("Race", component);
		if (something)
		{
			TSomething
				Race;
			int
				i = 0;
			while ((Race = get_array(i++, something)))
			{
				int
					RaceID = atoi(get_data(Race));
				tmpComponent->set_race(RaceID);
			}
		}
		else
		{
			tmpComponent->set_race_all();
		}
