...
bool
CPageEmpireInvadeMagistrateConfirm::handler(CPlayer *aPlayer)
{
//	system_log( "start page handler %s", get_name() );
	...
	ITEM("CONFIRM_MESSAGE",
			GETTEXT("Please confirm that last order!!!<BR>\n"
					"You have begun to process an Attack against the Magistrate.<BR>\n"
					"If you do this, you will lose great favor/be forever a criminal!!!<BR>\n"
					"Do you really want to attack?"));

//	system_log( "end page handler %s", get_name() );

	return output("empire/invade_magistrate_confirm.html");
}
