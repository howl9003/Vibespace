		...
		else
		{
			...
			else
			{
				...
				else
				{
					...
					{
						Message.format(GETTEXT("You requested that the Empire intercede in your war with %1$s. They responded by forcing peace between you and %2$s to peace."),
										Foe->get_nick(),
										Foe->get_nick());
						CString
							News;
						...
						News.clear();
						News.format( GETTEXT("Cowardly %1$s of %2$s requested the Empire to mediate the disbute between your council and %3$s. Your council cannot but accept it. However, you know this is not the end. They'll soon pay the price of their act."), aPlayer->get_nick(), Council->get_nick(), Council->get_nick() );
						Foe->send_time_news_to_all_member( (char*)News );
						Council->improve_relationship( CCouncilMessage::TYPE_SUGGEST_TRUCE, Foe );
						Council->change_honor_with_news( -10 );
