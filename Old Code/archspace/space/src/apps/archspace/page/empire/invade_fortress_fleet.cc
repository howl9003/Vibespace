...
bool
CPageEmpireInvadeFortressFleet::handler(CPlayer *aPlayer)
{
//	system_log( "start page handler %s", get_name() );
	...
	ITEM("STRING_FORTRESS_STATUS", GETTEXT("Fortress Status"));

	CFortressList *
		FortressList = EMPIRE->get_fortress_list();
	ITEM("FORTRESS_STATUS", FortressList->get_fortress_status_html(aPlayer));
...
//	system_log( "end page handler %s", get_name() );
	...
	return output("empire/invade_fortress_fleet.html");
}
