...
bool
CPageEmpireInvadeEmpirePlanetResult::handler(CPlayer *aPlayer)
{
//	system_log( "start page handler %s", get_name() );
	...
	{
		ITEM("ERROR_MESSAGE",
				GETTEXT("Wrong fleet number received. Please ask Archspace Development Team."));
		return output("empire/invade_empire_error.html");
	}
	...
	CPlanet *
		TargetPlanet = (CPlanet *)EmpirePlanetList->get(EmpirePlanetList->length() - 1);

	CBattle
		Battle(CBattle::WAR_EMPIRE_PLANET, aPlayer, EMPIRE, (void *)TargetPlanet);
	...
	DefenderFleetList->create_as_empire_fleet_group_volatile(DefenderAdmiralList, CEmpire::EMPIRE_FLEET_GROUP_TYPE_EMPIRE_PLANET);

	CDefensePlan
		OffensePlan;
	OffensePlan.set_owner(aPlayer->get_game_id());

	QUERY("capFleet_id", CapitalFleetIDString);
	int
		CapitalFleetID = as_atoi(CapitalFleetIDString);
	if (CapitalFleetID < 1)
	{
		ITEM("ERROR_MESSAGE",
				GETTEXT("The capital fleet has wrong ID. Please ask Archspace Development Team."));
		return output("empire/invade_empire_error.html");
	}
...
#undef SHIP_SIZE_CRUISER
	...
	Report.format(GETTEXT("Your fleet(s) arrived at the Empire's planet(#%1$s)."), 
					dec2unit(TargetPlanet->get_id()));
	Report += "<BR>\n";
	...
	if (Battle.init_battle_fleet(&OffensePlan, AttackerFleetList, AttackerAdmiralList, &DefensePlan, DefenderFleetList, DefenderAdmiralList))
	{
		...
		if (Battle.attacker_win())
		{
			...
			EMPIRE->remove_empire_planet(TargetPlanet->get_id());
			...
			ADMIN_TOOL->add_invade_empire_planet_log(
					(char *)format("The player %s has succeeded in invasion against the empire planet(%d)",
									aPlayer->get_nick(),
									TargetPlanet->get_id()));
		}
		else
		{
			...
			ADMIN_TOOL->add_invade_empire_planet_log(
					(char *)format("The player %s has failed in invasion against the empire planet(%d)",
									aPlayer->get_nick(),
									TargetPlanet->get_id()));
		}
	...
	if (DefenderFleetList->is_volatile() == true)
	{
		DefenderAdmiralList->remove_all();
		DefenderFleetList->remove_all();
	}
...
//	system_log( "end page handler %s", get_name() );
	...
	return output("empire/invade_empire_planet_result.html");
}
