...
CPageEmpireShowAction::handler(CPlayer *aPlayer)
{
	...
	switch (Action->get_action())
	{
		...
		case CEmpireAction::ED_DEMAND_TRIBUTE :
			...
			{
				Message.format("<A HREF=\"answer_action.as?ACTION_ID=%d&ANSWER=yes\"><IMG SRC=http://%s/image/as_game/bu_accept.gif BORDER = 0></A>\n<A HREF=\"answer_action.as?ACTION_ID=%d&ANSWER=no\"><IMG SRC=http://%s/image/as_game/bu_decline.gif BORDER = 0></A>\n", Action->get_id(), (char *)CGame::mImageServerURL, Action->get_id(), (char *)CGame::mImageServerURL);
			}
