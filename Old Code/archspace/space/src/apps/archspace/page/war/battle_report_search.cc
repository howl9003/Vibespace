...
//	system_log( "start page handler %s", get_name());
	...
	for (int i=BATTLE_RECORD_TABLE->length()-1 ; i>=0 && count<50 ; i--)
	{
		...
		{
			buf.format("<FORM ACTION=\"battle_report2.as\" METHOD=POST><INPUT TYPE=HIDDEN NAME=LOG_ID VALUE=%d><INPUT TYPE=IMAGE SRC=\"http://%s/image/as_game/bu_read.gif\"></FORM>", record->get_id(), (char *)CGame::mImageServerURL);
		}
		else
		{
			buf += "No Battle";
	...
	buf.clear();
	buf.format("%d report is found", count);
