...
bool
CPageBlockadePlanet::handler(CPlayer *aPlayer)
{
//	system_log( "start page handler %s", get_name() );
	...
	CFleet *Fleet = (CFleet *)FleetList->get_by_id(CapitalID);
	...
	for (int i=2 ; i<=FleetNumber ; i++)
	{
		...
		CFleet *Fleet = (CFleet *)FleetList->get_by_id(FleetID);
		...
		int LocationX = as_atoi(LocationXString);
		int LocationY = as_atoi(LocationYString);
		...
		DeployInfo.format("<INPUT TYPE=hidden NAME=\"FLEET%d_O\" VALUE=\"%d\">\n",
							i, Command);
		DeployInfo.format("<INPUT TYPE=hidden NAME=\"FLEET%d_X\" VALUE=\"%d\">\n",
							i, LocationX);
		DeployInfo.format("<INPUT TYPE=hidden NAME=\"FLEET%d_Y\" VALUE=\"%d\">\n",
							i, LocationY);
		DeployInfo.format("<INPUT TYPE=hidden NAME=\"FLEET%d_ID\" VALUE=\"%d\">\n",
							i, Fleet->get_id());
	...
	{
		for (int i=0 ; i<SortieFleetList.length() ; i++)
		{
			CFleet *
	...
	CString
		PlanetImage;
	PlanetImage.clear();
	PlanetImage.format("http://%s/image/as_game/", (char *)CGame::mImageServerURL);
	...
	ITEM("BORDER_PLANET_SELECT", PlanetList->blockade_border_planet_select_html());
	...
	for (int i = PlanetList->length()/2 ; i<PlanetList->length() ; i++)
	{
		...
		JSPlanetInfo += "{\n";
		JSPlanetInfo.format("    document.blockadePlanet.home.value = \"%s\";\n",
							Planet->get_name());
		JSPlanetInfo.format("    document.blockadePlanet.population.value = \"%s\";\n",
							Planet->get_population_with_unit());
		JSPlanetInfo.format("    document.blockadePlanet.size.value = \"%s\";\n",
							Planet->get_size_description());
		JSPlanetInfo.format("    document.blockadePlanet.attribute.value = \"%s\";\n",
							Planet->get_attribute_description());
		JSPlanetInfo.format("    document.blockadePlanet.resource.value = \"%s\";\n",
							Planet->get_resource_description());
		JSPlanetInfo.format("    document.blockadePlanet.building.value = "
							"\"%s %d     %s %d     %s %d\";\n",
	...
	ITEM("JS_PLANET_INFO", JSPlanetInfo);

	ITEM("FLEET_NUMBER", FleetNumber);
	ITEM("DEPLOY_INFO", DeployInfo);

	ITEM("TARGET_PLAYER_ID", TargetPlayerID);

//	system_log( "end page handler %s", get_name() );

	return output("war/blockade_planet.html");
}
