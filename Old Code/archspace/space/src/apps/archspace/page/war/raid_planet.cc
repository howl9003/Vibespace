...
//	system_log( "start page handler %s", get_name() );
	...
	if (aPlayer->get_council() == TargetPlayerCouncil)
	{
		...
		if (Relation != NULL)
		{
			CHECK(Relation->get_relation() != CRelation::RELATION_WAR &&
				...
				(char *)format(GETTEXT("You must be at war to raid %1$s"),
					TargetPlayer->get_nick()));
		}
		else
		{
			ITEM("ERROR_MESSAGE", (char *)format(GETTEXT("You must be at war to raid %1$s"),
					TargetPlayer->get_nick()));
	...
	{
		ITEM("PLANET_IMAGE",
				(char *)format("http://%s/image/as_game/domestic/planet_home_icon.gif",
								(char *)CGame::mImageServerURL));
	}
	else
	{
		ITEM("PLANET_IMAGE",
				(char *)format("http://%s/image/as_game/domestic/planet_icon.gif",
								(char *)CGame::mImageServerURL));
	}
