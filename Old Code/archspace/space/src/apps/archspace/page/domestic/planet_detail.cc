...
//	system_log("start page handler %s", get_name());
	...
	ITEM("FACTORY_UPKEEP",
			(char *)format(GETTEXT("Factory %1$s PP"),
							dec2unit(Planet->get_upkeep_per_turn(BUILDING_FACTORY))));
	ITEM("RESEARCH_LAB_UPKEEP",
			(char *)format(GETTEXT("Research Lab. %1$s PP"),
							dec2unit(Planet->get_upkeep_per_turn(BUILDING_RESEARCH_LAB))));
	ITEM("MILITARY_BASE_UPKEEP",
			(char *)format(GETTEXT("Military Base %1$s PP"),
							dec2unit(Planet->get_upkeep_per_turn(BUILDING_MILITARY_BASE))));
