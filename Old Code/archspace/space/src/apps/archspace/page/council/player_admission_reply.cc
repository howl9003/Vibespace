...
//	system_log("start page handler %s", get_name());
	...
	if ( !reaction )
	{
		ITEM( "RESULT_MESSAGE", GETTEXT("You didn't decided your reply.") );
		return output("council/speaker_menu_error.html");
	}
	QUERY( "ADMISSION", admissionNo );
	if ( !admissionNo )
	{
		ITEM("RESULT_MESSAGE", GETTEXT("Could not find the admission request record."));
		return output("council/speaker_menu_error.html");
	}
	...
	if( strcmp(reaction, "YES") == 0)
	{
		...
		buf2.format("%s\n",
				GETTEXT("Your admission has been accpted."));
		if (message == NULL)
		{
			buf2 += " ";
		}
