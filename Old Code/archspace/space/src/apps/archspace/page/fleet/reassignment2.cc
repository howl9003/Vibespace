...
//	system_log( "start page handler %s", get_name() );
	...
	if(!Fleet)
	{
		ITEM("ERROR_MESSAGE", GETTEXT("You didn't select a fleet."));
		return output("fleet/reassignment_error.html");
	}
	if(Fleet->get_status() != CFleet::FLEET_STAND_BY)
	{
		ITEM("ERROR_MESSAGE", GETTEXT("You can reassign only stand-by fleets."));
		return output("fleet/reassignment_error.html");
	}
	...
	switch(ReassignmentID)
	{
			...
			*/
			...
			else if( Pool->length() == 1 )
			{
				...
				else
				{
					...
					ITEM("SUBMIT",
							(char *)format("<INPUT TYPE=image SRC=\"http://%s/image/as_game/bu_reassignment.gif\" BORDER=0></TD>", (char *)CGame::mImageServerURL));
				}
			}
			else
			{ 
		...
		{
			ITEM("ERROR_MESSAGE",
					GETTEXT("You selected an unknown reassignment."));
			return output("fleet/reassignment_error.html");
		}
	}
}
