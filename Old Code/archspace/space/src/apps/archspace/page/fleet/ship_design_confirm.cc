...
//	system_log( "start page handler %s", get_name() );
	...
	if (!is_valid_name((const char *)ClassName))
	{
	...
	ITEM("COMPUTER",
			format("%s(%s %d)<INPUT TYPE=hidden NAME=COMPUTER VALUE=\"%d\">",
					GETTEXT("Level"), Computer->get_name(), Computer->get_level(), Computer->get_id()));
	...
	ITEM("ARMOR",
			format("%s(%s %d)<INPUT TYPE=hidden NAME=ARMOR VALUE=\"%d\">",
					GETTEXT("Level"), Armor->get_name(), Armor->get_level(), Armor->get_id()));
	...
	ITEM("ENGINE",
			format("%s(%s %d)<INPUT TYPE=hidden NAME=ENGINE VALUE=\"%d\">",
					GETTEXT("Level"), Engine->get_name(), Engine->get_level(), Engine->get_id()));
	...
	ITEM("SHIELD",
			format("%s(%s %d)<INPUT TYPE=hidden NAME=SHIELD VALUE=\"%d\">",
					GETTEXT("Level"), Shield->get_name(), Shield->get_level(), Shield->get_id()));

	if (Armor->get_armor_type() == AT_BIO &&
		aPlayer->has_ability(ABILITY_GREAT_SPAWNING_POOL))
	{
		ITEM("BUILDING_COST", dec2unit(Body->get_cost() * 90 / 100));
	}
	else
	{
