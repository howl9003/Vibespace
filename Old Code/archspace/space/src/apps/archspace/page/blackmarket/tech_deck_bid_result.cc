...
bool
CPageBlackMarketTechDeckBidResult::handler(CPlayer *aPlayer)
{
	...
	{
		if( techList->get_by_id( tech->get_prerequisite1() ) == NULL )
		{
	...
	CString
		message;
	message.clear();
	message.format( "Thank you for your bid. You bid %d for %s(%d)", newPrice, tech->get_name(), tech->get_level() );
	ITEM( "MESSAGE", message );
	BLACK_MARKET->bid( bid->get_id(), aPlayer->get_game_id(), newPrice );

	//system_log("end page handler %s", get_name());

	return output("black_market/tech_deck_bid_result.html");
}
