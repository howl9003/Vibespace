...
CCouncilMessage::record_html(bool aIsForAdmin)
{
	...
	HTML.format("<IMG SRC=\"http://%s/image/as_game/council/%s\" WIDTH=45 HEIGHT=16>", 
	...
	if (aIsForAdmin == true)
	{
		...
		HTML += "<INPUT TYPE=image SRC=\"http://%s/image/as_game/bu_read.gif\">\n";
	...
	HTML.format("<IMG SRC=\"http://%s/image/as_game/bu_read.gif\" WIDTH=45 HEIGHT=11 BORDER=0>\n",
				(char *)CGame::mImageServerURL);
	HTML += "</A>\n";
	HTML += "</TD>\n";
...
CCouncilMessage::check_need_required(int aMessageType)
{
	switch (aMessageType)
	{
		...
		case TYPE_DECLARE_TOTAL_WAR: return true;
