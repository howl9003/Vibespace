...
CRankTable::top_ten_info_html(int aID)
{
	...
	if (mRankType >= CRank::PLAYER_OVERALL && mRankType <= CRank::PLAYER_TECH)
	{
		...
		{
			Info += "<TD NOWRAP CLASS=\"tabletxt\" ALIGN=\"CENTER\" BGCOLOR=\"#171717\">";
			Info.format("<FONT SIZE=\"2\">%s</FONT>", GETTEXT("# of Fleet(s)"));
			Info += "</TD>\n";
		} if (mRankType == CRank::PLAYER_PLANET)
		{
			Info += "<TD NOWRAP CLASS=\"tabletxt\" ALIGN=\"CENTER\" BGCOLOR=\"#171717\">";
			Info.format("<FONT SIZE=\"2\">%s</FONT>", GETTEXT("# of Planet(s)"));
			Info += "</TD>\n";
		} if (mRankType == CRank::PLAYER_TECH)
		{
			Info += "<TD NOWRAP CLASS=\"tabletxt\" ALIGN=\"CENTER\" BGCOLOR=\"#171717\">";
			Info.format("<FONT SIZE=\"2\">%s</FONT>", GETTEXT("# of Tech(s)"));
			Info += "</TD>\n";
		}
		...
		for (int i=0 ; i<10 && i < length() ; i++)
		{
			...
			if (Player->get_game_id() == aID)
			{
				...
				Info += "<TD CLASS=\"white\">";
				Info.format("&nbsp;<IMG SRC=\"%s\">%s",
							get_race_symbol_url(Player->get_race()), Player->get_nick());
				Info += "</TD>\n";

				CCouncil *
					Council = Player->get_council();
				Info.format("<TD NOWRAP CLASS=\"white\">&nbsp;%s</TD>\n",
							Council->get_nick());
			...
			} else
			{
				...
				CCouncil *
					Council = Player->get_council();
				Info.format("<TD NOWRAP CLASS=\"tabletxt\">&nbsp;%s</TD>\n",
							Council->get_nick());
				...
				mRankType <= CRank::COUNCIL_DIPLOMATIC)
	{
		...
		Info += "<TD NOWRAP CLASS=\"tabletxt\" ALIGN=\"CENTER\" BGCOLOR=\"#171717\">";
		Info.format("<FONT SIZE=\"2\">%s</FONT>", GETTEXT("# of Member(s)"));
		Info += "</TD>\n";
		...
		for (int i=0 ; i<10 && i < length(); i++)
		{
			...
			if (Council->get_id() == aID)
			{
				...
				if (Speaker != NULL)
				{
					Info += "<TD CLASS=\"white\">";
					Info.format("&nbsp;<IMG SRC=\"%s\">%s",
								get_race_symbol_url(Speaker->get_race()), Speaker->get_nick());
					Info += "</TD>\n";
				}
				else
				{
...
CRankTable::get_race_symbol_url(int aRace)
{
	static CString
		ImageURL;
	ImageURL.clear();
	ImageURL.format("http://%s/image/as_game/info/", (char *)CGame::mImageServerURL);
