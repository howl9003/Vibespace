...
bool
CEncyclopediaTechIndex::set(int aTechType)
{
	...
	static CString
		ImageURL;
	ImageURL.clear();
	ImageURL.format("http://%s/image/as_login/encyclopedia/",
...
bool
CEncyclopediaTechPage::set(CTech *aTech)
{
	mConversion.remove_all();
	...
	mConversion.set_value("STRING_PREREQUISITE_S_", GETTEXT("Prerequisite(s)"));
	...
	CTech *
		Tech1 = TECH_TABLE->get_by_id(aTech->get_prerequisite1());
	CTech *
		Tech2 = TECH_TABLE->get_by_id(aTech->get_prerequisite2());

	if (!Tech1)
	{
		mConversion.set_value("PREREQUISITE", GETTEXT("NONE"));
	} else
	{
		PrerequisiteName.format("<A HREF=\"%d.html\">%s</A>",
								Tech1->get_id(), Tech1->get_name());
		if (Tech2) PrerequisiteName.format(", <A HREF=\"%d.html\">%s</A>",
								Tech2->get_id(), Tech2->get_name());
		mConversion.set_value("PREREQUISITE", (char *)PrerequisiteName);
	}
...
bool
CEncyclopediaRaceIndex::set(CRaceTable *aRaceTable)
{
	CString
		Table;
...
bool
CEncyclopediaRacePage::set(CRace* aRace)
{
	mConversion.remove_all();
	...
	for (int i=ABILITY_BEGIN ; i<ABILITY_MAX ; i++)
	{
		if (AbilitySet.has(i))
		{
			if( AbilityName.length() )
				AbilityName += "<BR>\n";
			AbilityName += index_to_ability_name(i);
		}
	}
	if( aRace->get_morale_modifier() ){
		if( AbilityName.length() )
			AbilityName += "<BR>\n";
		if( aRace->get_morale_modifier() > 0 )
			AbilityName.format( "Morale Modifier %d", -aRace->get_morale_modifier() );
		else
			AbilityName.format( "Morale Modifier +%d", -aRace->get_morale_modifier() );
	}
	if( aRace->get_berserk_modifier() ){
		if( AbilityName.length() )
			AbilityName += "<BR>\n";
		if( aRace->get_berserk_modifier() > 0 )
			AbilityName.format( "Berserk Modifier +%d", aRace->get_berserk_modifier() );
		else
			AbilityName.format( "Berserk Modifier %d", aRace->get_berserk_modifier() );
	}
	if( aRace->get_survival_modifier() ){
		if( AbilityName.length() )
			AbilityName += "<BR>\n";
		if( aRace->get_survival_modifier() > 0 )
			AbilityName.format( "Commander Survival +%d", aRace->get_survival_modifier() );
		else
			AbilityName.format( "Commander Survival %d", aRace->get_survival_modifier() );
	}
...
bool
CEncyclopediaProjectIndex::set(int aProjectType)
{
	mConversion.remove_all();
...
bool
CEncyclopediaProjectPage::set(CProject *aProject)
{
	mConversion.remove_all();
	...
	mConversion.set_value("STRING_PREREQUISITE", GETTEXT("Prerequisite"));
	...
	CTech *
		Prerequisite = TECH_TABLE->get_by_id(aProject->get_prerequisite());

	if (Prerequisite)
	{
		PrerequisiteName.format("<A HREF=\"../tech/%d.html\">%s</A>",
								Prerequisite->get_id(), Prerequisite->get_name());
	} else
	{
		PrerequisiteName = GETTEXT("Unknown");
	}
...
bool
CEncyclopediaComponentIndex::set(int aComponentCategory)
{
	mConversion.remove_all();
...
bool
CEncyclopediaComponentPage::set(CComponent *aComponent)
{
	mConversion.remove_all();
	...
	switch (aComponent->get_category())
	{
		case CComponent::CC_ARMOR :
		{
			...
			CTech *
				Prerequisite = TECH_TABLE->get_by_id(Armor->get_prerequisite());
			if (Prerequisite)
			{
				static CString
					PrerequisiteName;
...
//				PrerequisiteName.format("<A HREF=\"../tech/%d.html\">%s</A>",
				PrerequisiteName.format("%s",
//										Prerequisite->get_id(), Prerequisite->get_name());
										Prerequisite->get_name());
				mConversion.set_value("PREREQUISITE", (char *)PrerequisiteName);
			} else
			{
				mConversion.set_value("PREREQUISITE", GETTEXT("NONE"));
			...
			mConversion.set_value("STRING_DEFENSE_RATE", GETTEXT("Defense Rate"));
		...
		case CComponent::CC_COMPUTER :
		{
			...
			CTech *
				Prerequisite = TECH_TABLE->get_by_id(Computer->get_prerequisite());
			if (Prerequisite)
			{
				static CString
					PrerequisiteName;
...
//										Prerequisite->get_id(), Prerequisite->get_name());
			...
			mConversion.set_value("STRING_ATTACK_RATE", GETTEXT("Attack Rate"));
		...
		case CComponent::CC_SHIELD :
		{
			...
			CTech *
				Prerequisite = TECH_TABLE->get_by_id(Shield->get_prerequisite());
			if (Prerequisite)
			{
				static CString
					PrerequisiteName;
...
//										Prerequisite->get_id(), Prerequisite->get_name());
		...
		case CComponent::CC_ENGINE :
		{
			...
			CTech *
				Prerequisite = TECH_TABLE->get_by_id(Engine->get_prerequisite());
			if (Prerequisite)
			{
				static CString
					PrerequisiteName;
...
//										Prerequisite->get_id(), Prerequisite->get_name());
		...
		case CComponent::CC_DEVICE :
		{
			...
			CTech *
				Prerequisite = TECH_TABLE->get_by_id(Device->get_prerequisite());
			if (Prerequisite)
			{
				static CString
					PrerequisiteName;
...
//										Prerequisite->get_id(), Prerequisite->get_name());
		...
		case CComponent::CC_WEAPON :
		{
			...
			CTech *
				Prerequisite = TECH_TABLE->get_by_id(Weapon->get_prerequisite());
			if (Prerequisite)
			{
				static CString
					PrerequisiteName;
...
bool
CEncyclopediaShipIndex::set(CShipSizeTable *aShipTable)
{
	CString
		Table;
...
bool
CEncyclopediaShipPage::set(CShipSize *aShip)
{
	mConversion.remove_all();
	...
	static CString
		ShipImageURL;
	ShipImageURL.clear();
	ShipImageURL.format("http://%s/image/as_login/encyclopedia/",
						(char *)CGame::mImageServerURL);
...
bool
CEncyclopediaSpyIndex::set(CSpyTable *aSpyTable)
{
	mConversion.remove_all();
...
bool
CEncyclopediaSpyPage::set(CSpy *aSpy)
{
	mConversion.remove_all();
	...
	CTech *
		Prerequisite = TECH_TABLE->get_by_id(aSpy->get_prerequisite());
	...
	{
		PrerequisiteName = GETTEXT("None");
	}

	mConversion.set_value("REQUIRED_TECH", (char *)PrerequisiteName);
...
CEncyclopediaTechIndexGame::set(int aTechType)
{
	...
	mName = format("game/tech_index%d.html", aTechType+1);
...
CEncyclopediaTechPageGame::set(CTech *aTech)
{
	...
	mName = format("game/tech/%d.html", aTech->get_id());
...
CEncyclopediaRacePageGame::set(CRace* aRace)
{
	...
	mName = format("game/race/%d.html", aRace->get_id());
...
CEncyclopediaProjectIndexGame::set(int aProjectType)
{
	...
	mName = format("game/project_index%d.html", aProjectType+1);
...
CEncyclopediaProjectPageGame::set(CProject *aProject)
{
	...
	mName = format("game/project/%d.html", aProject->get_id());
...
CEncyclopediaComponentIndexGame::set(int aComponentCategory)
{
	...
	mName = format("game/component_index%d.html", aComponentCategory+1);
...
//										Prerequisite->get_id(), Prerequisite->get_name());
	...
	mName = format("game/component/%d.html", aComponent->get_id());
...
CEncyclopediaShipPageGame::set(CShipSize *aShip)
{
	...
	mName = format("game/ship/%d.html", aShip->get_id());
...
CEncyclopediaSpyPageGame::set(CSpy *aSpy)
{
	...
	mName = format("game/special_ops/%d.html", aSpy->get_id());
