...
CPlanet::change_population(int aPopulation)
{
	...
	else
	{
		...
		else if (mPopulation < -aPopulation)
		{
			mPopulation = 0;
		}
		else
...
CPlanet::build_new_building(int aNogadaPoint)
{
	...
	if (mBuildingProgress.get(BUILDING_FACTORY) > BuildingCost)
	{
	...
	if (mBuildingProgress.get(BUILDING_MILITARY_BASE) > BuildingCost)
	{
	...
	if (mBuildingProgress.get(BUILDING_RESEARCH_LAB) > BuildingCost)
	{
...
void CPlanet::process_population_growth()
{
	...
	if (!get_population()) 
		...
		GrowthRatio = (mMaxPopulation - get_population()) * 5 
							/ get_population();
	if (GrowthRatio < -5) GrowthRatio = -5;
	if (GrowthRatio > 5) GrowthRatio = 5;

	// base growth = 50 + (growth control model * 10)
	// min 10
	// if current pop > max pop, base growth = 0;
	int
		BaseGrowth;
	if (get_population() >= get_max_population())
		BaseGrowth = 0;
	else {
		BaseGrowth = 50 + 
				mControlModel.get_growth()*10;
		if( BaseGrowth < 10 ) BaseGrowth = 10;
		if( BaseGrowth > 150 ) BaseGrowth = 150;
	}

	// pop growth = current pop * growth ratio + base growth
	// new pop = current pop + pop growth
	change_population(get_population() * GrowthRatio / 100 + BaseGrowth);
}
...
//		SLOG("remove PA_RADIATION");
		get_owner()->time_news(
				(char *)format(
						GETTEXT("You clear the radiation of %1$s's solar by solar manipulation technology."),
						get_name()));
...
CPlanet::build_control_model(CPlayer *aOwner)
{
	...
	{
		mControlModel.change_environment(-2),
		mControlModel.change_production(5);
	}
	...
	if (mAttribute.has(PA_OCEAN))
	{
		mControlModel.change_environment(1),
		mControlModel.change_growth(3),
		mControlModel.change_facility_cost(-5);
	}
...
CPlanet::html_management_record()
{
	...
	CString
		...
		ImageFile;
	ImageURL.clear();
	ImageURL.format("http://%s/image/as_game/",
	...
	if (mOrder == 0)
	{
		ImageFile.format("%s%s", (char *)ImageURL, "domestic/planet_home_icon.gif");
	} else
	{
		ImageFile.format("%s%s", (char *)ImageURL, "domestic/planet_icon.gif");
	}
	...
	Info.format("<TD CLASS=\"tabletxt\" WIDTH=\"114\" ALIGN=\"RIGHT\">%s&nbsp;:</TD>\n",
				GETTEXT("Population"));
	Info.format("<TD CLASS=\"tabletxt\" WIDTH=\"125\">&nbsp;%s</TD>\n",
	...
	Info.format("<TD CLASS=\"tabletxt\" WIDTH=\"114\" ALIGN=\"RIGHT\">%s&nbsp;:</TD>\n",
				GETTEXT("Production"));
	Info.format("<TD CLASS=\"tabletxt\" WIDTH=\"125\">&nbsp;%s PP</TD>\n",
				dec2unit(get_production_per_turn()));
...
CPlanet::calc_waste_rate()
{
	...
	int
		Efficiency = mControlModel.get_efficiency();
	...
	if( Waste > 90 ) Waste = 90;	// max waste 90
