...
{
	SLOG("YOSHIKI : CCouncil::add_member()-1, mMembers.length() = %d", mMembers.length());
	if (!aPlayer) return false;
	SLOG("YOSHIKI : CCouncil::add_member()-2, mMembers.length() = %d", mMembers.length());
	if (mMembers.get_by_game_id(aPlayer->get_game_id())) return false;

	SLOG("YOSHIKI : CCouncil::add_member()-3, mMembers.length() = %d", mMembers.length());
	mMembers.add_player(aPlayer);
	SLOG("YOSHIKI : CCouncil::add_member()-4, mMembers.length() = %d", mMembers.length());

	rearrange_cluster();
	SLOG("YOSHIKI : CCouncil::add_member()-5, mMembers.length() = %d", mMembers.length());
...
CCouncil::select_project_html()
{
	...
	if (Speaker != NULL)
	{
		...
		for (int i=0 ; i<mProjectList.length() ; i++)
		{
			...
			if (Speaker->know_tech(Project->get_prerequisite()) == false) continue;
...
{
	CProject *
		Project = mProjectList.get_by_id(aProjectID);
	...
	CPlayer *
		Speaker = PLAYER_TABLE->get_by_game_id(mSpeakerID);
	if (Speaker->know_tech(Prerequisite) == false)
	{
		return GETTEXT("You don't have required tech for this council project.");
	}
...
{
	if (mProjectList.length() == 0) return -1;
	...
	for (int i=0 ; i<mProjectList.length() ; i++)
	{
		...
		int
			Prerequisite = Project->get_prerequisite();
		if (Speaker->know_tech(Prerequisite) == true) Know = true;
	}
...
CCouncil::get_speaker()
{
	...
	if (Speaker->get_game_id() == EMPIRE_GAME_ID)
	{
		return NULL;
	}
	else
...
CCouncil::set_improve_relation_effect_timer(CCouncil *aCouncil)
{
	...
	CCouncilActionImproveRelation *Action = 
								new CCouncilActionImproveRelation();
...
CCouncil::declare_war(CCouncil* aCouncil)
{
	...
	if (get_honor() < aCouncil->get_honor())
		change_honor_with_news(
				(int)((aCouncil->get_honor()-get_honor())/6));
...
CCouncil::declare_total_war(CCouncil* aCouncil)
{
	...
	{
		if (Action->get_start_time() <= PERIOD_24HOURS*CGame::mSecondPerTurn)
			change_honor_with_news(-60);	
		else if (Action->get_start_time() <= PERIOD_48HOURS*CGame::mSecondPerTurn)
			change_honor_with_news(-30);
		else // over 48 hours
			change_honor_with_news(-15);
...
CCouncil::info_members_information_html()
{
	...
	for (int i=0 ; i<mMembers.length() ; i++)
	{
		...
		Info.format("<TD CLASS=\"tabletxt\" WIDTH=\"151\">%s</TD>\n",
					Member->get_nick());
...
CCouncil::council_war_status_information_html()
{
	...
	Info += "<TH CLASS=\"tabletxt\" WIDTH=\"43\" COLOR=\"#666666\">";
	Info += GETTEXT("DA");
	Info += "</TH>\n";
...
//		Info.format("<TD CLASS=\"tabletxt\" WIDTH=\"43\" ALIGN=\"CENTER\">%s</TD>\n",
//					dec2unit(2));
