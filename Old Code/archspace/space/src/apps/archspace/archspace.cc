...
CArchspace::prepare()
{
	...
	if (DBCon > 0)
	{
		...
		mMaxDescription = (DBCon > MaxFD) ? (MaxFD = DBCon) : MaxFD;
	}
...
CArchspace::handle_exception()
{
	...
	{
		SLOG("ERROR : On exception in database connection");
		kill(getpid(), SIGTERM);
	}

	if (FD_ISSET(mKeyServerInfo.pipe[0], &mExceptions))
	{
		SLOG("ERROR : On exception in key thread pipe");
		kill(getpid(), SIGTERM);
	}
}
...
CArchspace::handle_output()
{
	...
	{
		mStoreCenter->lock_packet();
		signal_masking();
		if (mStoreCenter->CStoreCenter::write() < 0)
		{
			SLOG("ERROR : Could not write packet to store center %d, %s", errno, strerror(errno));
			mServerLoop = false;
		}
		signal_unmasking();
		mStoreCenter->unlock_packet();
	}
}
...
CArchspace::connect_database()
{
	...
	if (!DBConnection)
	{
		SLOG("ERROR : Could not get DBConnection");
		return false;
	}
	...
	SLOG("Database connection %s", DBConnection);
...
//	CArchspaceStoreCenter::kill_database_packet_thread();
	...
	for(int i=0; i<COUNCIL_TABLE->length(); i++)
	{
		...
		STORE_CENTER->CStoreCenter::send();
	...
	STORE_CENTER->CStoreCenter::push();

	STORE_CENTER->CStoreCenter::send();

	STORE_CENTER->CStoreCenter::push();

	while(!STORE_CENTER->packet_empty())
	{
		Ret = STORE_CENTER->CStoreCenter::write();
		if (Ret == -1) break;
	}
