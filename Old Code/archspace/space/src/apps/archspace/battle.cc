...
void
CVector::set_vector( int aX, int aY, double aDirection )
{
	mX = aX;
	mY = aY;
	mDirection = 0;
...
void
CVector::turn_to( CVector *aVector, double aMaxTurn )
{
	double
		DeltaDirection = (double)delta_direction( aVector );
...
bool
CVector::is_heading_to( CVector *aVector )
{
	int
		Delta = delta_direction( aVector );
	if( Delta < 2 || Delta > 358 ) return true;
	return false;
}

int
CVector::delta_direction( CVector *aVector )
{
	int
		dX = aVector->get_x() - get_x(),
		dY = aVector->get_y() - get_y();
	...
	int
		Delta = Direction - (int)mDirection;
...
bool
CVector::is_in_range( CVector *aVector, int aDistance, int aDirection )
{
...
void
CVector::move( int aDeltaX, int aDeltaY )
{
	mX += aDeltaX;
	mY += aDeltaY;
...
void
CVector::move( int aLength )
{
	double
		dX = aLength*as_cos( (int)mDirection ),
		dY = aLength*as_sin( (int)mDirection );

	move( (int)dX, (int)dY );
}

int
CVector::distance( CVector *aVector )
{
	int
		dX = get_x() - aVector->get_x(),
		dY = get_y() - aVector->get_y();

	return (int)hypot((double)dX, (double)dY);
}

bool
CVector::touch_border()
{
...
void
CVector::rotate_about( int aDirection, CVector *aFixedPoint )
{
...
CTurret::recharge( CBattleFleet *aFleet )
{
	...
	Cooling = Cooling*(100+aFleet->get_morale()/-5+10+aFleet->get_exp()/-5+10)/100;
...
CBattleFleet::init_common()
{
	...
	switch( mCommander->get_special_ability() ){
		case CAdmiral::SA_ENGINEERING_SPECIALIST :
			if( mCommander->get_level() <= 5 ){
			...
			} else if( mCommander->get_level() <= 8 ){
				NewEffect = new CFleetEffect( CFleetEffect::FE_ARMOR_DR, 5, CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_REPAIR, 1, CFleetEffect::AT_PROPORTIONAL );
				NewEffect->set_period( 10 );
				add_static_effect( NewEffect );
			} else if( mCommander->get_level() <= 12 ){
				NewEffect = new CFleetEffect( CFleetEffect::FE_ARMOR_DR, 8, CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_REPAIR, 1, CFleetEffect::AT_PROPORTIONAL );
				NewEffect->set_period( 10 );
				add_static_effect( NewEffect );
				...
				NewEffect->set_period( 10 );
				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_REPAIR_SPEED, mCommander->get_level(), CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
			} else if( mCommander->get_level() == 16 ){
				NewEffect = new CFleetEffect( CFleetEffect::FE_ARMOR_DR, 11, CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_REPAIR, 1, CFleetEffect::AT_PROPORTIONAL );
				NewEffect->set_period( 10 );
				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_REPAIR_SPEED, mCommander->get_level(), CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_IMPENETRABLE_ARMOR, mCommander->get_level(), CFleetEffect::AT_ABSOLUTE );
				add_static_effect( NewEffect );
			} else if( mCommander->get_level() == 17 ){
				NewEffect = new CFleetEffect( CFleetEffect::FE_ARMOR_DR, 12, CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_REPAIR, 1, CFleetEffect::AT_PROPORTIONAL );
				NewEffect->set_period( 10 );
				add_static_effect( NewEffect );
				...
				NewEffect = new CFleetEffect( CFleetEffect::FE_IMPENETRABLE_ARMOR, mCommander->get_level(), CFleetEffect::AT_ABSOLUTE );
				add_static_effect( NewEffect );
			} else if( mCommander->get_level() == 18 ){
				NewEffect = new CFleetEffect( CFleetEffect::FE_ARMOR_DR, 13, CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_REPAIR, 2, CFleetEffect::AT_PROPORTIONAL );
				NewEffect->set_period( 10 );
				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_REPAIR_SPEED, mCommander->get_level(), CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
				...
				NewEffect = new CFleetEffect( CFleetEffect::FE_IMPENETRABLE_ARMOR, mCommander->get_level(), CFleetEffect::AT_ABSOLUTE );
				add_static_effect( NewEffect );
			} else if( mCommander->get_level() == 20 ){
				NewEffect = new CFleetEffect( CFleetEffect::FE_ARMOR_DR, 20, CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_REPAIR, 3, CFleetEffect::AT_PROPORTIONAL );
				NewEffect->set_period( 10 );
				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_REPAIR_SPEED, mCommander->get_level()*2, CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_IMPENETRABLE_ARMOR, mCommander->get_level()*2, CFleetEffect::AT_ABSOLUTE );
				add_static_effect( NewEffect );
			}
			break;
		case CAdmiral::SA_SHIELD_SYSTEM_SPECIALIST :
			if( mCommander->get_level() <= 5 ){
				NewEffect = new CFleetEffect( CFleetEffect::FE_SHIELD_SOLIDITY, 1, CFleetEffect::AT_ABSOLUTE );
				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_SHIELD_STRENGTH, 5, CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
			} else if( mCommander->get_level() <= 10 ){
				NewEffect = new CFleetEffect( CFleetEffect::FE_SHIELD_SOLIDITY, 1, CFleetEffect::AT_ABSOLUTE );
				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_SHIELD_STRENGTH, mCommander->get_level(), CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
			} else if( mCommander->get_level() <= 15 ){
				NewEffect = new CFleetEffect( CFleetEffect::FE_SHIELD_SOLIDITY, 1, CFleetEffect::AT_ABSOLUTE );
				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_SHIELD_STRENGTH, mCommander->get_level(), CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_SHIELD_RECHARGE_RATE, mCommander->get_level(), CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
			} else if( mCommander->get_level() <= 19 ){
				NewEffect = new CFleetEffect( CFleetEffect::FE_SHIELD_SOLIDITY, 2, CFleetEffect::AT_ABSOLUTE );
				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_SHIELD_STRENGTH, mCommander->get_level()*3/2, CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_SHIELD_RECHARGE_RATE, mCommander->get_level(), CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_IMPENETRABLE_SHIELD, mCommander->get_level(), CFleetEffect::AT_ABSOLUTE );
				add_static_effect( NewEffect );
			} else if( mCommander->get_level() == 20 ){
				NewEffect = new CFleetEffect( CFleetEffect::FE_SHIELD_SOLIDITY, 3, CFleetEffect::AT_ABSOLUTE );
				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_SHIELD_STRENGTH, mCommander->get_level()*2, CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_SHIELD_RECHARGE_RATE, mCommander->get_level()*2, CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_IMPENETRABLE_SHIELD, mCommander->get_level()*2, CFleetEffect::AT_ABSOLUTE );
				add_static_effect( NewEffect );
			}
			break;
		case CAdmiral::SA_MISSILE_SPECIALIST :
			if( mCommander->get_level() <= 5 ){
				NewEffect = new CFleetEffect( CFleetEffect::FE_MISSILE_AR, mCommander->get_level()+4, CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
			} else if( mCommander->get_level() <= 10 ){
				NewEffect = new CFleetEffect( CFleetEffect::FE_MISSILE_AR, mCommander->get_level()+6, CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_MISSILE_DAMAGE, mCommander->get_level(), CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
			} else if( mCommander->get_level() <= 17 ){
				NewEffect = new CFleetEffect( CFleetEffect::FE_MISSILE_AR, mCommander->get_level()+8, CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_MISSILE_DAMAGE, mCommander->get_level(), CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_MISSILE_COOLING_TIME, -mCommander->get_level()/2, CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
			} else if( mCommander->get_level() <= 19 ){
				NewEffect = new CFleetEffect( CFleetEffect::FE_MISSILE_AR, mCommander->get_level()+8, CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_MISSILE_DAMAGE, mCommander->get_level(), CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_MISSILE_COOLING_TIME, -mCommander->get_level(), CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
			} else if( mCommander->get_level() == 20 ){
				NewEffect = new CFleetEffect( CFleetEffect::FE_MISSILE_AR, mCommander->get_level()*2, CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_MISSILE_DAMAGE, mCommander->get_level()*2, CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_MISSILE_COOLING_TIME, -mCommander->get_level()*2, CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
			}
			break;
		case CAdmiral::SA_ENERGY_SYSTEM_SPECIALIST :
			if( mCommander->get_level() <= 5 ){
				NewEffect = new CFleetEffect( CFleetEffect::FE_BEAM_DAMAGE, mCommander->get_level(), CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
			} else if( mCommander->get_level() <= 10 ){
				NewEffect = new CFleetEffect( CFleetEffect::FE_BEAM_DAMAGE, mCommander->get_level(), CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_BEAM_COOLING_TIME, -mCommander->get_level(), CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
			} else if( mCommander->get_level() <= 16 ){
				NewEffect = new CFleetEffect( CFleetEffect::FE_BEAM_DAMAGE, mCommander->get_level(), CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_BEAM_COOLING_TIME, -mCommander->get_level(), CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_BEAM_AR, mCommander->get_level()-5, CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
			} else if( mCommander->get_level() <= 19 ){
				NewEffect = new CFleetEffect( CFleetEffect::FE_BEAM_DAMAGE, mCommander->get_level(), CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_BEAM_COOLING_TIME, -mCommander->get_level(), CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_BEAM_AR, mCommander->get_level(), CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
			} else if( mCommander->get_level() == 20 ){
				NewEffect = new CFleetEffect( CFleetEffect::FE_BEAM_DAMAGE, mCommander->get_level()*2, CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_BEAM_COOLING_TIME, -mCommander->get_level()*2, CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_BEAM_AR, mCommander->get_level()*2, CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
			}
			break;
		case CAdmiral::SA_BALLISTIC_EXPERT :
			if( mCommander->get_level() <= 5 ){
				NewEffect = new CFleetEffect( CFleetEffect::FE_PROJECTILE_COOLING_TIME, -mCommander->get_level(), CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
			} else if( mCommander->get_level() <= 10 ){
				NewEffect = new CFleetEffect( CFleetEffect::FE_PROJECTILE_COOLING_TIME, -mCommander->get_level(), CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_PROJECTILE_AR, mCommander->get_level(), CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
			} else if( mCommander->get_level() <= 16 ){
				NewEffect = new CFleetEffect( CFleetEffect::FE_PROJECTILE_COOLING_TIME, -mCommander->get_level(), CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_PROJECTILE_AR, mCommander->get_level(), CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_PROJECTILE_DAMAGE, mCommander->get_level()-5, CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
			} else if( mCommander->get_level() <= 19 ){
				NewEffect = new CFleetEffect( CFleetEffect::FE_PROJECTILE_COOLING_TIME, -mCommander->get_level(), CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
				...
				NewEffect = new CFleetEffect( CFleetEffect::FE_PROJECTILE_COOLING_TIME, -mCommander->get_level()*2, CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_PROJECTILE_AR, mCommander->get_level()*2, CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
	...
	switch( mCommander->get_racial_ability() ){
		case CAdmiral::RA_IRRATIONAL_TACTICS :
			if( mCommander->get_level() <= 7 ){
				...
				add_static_effect( NewEffect );
			} else if( mCommander->get_level() <= 13 ){
		...
		case CAdmiral::RA_INTUITION :
			...
			if( mCommander->get_level() >= 11 ){
				if( mCommander->get_level() <= 14 ){
					NewEffect = new CFleetEffect( CFleetEffect::FE_CRITICAL_HIT, 3, CFleetEffect::AT_ABSOLUTE );
				} else if( mCommander->get_level() <= 19 ){
					NewEffect = new CFleetEffect( CFleetEffect::FE_CRITICAL_HIT, 5, CFleetEffect::AT_ABSOLUTE );
				} else {
					NewEffect = new CFleetEffect( CFleetEffect::FE_CRITICAL_HIT, 7, CFleetEffect::AT_ABSOLUTE );
				}
				add_static_effect( NewEffect );
			}
			break;
		case CAdmiral::RA_LONE_WOLF:
			mBerserkModifier += 5;
			if( mCommander->get_level() >= 13 )
				mAttribute += WEAK_CLOAKING;
			break;
		case CAdmiral::RA_DNA_POISON_REPLICATER:
			if( mCommander->get_level() <= 10 ){
				NewEffect = new CFleetEffect( CFleetEffect::FE_PROJECTILE_DAMAGE, mCommander->get_level()/4, CFleetEffect::AT_ABSOLUTE );
				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_MISSILE_DAMAGE, mCommander->get_level(), CFleetEffect::AT_ABSOLUTE );
				add_static_effect( NewEffect );
			} else if( mCommander->get_level() <= 15 ){
				NewEffect = new CFleetEffect( CFleetEffect::FE_PROJECTILE_DAMAGE, mCommander->get_level()/2, CFleetEffect::AT_ABSOLUTE );
				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_MISSILE_DAMAGE, mCommander->get_level()*3/2, CFleetEffect::AT_ABSOLUTE );
				add_static_effect( NewEffect );
			} else if( mCommander->get_level() <= 19 ){
				NewEffect = new CFleetEffect( CFleetEffect::FE_PROJECTILE_DAMAGE, mCommander->get_level()*3/4, CFleetEffect::AT_ABSOLUTE );
				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_MISSILE_DAMAGE, mCommander->get_level()*2, CFleetEffect::AT_ABSOLUTE );
				add_static_effect( NewEffect );
			} else if( mCommander->get_level() == 20 ){
				NewEffect = new CFleetEffect( CFleetEffect::FE_PROJECTILE_DAMAGE, mCommander->get_level(), CFleetEffect::AT_ABSOLUTE );
				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_MISSILE_DAMAGE, mCommander->get_level()*5/2, CFleetEffect::AT_ABSOLUTE );
				add_static_effect( NewEffect );
			} 
			break;
		case CAdmiral::RA_BREEDER_MALE :
			NewEffect = new CFleetEffect( CFleetEffect::FE_REPAIR_SPEED, mCommander->get_level()+20, CFleetEffect::AT_PROPORTIONAL );
			add_static_effect( NewEffect );
			break;
		...
		case CAdmiral::RA_XENOPHOBIC_FANATIC :
			...
			if( mCommander->get_level() <= 7 ){
				...
				NewEffect = new CFleetEffect( CFleetEffect::FE_PSI_DEFENSE, -30, CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
			}
			break;
		case CAdmiral::RA_MENTAL_GIANT :
			...
			if( mCommander->get_level() <= 6 ){
				...
				NewEffect = new CFleetEffect( CFleetEffect::FE_PSI_DEFENSE, -5, CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_PSI_ATTACK, 5, CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
			} else if( mCommander->get_level() <= 15 ){
				NewEffect = new CFleetEffect( CFleetEffect::FE_PSI_ATTACK, 5, CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
				...
				NewEffect = new CFleetEffect( CFleetEffect::FE_PSI_ATTACK, 10, CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
				mAttribute += COMPLETE_CLOAKING_DETECTION;
			} else if( mCommander->get_level() == 20 ){
				NewEffect = new CFleetEffect( CFleetEffect::FE_PSI_ATTACK, 20, CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
				mAttribute += COMPLETE_CLOAKING_DETECTION;
			}
			break;
		...
		case CAdmiral::RA_PSYCHIC_PROGENITOR :
			mBerserkModifier += 5;
			mMoraleModifier += 10;
			if( mCommander->get_level() <= 2 ){
				NewEffect = new CFleetEffect( CFleetEffect::FE_PSI_ATTACK, 25, CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
			} else if( mCommander->get_level() <= 6 ){
				NewEffect = new CFleetEffect( CFleetEffect::FE_PSI_ATTACK, 25, CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
				mAttribute += COMPLETE_CLOAKING_DETECTION;
			} else if( mCommander->get_level() <= 12 ){
				NewEffect = new CFleetEffect( CFleetEffect::FE_PSI_ATTACK, 50, CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
				mAttribute += COMPLETE_CLOAKING_DETECTION;
			} else if( mCommander->get_level() <= 17 ){
				NewEffect = new CFleetEffect( CFleetEffect::FE_PSI_ATTACK, 75, CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
				mAttribute += COMPLETE_CLOAKING_DETECTION;
			} else if( mCommander->get_level() <= 19 ){
				NewEffect = new CFleetEffect( CFleetEffect::FE_PSI_ATTACK, 75, CFleetEffect::AT_PROPORTIONAL );
				...
				NewEffect = new CFleetEffect( CFleetEffect::FE_PSI_ATTACK, 100, CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
				mAttribute += COMPLETE_CLOAKING_DETECTION;
				mAttribute += COMPLETE_CLOAKING;
			}
		...
		case CAdmiral::RA_ARTIFACT_COOLING_ENGINE :
			if( mCommander->get_level() <= 12 ){
				NewEffect = new CFleetEffect( CFleetEffect::FE_COOLING_TIME, -mCommander->get_level(), CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
			} else if( mCommander->get_level() <= 18 ){
				...
				NewEffect = new CFleetEffect( CFleetEffect::FE_COOLING_TIME, -mCommander->get_level()*5/2, CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
			}
			break;
		case CAdmiral::RA_LYING_DORMANT :
			...
			if( mCommander->get_level() >= 7 ){
				if( mCommander->get_level() <= 13 ){
					NewEffect = new CFleetEffect( CFleetEffect::FE_STEALTH, 25, CFleetEffect::AT_PROPORTIONAL );
					add_static_effect( NewEffect );
				} else if( mCommander->get_level() <= 19 ){
					NewEffect = new CFleetEffect( CFleetEffect::FE_STEALTH, 50, CFleetEffect::AT_PROPORTIONAL );
					add_static_effect( NewEffect );
				} else if( mCommander->get_level() == 20 ){
					NewEffect = new CFleetEffect( CFleetEffect::FE_STEALTH, 100, CFleetEffect::AT_PROPORTIONAL );
					add_static_effect( NewEffect );
				}
			}
			break;
		case CAdmiral::RA_MISSILE_CRATERS :
			if( mCommander->get_level() <= 7 ){
				NewEffect = new CFleetEffect( CFleetEffect::FE_COOLING_TIME, -mCommander->get_level(), CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_ARMOR_DR, -mCommander->get_level(), CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
			} else if( mCommander->get_level() <= 14 ){
				NewEffect = new CFleetEffect( CFleetEffect::FE_COOLING_TIME, -mCommander->get_level()*3/2, CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_ARMOR_DR, -mCommander->get_level()*3/2, CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
			} else if( mCommander->get_level() <= 19 ){
				NewEffect = new CFleetEffect( CFleetEffect::FE_COOLING_TIME, -mCommander->get_level()*7/4, CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_ARMOR_DR, -mCommander->get_level()*3/2, CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
			} else if( mCommander->get_level() == 20 ){
		...
		case CAdmiral::RA_METEOR_DRONES :
			NewEffect = new CFleetEffect( CFleetEffect::FE_GENERIC_DEFENSE, mCommander->get_level(), CFleetEffect::AT_PROPORTIONAL );
			add_static_effect( NewEffect );
			break;
		case CAdmiral::RA_CYBER_SCAN_UNIT :
			mAttribute += WEAK_CLOAKING_DETECTION;
			if( mCommander->get_level() >= 9 ){
				if( mCommander->get_level() <= 12 ){
					NewEffect = new CFleetEffect( CFleetEffect::FE_AR, 3, CFleetEffect::AT_PROPORTIONAL );
					add_static_effect( NewEffect );
				} else if( mCommander->get_level() <= 15 ){
					NewEffect = new CFleetEffect( CFleetEffect::FE_AR, 5, CFleetEffect::AT_PROPORTIONAL );
					add_static_effect( NewEffect );
				} else if( mCommander->get_level() <= 19 ){
					NewEffect = new CFleetEffect( CFleetEffect::FE_AR, 7, CFleetEffect::AT_PROPORTIONAL );
					add_static_effect( NewEffect );
				} else if( mCommander->get_level() == 20 ){
		...
		case CAdmiral::RA_PATTERN_BROADCASTER :
			if( mCommander->get_level() <= 7 ){
				NewEffect = new CFleetEffect( CFleetEffect::FE_STEALTH, -25, CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_MISINTERPRET, 25, CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
			} else if( mCommander->get_level() <= 13 ){
				NewEffect = new CFleetEffect( CFleetEffect::FE_STEALTH, -50, CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_MISINTERPRET, 50, CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
			} else if( mCommander->get_level() <= 19 ){
				NewEffect = new CFleetEffect( CFleetEffect::FE_STEALTH, -75, CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_MISINTERPRET, 75, CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
			} else if( mCommander->get_level() <= 20 ){
				NewEffect = new CFleetEffect( CFleetEffect::FE_STEALTH, -90, CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_MISINTERPRET, 90, CFleetEffect::AT_PROPORTIONAL );
				add_static_effect( NewEffect );
			}
			break;
		...
		case CAdmiral::RA_RETREAT_SHIELD :
			...
			if( mCommander->get_level() <= 5 ){
				...
				NewEffect = new CFleetEffect( CFleetEffect::FE_SHIELD_SOLIDITY, 1, CFleetEffect::AT_ABSOLUTE);
 				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_SHIELD_STRENGTH, 10, CFleetEffect::AT_PROPORTIONAL );
 				add_static_effect( NewEffect );
			} else if( mCommander->get_level() <= 15 ){
				NewEffect = new CFleetEffect( CFleetEffect::FE_SHIELD_SOLIDITY, 2, CFleetEffect::AT_ABSOLUTE);
 				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_SHIELD_STRENGTH, 20, CFleetEffect::AT_PROPORTIONAL );
 				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_IMPENETRABLE_SHIELD, mCommander->get_level(), CFleetEffect::AT_ABSOLUTE );
 				add_static_effect( NewEffect );
			} else if( mCommander->get_level() <= 19 ){
				NewEffect = new CFleetEffect( CFleetEffect::FE_SHIELD_SOLIDITY, 3, CFleetEffect::AT_ABSOLUTE);
 				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_SHIELD_STRENGTH, 30, CFleetEffect::AT_PROPORTIONAL );
 				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_IMPENETRABLE_SHIELD, mCommander->get_level()*2, CFleetEffect::AT_ABSOLUTE );
 				add_static_effect( NewEffect );
			} else if( mCommander->get_level() == 20 ){
				mAttribute += COMPLETE_CLOAKING;
				NewEffect = new CFleetEffect( CFleetEffect::FE_SHIELD_SOLIDITY, 5, CFleetEffect::AT_ABSOLUTE);
 				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_SHIELD_STRENGTH, 100, CFleetEffect::AT_PROPORTIONAL );
 				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_IMPENETRABLE_SHIELD, 100, CFleetEffect::AT_ABSOLUTE );
 				add_static_effect( NewEffect );
			}
			break;
		...
		case CAdmiral::RA_RIGID_THINKING :
			...
			mMoraleModifier -= 10;
			if( mCommander->get_level() <= 10 ){
				NewEffect = new CFleetEffect( CFleetEffect::FE_PSI_DEFENSE, 10+mCommander->get_level(), CFleetEffect::AT_PROPORTIONAL );
 				add_static_effect( NewEffect );
			} else if( mCommander->get_level() <= 19 ){
				NewEffect = new CFleetEffect( CFleetEffect::FE_PSI_DEFENSE, 20+mCommander->get_level(), CFleetEffect::AT_PROPORTIONAL );
 				add_static_effect( NewEffect );
			} else if( mCommander->get_level() == 20 ){
				NewEffect = new CFleetEffect( CFleetEffect::FE_PSI_DEFENSE, 50, CFleetEffect::AT_PROPORTIONAL );
 				add_static_effect( NewEffect );
			}
			break;
		case CAdmiral::RA_SCAVENGER :
			if( mCommander->get_level() <= 10 ){
				NewEffect = new CFleetEffect( CFleetEffect::FE_REPAIR, 1, CFleetEffect::AT_PROPORTIONAL );
				NewEffect->set_period( 10 );
				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_REPAIR_SPEED, 10+mCommander->get_level(), CFleetEffect::AT_PROPORTIONAL );
 				add_static_effect( NewEffect );
			} else if( mCommander->get_level() <= 19 ){
				NewEffect = new CFleetEffect( CFleetEffect::FE_REPAIR, 2, CFleetEffect::AT_PROPORTIONAL );
				NewEffect->set_period( 10 );
				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_REPAIR_SPEED, 20+mCommander->get_level(), CFleetEffect::AT_PROPORTIONAL );
 				add_static_effect( NewEffect );
			} else if( mCommander->get_level() == 20 ){
				NewEffect = new CFleetEffect( CFleetEffect::FE_REPAIR, 3, CFleetEffect::AT_PROPORTIONAL );
				NewEffect->set_period( 10 );
				add_static_effect( NewEffect );
				NewEffect = new CFleetEffect( CFleetEffect::FE_REPAIR_SPEED, 50, CFleetEffect::AT_PROPORTIONAL );
 				add_static_effect( NewEffect );
			}
			break;
	...
	mMorale = calc_PA( mMorale, CFleetEffect::FE_MORALE );
...
{
	int
		Speed = mEngine->get_battle_speed(mSize-1) * (100+mMorale/5-20+mExp*3/20)/100;
...
{
	int
		Mobility = mEngine->get_battle_mobility(mSize-1)*(100+mMorale/10+mExp*3/10-15);
...
int
CBattleFleet::get_power()
{
	return mOriginalPower*get_active_ratio();
}
...
int
CBattleFleet::change_morale( int aMorale )
{
	mMorale += aMorale;
...
CBattleFleet::damage_first_active( int aDam, bool aPSI )
{
	...
	int
		MoraleDrop = aDam*100/mTotalHP,
		PSIMoraleDrop = MoraleDrop;

	if( aPSI ){
		PSIMoraleDrop = calc_minus_PA( PSIMoraleDrop, CFleetEffect::FE_PSI_DEFENSE );
...
CBattleFleet::get_highest_DAR_encounter_fleet()
{
	...
	for( int i = 0; i < mEncounterList.length(); i++ ){
		CTargetFleet
			*Target = (CTargetFleet*)mEncounterList.get(i);
		if( Target->get_danger_rating() > HighestDAR ){
...
{
	if( aEnemy->is_cloaked() == false ) return true;
	if( mAttribute.has( COMPLETE_CLOAKING_DETECTION ) ) return true;

	if( aEnemy->is_weak_cloaked() && mAttribute.has( WEAK_CLOAKING_DETECTION ) ) return true;
	return false;
}
...
CBattleFleet::damage( CTurret *aTurret, CBattleFleet *aEnemy, int aHitChance, int aCritChance, int *aTotalDamage, int *aHitCount, int *aMissCount, int *aSunkenCount )
{
	...
	for( int i = 0; i < ActiveShip; i++ ){
		...
		bool
			PSIIgnore = false;
		if (aEnemy->has_effect(CFleetEffect::FE_PSI_NEUTRALIZATION_FIELD))
		{
			if (number(100) <= calc_PA(0, CFleetEffect::FE_PSI_NEUTRALIZATION_FIELD))
				PSIIgnore = true;
		}
		...
		{	// hit
			...
			Dam -= aTurret->get_number() * ShieldSolidity;
			if( Dam <= 0 ){
				(*aMissCount)++;
				continue;
			}
			...
			*aTotalDamage += Dam;
			...
			int
				MoraleDrop = RealDam*100/aEnemy->get_total_hp();

			if( aTurret->has_effect( CFleetEffect::WE_PSI ) ){
				int
					PSIMoraleDrop = MoraleDrop;

				PSIMoraleDrop = calc_PA( PSIMoraleDrop, CFleetEffect::FE_PSI_ATTACK );
				PSIMoraleDrop = aEnemy->calc_minus_PA( PSIMoraleDrop, CFleetEffect::FE_PSI_DEFENSE );
				MoraleDrop += PSIMoraleDrop;
			}

			aEnemy->change_morale( -MoraleDrop );

			change_morale( RealDam*100/get_total_hp() );
...
void
CBattleFleetList::init(CPlayer *aOwner, int aSide)
{
	mOwner = aOwner;
	mSide = aSide;
...
CBattleFleetList::update_fleet_after_battle(CPlayer *aEnemy, int aWarType, bool aWin)
{
	...
	if (ScrapPP > 0)
	{
		...
		aEnemy->time_news(
				(char *)format(GETTEXT("You have earned %1$s PP with salvaging enemy ships."),
								dec2unit(ScrapPP)));
	}
}
...
CBattleFleetList::calc_total_power()
{
	...
	for( int i = 0; i < length(); i++ ){
		...
		mTotalPower += Fleet->get_original_power();
	}
}

void
CBattleFleetList::update_morale(int aMoraleUp, int aCapitalMorale, int aFleetMorale)
{
	for (int i=0 ; i<length() ; i++)
	{
		CBattleFleet *
		...
		int
			WeakMoraleBreak = 25 + Fleet->get_morale_modifier(),
			NormalMoraleBreak = 50 + Fleet->get_morale_modifier(),
			CompleteMoraleBreak = 75 + Fleet->get_morale_modifier();
		...
		if (Fleet->get_morale() < CompleteMoraleBreak)
		{
			...
			else if (OldMoraleStatus == CBattleFleet::MORALE_NORMAL_BREAK)
			{
				...
				else
				{
					...
					{
						Fleet->set_status(CBattleFleet::STATUS_DISORDER, 150 - Fleet->get_morale() - Fleet->get_efficiency());
					}
				}
			}
		}
...
CBattle::deploy_by_plan(CPlayer *aPlanOwner, CDefensePlan *aPlan, CFleetList *aFleetList, CAdmiralList *aAdmiralList, CBattleFleetList *aBattleFleetList)
{
	...
	if (aBattleFleetList->get_side() == CBattle::SIDE_DEFENSE)
	{
		...
		for (int i=0 ; i<DefenseFleetList->length() ; i++)
		{
			...
			else if (Fleet->get_status() != CFleet::FLEET_STAND_BY)
			{
				NumberOfMissingFleet++;
		...
		if (DefenseFleetList->length()/2 <= NumberOfMissingFleet) return false;
	}
...
CBattle::run_step()
{
	...
	for (int i=0 ; i<mOffenseFleetList.length() ; i++)
	{
		...
		int
			...
			PSI_Dam = Fleet->calc_PA(0, CFleetEffect::FE_PSI_DAMAGE);
		if (Dam > 0)
...
CBattle::prepare_turn(CBattleFleetList *aActive, CBattleFleetList *aPassive)
{
	for (int i=0 ; i<aActive->length() ; i++)
	{
		...
		if (Owner->has_ability(ABILITY_FRAGILE_MIND_STRUCTURE))
		{
			...
			{
				NewEffect->set_amount(20 + 30);
			}
			else
			{
				NewEffect->set_amount(20);
			}
...
#define ANTI_PSI_DEVICE		5526
		...
		{
			CDevice *
				Device = Fleet->get_device(i);
			if (Device != NULL)
			{
				if (Device->get_id() == ANTI_PSI_DEVICE)
				{
					NewEffect = new CFleetEffect(CFleetEffect::FE_PSI_NEUTRALIZATION_FIELD,
													95,
													CFleetEffect::AT_ABSOLUTE);
					Fleet->add_dynamic_effect(NewEffect);
					break;
				}
			}
...
CBattle::init_raid(CFleet *aFleet)
{
	...
	for(int i=0; i<mDefender->get_fleet_list()->length(); i++)
	{
				...
				Mission.get_target() == Planet->get_id())
		{
			...
			if (Comma) FleetName += ", ";
			FleetName.format(GETTEXT("the fleet %1$s on a patrol mission"),
								Fleet->get_nick());
		}
					...
					Mission.get_target() == Planet->get_id())
		{
			...
			if (Comma) FleetName += ", ";
			FleetName.format(GETTEXT("the fleet %1$s on a station mission"),
								Fleet->get_nick());
		}
	}
	...
	{
		mReport.format(GETTEXT("%1$s has %2$s to protect %3$s."),
						mDefender->get_nick(),
						(char *)FleetName,
						Planet->get_name());
	}
...
CBattle::raid_fleet_detection_check()
{
	...
	Roll += (BattleFleet->get_size()) - 5;

	// by raid bonus
	CAdmiral *Admiral = BattleFleet->get_admiral();
	Roll += Admiral->get_raid_level()*5;
	...
	for(int i=0; i<mDefenseFleetList.length(); i++)
	{
		...
		int Chance = Admiral->get_detection_level()*3 + 10;
		Chance += Admiral->get_prevent_raid_level();
		Chance *= (100+Planet->get_building().get(BUILDING_MILITARY_BASE));

		int
			Check;
		if (mAttacker->has_ability(ABILITY_STEALTH))
		{
			Check = number(Roll*100) + 25;
		}
		else
		{
			Check = number(Roll*100);
		}
...
CBattle::raid_bomb()
{
	...
	int
		MilitaryBase = Planet->get_building().get(BUILDING_MILITARY_BASE);
	...
	if (MilitaryBase > 0)
	{
		if (Damage*2 > MilitaryBase)
		{
			...
			mReport.format(GETTEXT("%1$s destroyed all military bases on the %2$s."),
					RaidFleet->get_nick(), 
					Planet->get_name());
			mReport += "<BR>\n";
			mDefender->time_news((char *)format("%s<BR>\n",
									GETTEXT("All military bases have been destroyed by raid.")));
		}
		else
		{
			MilitaryBase -= Damage*2;
			Planet->get_building().set(BUILDING_MILITARY_BASE, MilitaryBase);
			mReport.format(GETTEXT("The military bases on the %1$s protected the raid successfully."),
					Planet->get_name());
			mReport += "<BR>\n";
			mDefender->time_news((char*)format(
					GETTEXT("%1$s unit(s) of military base are/is destroyed by the raid."), 
						dec2unit(Damage*2)));
			Damage = 0;
		}
	}
	...
	if (Damage > 0)
	{
		...
		if (Factory > 0)
		{
			...
			mDefender->time_news((char*)format(
						GETTEXT("%1$s unit(s) of factory are/is destroyed by the raid."), 
			...
			{
				mReport.format(GETTEXT("%1$s has destroyed %2$s factory on the %3$s."),
								RaidFleet->get_nick(), 
								dec2unit(DamagedFactory),
								Planet->get_name());
			}
			...
			{
				mReport.format(GETTEXT("%1$s has destroyed %2$s factories on the %3$s."),
								RaidFleet->get_nick(), 
								dec2unit(DamagedFactory),
								Planet->get_name());
			}
		...
		if (ResearchLab > 0)
		{
			...
			mDefender->time_news((char*)format(
						GETTEXT("%1$s has destroyed %2$s research lab(s)."), 
							RaidFleet->get_nick(), 
		...
		if (Population > 0)
		{
			...
			mDefender->time_news(
					(char *)format(GETTEXT("The population was reduced by %1$s by the raid."), 
							dec2unit(DamagedPopulation)));

			Planet->change_population(-DamagedPopulation);

			mReport.format(GETTEXT("%1$s reduced population on the %$2s by %3$s."),
					RaidFleet->get_nick(), 
					Planet->get_name(),
					dec2unit(DamagedPopulation));
			mReport += "<BR>\n";
	...
	Admiral->gain_exp(1000);
	mAttacker->time_news(format(GETTEXT("%1$s made successful raid, he/she gained 1000 points of exp."), Admiral->get_name()));
...
CBattle::privateer_fleet_detection_check(bool aBattle)
{
	...
	Roll += (PrivateerFleet->get_size()) - 5;

	// by privateer bonus
	CAdmiral *Admiral = PrivateerFleet->get_admiral();
	Roll += Admiral->get_privateer_level()*5;
...
CBattleRecord::add_fleet( CBattleFleet *aFleet )
{
	...
	Buf.format( "FL/%d/%d/%s/%s/%s/%s/%d/%d/%d/%d/%d\n", aFleet->get_real_owner(), aFleet->get_real_id(), (char *)FleetNickString, aFleet->get_admiral()->get_name(), (char *)FleetClassNameString, aFleet->get_size_name(), aFleet->count_active_ship(), aFleet->get_x(), aFleet->get_y(), (int)aFleet->get_direction(), aFleet->get_command() );

	// start telecard 2001/02/07
	Buf.format( "# %s/%s/%s/%s/%s\n",
					aFleet->get_size_name(),
					aFleet->get_armor()->get_name(),
					aFleet->get_engine()->get_name(),
...
CBattleRecord::get_war_type_string()
{
	...
	{
		return GETTEXT("Magistrate Counterattack");
	}
	else if (mWarType == CBattle::WAR_EMPIRE_PLANET)
	{
	...
	{
		return GETTEXT("Empire Planet Counterattack");
	}
	else if (mWarType == CBattle::WAR_FORTRESS)
	{
