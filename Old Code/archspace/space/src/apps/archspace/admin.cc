...
char *
CAdminTool::mSEDPath = "/usr/bin/sed";
...
CPortalUserData::CPortalUserData()
{
	mID = 0;
	mICQ = 0;
	mAge = 0;
...
CAdminData::CAdminData(char *aAdminType, int aPortalID)
{
	mAdminType = aAdminType;
	mPortalID = aPortalID;
}

CAdminData::~CAdminData()
{
}

bool
CAdminData::set_admin_type(char *aAdminType)
{
	mAdminType = aAdminType;
	return true;
}
...
bool
CAdminData::set_portal_id(int aPortalID)
{
	mPortalID = aPortalID;
	return true;
}

CAdminDataList::CAdminDataList()
{
}
...
bool
CAdminDataList::free_item(TSomething aItem)
{
	CAdminData *
		AdminData = (CAdminData *)aItem;
...
int
CAdminDataList::compare(TSomething aItem1, TSomething aItem2) const
{
	CAdminData
		*AdminData1 = (CAdminData *)aItem1,
		*AdminData2 = (CAdminData *)aItem2;

	if (AdminData1->get_portal_id() > AdminData2->get_portal_id()) return 1;
	if (AdminData1->get_portal_id() < AdminData2->get_portal_id()) return -1;
	return 0;
}

int
CAdminDataList::compare_key(TSomething aItem, TConstSomething aKey) const
{
	CAdminData *
		AdminData = (CAdminData *)aItem;
 
	if (AdminData->get_portal_id() > (int)aKey) return 1;
	if (AdminData->get_portal_id() < (int)aKey) return -1;
	return 0;
}
 
bool
CAdminDataList::add_admin_data(CAdminData *aAdminData)
{
	if (aAdminData == NULL) return false;

	int
		Index = find_sorted_key((void *)aAdminData->get_portal_id());
	if (Index >= 0) return false;

	insert_sorted(aAdminData);

	return true;
}

bool
CAdminDataList::remove_admin_data(CAdminData *aAdminData)
{
	int
		Index = find_sorted_key((void *)aAdminData->get_portal_id());
	if (Index < 0) return false;
...
bool
CAdminDataList::remove_admin_data(int aPortalID)
{
	int
...
{
	char *
		AdminListFileName = CONFIG->get_string("Customer Support", "AdminListFile", NULL);
	if (AdminListFileName == NULL)
	{
		SLOG("ERROR : Couldn't get the admin list file name!");
		return false;
	}

	FILE *
		AdminListFile = fopen(AdminListFileName, "r");
	if (AdminListFile == NULL)
	{
		SLOG("ERROR : Couldn't load the admin list file!");
		return false;
	}
	...
	{
		char *
			Temp = fgets(BufferIndex, 16, AdminListFile);
		if (Temp == NULL) break;

		char *
			AdminType = strtok_r(BufferIndex, Separator, &Addr);
		if (AdminType == NULL)
		{
			SLOG("ERROR : AdminType is NULL in CAdminDataList::load()");
			return false;
		}
		else if (strcmp(AdminType, "S") && strcmp(AdminType, "M"))
		{
			SLOG("ERROR : AdminType is neither 'S' nor 'M' in CAdminDataList::load()");
			return false;
		}

		char *
			PortalIDString = strtok_r(NULL, Separator, &Addr);
		if (PortalIDString == NULL)
		{
			SLOG("ERROR : PortalIDString is NULL in CAdminDataList::load()");
			return false;
		}
		int
		...
		if (PortalID < 1)
		{
			SLOG("ERROR : PortalID is less than 1 in CAdminDataList::load()");
			return false;
		}

		CAdminData *
			AdminData = new CAdminData(AdminType, PortalID);

		add_admin_data(AdminData);
	}
...
CAdminDataList::save()
{
	...
	FILE *
		AdminListFile = fopen(AdminListFileName, "w");
	if (AdminListFile == NULL)
	{
		SLOG("ERROR : Couldn't load the admin list file!");
		return false;
	...
	{
		CAdminData *
			AdminData = (CAdminData *)get(i);
		fprintf(AdminListFile, "%s|%d\n",
				AdminData->get_admin_type(),
				AdminData->get_portal_id());
	}
...
bool 
CAdminTool::save_admin_list()
{
	return mAdminDataList.save();
}

bool
CAdminTool::add_admin_data(CAdminData *aAdminData)
{
	return mAdminDataList.add_admin_data(aAdminData);
}
...
CAdminTool::admin_list_html()
{
	...
	for (int i=0 ; i<mAdminDataList.length() ; i++)
	{
		CAdminData *
			AdminData = (CAdminData *)mAdminDataList.get(i);
		int
			PortalID = AdminData->get_portal_id();
		CPlayer *
			Admin = PLAYER_TABLE->get_by_portal_id(PortalID);
		if (Admin == NULL)
		{
			SLOG("ERROR : Admin is NULL in CAdminTool::admin_list_html(), PortalID = %d", PortalID);
			continue;
		}
		...
		List.format("<SELECT NAME=ADMIN_TYPE%d>", i);
		if (!strcmp(AdminData->get_admin_type(), "S"))
		{
			List.format("<OPTION SELECTED VALUE=SUPER_USER>%s</OPTION>", GETTEXT("Super User"));
		}
		else
		{
			List.format("<OPTION VALUE=SUPER_USER>%s</OPTION>", GETTEXT("Super User"));
		}
		if (!strcmp(AdminData->get_admin_type(), "M"))
		{
			List.format("<OPTION SELECTED VALUE=MODERATOR>%s</OPTION>",
						GETTEXT("Moderator"));
		}
		else
		{
			List.format("<OPTION VALUE=MODERATOR>%s</OPTION>", GETTEXT("Moderator"));
		}
		List.format("<OPTION VALUE=DEPRIVATION>%s</OPTION>", GETTEXT("Deprivation"));
		List += "</SELECT>";
		List += "</TD>\n";
		List += "</TR>\n";
	}
