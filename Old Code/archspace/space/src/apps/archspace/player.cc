	...
	switch(CStore::type())
	{
		case QUERY_INSERT :
			Query.format("INSERT INTO player"
				...
				" ability, protected_mode, protected_terminate_time)"
				" VALUES (%d, %d, '%s', %d, %d,"
				" %d, %d, %d, %d, %d, %d, %d,"
				" '%s', %d, %d)",
						mGameID, 
...
//						(char *)mLastLoginIP,
		...
		case QUERY_UPDATE :
			Query.format("UPDATE player SET turn = %d, admiral_timer = %d", 
					mTurn, mAdmiralTimer);
...
{
	if (aProduction == 0) return true;

	if (aProduction > 0)
	{
		if ((long long int)aProduction >
			(long long int)MAX_PLAYER_PP - (long long int)mProduction)
		{
			mProduction = MAX_PLAYER_PP;
		}
		else
	...
	{
		if (mProduction + aProduction > MAX_PLAYER_PP)
		{
			mProduction = MAX_PLAYER_PP;
		}
...
{
	if (aProduction > MAX_SHIP_PRODUCTION)
	{
		mShipProduction = MAX_SHIP_PRODUCTION;
	}
	else if (aProduction < 0)
	{
		mShipProduction = 0;
...
CPlayer::change_ship_production(int aProduction)
{
	...
	{
		if ((long long int)aProduction >
			(long long int)MAX_SHIP_PRODUCTION - (long long int)mShipProduction)
		{
			mShipProduction = MAX_SHIP_PRODUCTION;
		}
		else
	...
	{
		if (mShipProduction + aProduction > MAX_SHIP_PRODUCTION)
		{
			mShipProduction = MAX_SHIP_PRODUCTION;
		}
		else if (mShipProduction < -aProduction)
		{
			mShipProduction = 0;
		}
		else
...
CPlayer::change_invested_ship_production(int aProduction)
{
	...
	{
		if ((long long int)aProduction >
			(long long int)MAX_INVESTED_SHIP_PP - (long long int)mInvestedShipProduction)
		{
			mInvestedShipProduction = MAX_INVESTED_SHIP_PP;
		}
		else
		{
			mInvestedShipProduction += aProduction;
		}
	}
	else
	{
		if (mInvestedShipProduction + aProduction > MAX_INVESTED_SHIP_PP)
		{
			mInvestedShipProduction = MAX_INVESTED_SHIP_PP;
		}
		else if (mInvestedShipProduction < -aProduction)
		{
			mInvestedShipProduction = 0;
		}
		else
...
{
	if (aResearch == 0) return true;
	
	if (aResearch > 0)
	{
		if ((long long int)aResearch >
			(long long int)MAX_RESEARCH - (long long int)mResearch)
		{
			mResearch = MAX_RESEARCH;
		}
		else
		{
			mResearch += aResearch;
	...
	{
		if (mResearch + aResearch > MAX_RESEARCH)
		{
			mResearch = MAX_RESEARCH;
		}
		else if (mResearch < -aResearch)
		{
			mResearch = 0;
		}
		else
...
CPlayer::get_research_cost(CTech *aTech)
{
	int
		ResearchCost = 10000 * 
				(int)pow((double)2.0, (double)(aTech->get_level()));
	int
		Discount = 0;

	if (!mTargetTech)
		Discount = 20 + mControlModel.get_research()*4;

	if (Discount < 0) Discount = 0;
	if (Discount > 40) Discount = 40;
	
	return ResearchCost * (100 - Discount)/100;
}
...
CPlayer::update_turn()
{
	...
	for (int i=0 ; i<mEffectList.length() ; i++)
	{
		CPlayerEffect *
			Effect = (CPlayerEffect *)mEffectList.get(i);
		if (Effect->type() == CPlayerEffect::PA_IMPERIAL_RETRIBUTION)
		{
			continue;
		}
		else
...
CPlayer::get_extra_income_by_effect()
{
	...
	{
		int
			OldRP = mNewResource.get(RESOURCE_PRODUCTION);
		int
...
CPlayer::known_science_list_html(CTech::ETechType aType)
{
	...
	for(int i=0; i<mTechList.length(); i++)
	{
		...
		if (Tech->get_type() != aType) continue;
		HTML.format("&nbsp;<A HREF=\"/encyclopedia/game/tech/%d.html\">%s</A><BR>\n",
					Tech->get_id(), Tech->get_name());
	}
...
CPlayer::discover_tech(int aTechID, bool aIsInnate)
{
	...
	set_component_list(); /* telecard 2000/10/05 */
	...
	for (int i=0 ; i<ComponentList.length() ; i++)
	{
		...
		if (Component->is_for_race(mRace->get_id()) == false) continue;
...
CPlayer::set_available_tech_list()
{
	...
	for(int i=0; i<mTechList.length(); i++)
	{
		CKnownTech
			*Tech = (CKnownTech *)mTechList.get(i);
		if (Tech->get_attribute() != CTech::ATTR_INNATE && 
			Tech->get_level() > MaxKnownTechLevel)
			MaxKnownTechLevel = Tech->get_level();
	}
	...
	for(int i=0; i<TECH_TABLE->length(); i++)
	{
		...
		int
			TechID = Tech->get_id(),
			Prerequisite1 = Tech->get_prerequisite1(),
			Prerequisite2 = Tech->get_prerequisite2();

		if (!mTechList.get_by_id(TechID))
		{
			if ((Tech->get_level() <= MaxKnownTechLevel+1) &&
				(!Prerequisite1 || mTechList.get_by_id(Prerequisite1)) &&
				(!Prerequisite2 || mTechList.get_by_id(Prerequisite2)))
...
CPlayer::set_project_list()
{
	...
	for(int i=0; i<PROJECT_TABLE->length(); i++)
	{
		...
		if (Project->get_type() != TYPE_FIXED &&
			Project->get_type() != TYPE_PLANET) continue;
		int
			ProjectID = Project->get_id();
		if (mPurchasedProjectList.get_by_id(ProjectID)) continue;
		if (!Project->is_for_race(get_race())) continue;
...
CPlayer::set_component_list()
{
	...
	for(int i=0; i<mTechList.length(); i++)
	{
		CKnownTech *
			Tech = (CKnownTech *)mTechList.get(i);
		ComponentIDList += Tech->get_component_list();
	}

	for(int i=0; i<ComponentIDList.length(); i++)
	{
		CComponent *
			Component = COMPONENT_TABLE->get_by_id((int)ComponentIDList.get(i));

		if (Component == NULL) continue;
		if (Component->is_for_race(mRace->get_id()) == false) continue;

		mComponentList.add_component(Component);
	}
}
...
{
	CPurchasedProject *
		Project = mPurchasedProjectList.get_by_id(aProject);
...
void
CPlayer::buy_secret_project( int aProjectID )
{
	CProject*
...
{
	if (mAdmiralPool.length() >= 20) return;
	...
	int CreateTime = PERIOD_CREATE_ADMIRAL
			-(Military * ADMIRAL_MILITARY_BONUS);
...
CPlayer::can_buy_any_project()
{
	...
	{
		CProject *
			Project = (CProject *)mProjectList.get(i);
		int
			Prerequisite = Project->get_prerequisite();
		if (know_tech(Prerequisite)) Know = true;
	}
...
CPlayer::project_list_html()
{
	...
	List += "<TH CLASS=\"tabletxt\" WIDTH=\"144\" BGCOLOR=\"#171717\">";
	List += GETTEXT("Project Name"); 
	List += "</TH>\n";

	List += "<TH WIDTH=\"255\" CLASS=\"tabletxt\" BGCOLOR=\"#171717\">";
	List += GETTEXT("Effect");
	List += "</TH>\n";

	List += "<TH WIDTH=\"103\" CLASS=\"tabletxt\" BGCOLOR=\"#171717\">";
	List += GETTEXT("Cost");
	List += "</TH>\n";

	List += "<TH CLASS=\"tabletxt\" WIDTH=\"38\" BGCOLOR=\"#171717\">";
	List += "&nbsp;";
	List += "</TH>\n";
	...
	for (int i=0; i<mProjectList.length(); i++)
	{
		...
		if (!Project->is_for_race(mRace->get_id())) continue;

		int
			Prerequisite = Project->get_prerequisite();
		if (know_tech(Prerequisite) == false) continue;
...
CPlayer::change_council_production(int aProduction)
{
	...
	{
		if ((long long int)aProduction >
			(long long int)MAX_COUNCIL_PP - (long long int)mCouncilProduction)
		{
			mCouncilProduction = MAX_COUNCIL_PP;
		}
		else
		{
			mCouncilProduction += aProduction;
		}
	}
	else
	{
		if (mCouncilProduction + aProduction > MAX_COUNCIL_PP)
		{
			mCouncilProduction = MAX_COUNCIL_PP;
		}
		else if (mCouncilProduction < -aProduction)
		{
			mCouncilProduction = 0;
		}
		else
...
{
	if (aDonation > MAX_GENERAL_INT)
	{
		mCouncilDonation = MAX_GENERAL_INT;
	}
	else
	{
		mCouncilDonation = aDonation;
	}
...
{
	if (aDonation == 0) return true;
	
	if (aDonation > 0)
	{
		if ((long long int)aDonation >
			(long long int)MAX_GENERAL_INT - (long long int)mCouncilDonation)
		{
			mCouncilDonation = MAX_GENERAL_INT;
		}
		else
	...
	{
		if (mCouncilDonation + aDonation > MAX_GENERAL_INT)
		{
			mCouncilDonation = MAX_GENERAL_INT;
		}
		else if (mCouncilDonation < -aDonation)
		{
			mCouncilDonation = 0;
		}
		else
...
{
	if (aGain > MAX_VOTE_POWER)
	{
		mCouncilVoteGain = MAX_VOTE_POWER;
	}
	else
	{
		mCouncilVoteGain = aGain;
	}
...
CPlayer::change_council_vote_gain(int aGain)
{
	...
	if (aGain > 0)
	{
		if ((long long int)aGain >
			(long long int)MAX_VOTE_POWER - (long long int)mCouncilVoteGain)
		{
			mCouncilVoteGain = MAX_VOTE_POWER;
		}
		else
	...
	{
		if (mCouncilVoteGain + aGain > MAX_VOTE_POWER)
		{
			mCouncilVoteGain = MAX_VOTE_POWER;
		}
		else if (mCouncilVoteGain < -aGain)
		{
			mCouncilVoteGain = 0;
		}
		else
...
{
	int
		ret = mLastTurnResource.get(RESOURCE_PRODUCTION)*(30+mControlModel.get_military()*5)/100;
	if( ret < 0 ) return 0;
	return ret;
}
...
CPlayer::get_honor_description()
{
	...
	else if (mHonor <= 60) return GETTEXT("Average Citizen");
...
CPlayer::mission_handler()
{
	...
	for (int i=mFleetList.length()-1; i>=0; i--)
	{
		...
		if (Mission.get_mission() == CMission::MISSION_STATION_ON_PLANET)
			if (get_turn()%2) Admiral->gain_exp(1);
		...
		if (Mission.is_over())
		{
			switch(Mission.get_mission())
			{
				case CMission::MISSION_TRAIN :
				{
					...
					if (NewExp < 5) NewExp = 5;
					Fleet->change_exp(NewExp);
					Buf.format(GETTEXT("Your fleet %1$s has gained %2$s point(s) of exp. from the training."),
								Fleet->get_name(),
								dec2unit(NewExp));
					Buf += "<BR>\n";
					int
						AdmiralExp = Fleet->get_current_ship() *
										(Fleet->get_body()-4000) *
										(mControlModel.get_military() + 2)/4;
					if (AdmiralExp < 10) AdmiralExp = 10;
...
CPlayer::change_research_invest(int aAmount)
{
	...
	if (aAmount > 0)
	{
		if ((long long int)aAmount >
			(long long int)MAX_RESEARCH_INVESTMENT_PP - (long long int)mResearchInvest)
		{
			mResearchInvest = MAX_RESEARCH_INVESTMENT_PP;
		}
		else
		{
			mResearchInvest += aAmount;
		}
	}
	else
	{
		if (mResearchInvest + aAmount > MAX_RESEARCH_INVESTMENT_PP)
		{
			mResearchInvest = MAX_RESEARCH_INVESTMENT_PP;
		}
		else if (mResearchInvest < -aAmount)
		{
			mResearchInvest = 0;
		}
		else
...
CPlayer::declare_war(CPlayer *aPlayer)
{
	...
	// a player who has a lower honor, reduce honor point 
	// which amount is (difference of honor) / 6
	if (get_honor() < aPlayer->get_honor())
		change_honor_with_news(
				(int)((aPlayer->get_honor()-get_honor())/6));

	CPlayerAction *Action = check_break_pact_effect_timer(aPlayer);
	if (Action)
	{
		if (Action->get_start_time() <= PERIOD_24HOURS*CGame::mSecondPerTurn)
			change_honor_with_news(-40);	
		else if (Action->get_start_time() <= PERIOD_48HOURS*CGame::mSecondPerTurn)
			change_honor_with_news(-20);
		else // over 48 hours
			change_honor_with_news(-10);
	...
	if (mAbility.has(ABILITY_PACIFIST))
	{
		int
			OldHonor = mHonor;
		set_honor(OldHonor - 10);

		time_news((char *)format(GETTEXT("You have declared war with %1$s, and your honor has been decreased by 10!"), 
									aPlayer->get_nick()));
	}
	else
	{
...
CPlayer::break_ally(CPlayer *aPlayer)
{
	...
	// which amount is (difference of honor) / 6
	if (get_honor() < aPlayer->get_honor())
		change_honor_with_news((int)((aPlayer->get_honor() - get_honor())/6));
...
CPlayer::get_message_list_html(int First, bool aIsForAdmin)
{
	...
	HTML.format("<IMG SRC=http://%s/image/as_game/council/required1_icon.gif WIDTH=19 HEIGHT=16 ALIGN=ABSBOTTOM>\n&nbsp;&nbsp;&nbsp;&nbsp;",
				(char *)CGame::mImageServerURL);
	HTML += GETTEXT("Not Required");
	HTML.format("<IMG SRC=http://%s/image/as_game/council/notrequired1_icon.gif WIDTH=19 HEIGHT=16 ALIGN=ABSBOTTOM>",
				(char *)CGame::mImageServerURL);
	HTML += "</TD>\n";
...
CPlayer::improve_relationship(CDiplomaticMessage::EType aType, CPlayer *aPlayer)
{
	...
	else if (Relation->get_relation() == CRelation::RELATION_WAR &&
...
CPlayer::war_target_list_html()
{
	...
	List.format("<FONT COLOR=\"666666\">%s</FONT>", GETTEXT("# of Planet(s)"));
	List += "</TH>\n";
	...
	for (int i=0 ; i<mRelationList.length() ; i++)
	{
		...
		if (TargetPlayer->get_power()*100/mPower > PLAYER_ATTACK_RESTRICTION_POWER_MAX ||
			TargetPlayer->get_power()*100/mPower < PLAYER_ATTACK_RESTRICTION_POWER_MIN) continue;
	...
	for (int i=0 ; i<CouncilRelationList->length() ; i++)
	{
		...
		for (int i=0 ; i<MemberList->length() ; i++)
		{
			...
			if (Member->get_power()*100/mPower > PLAYER_ATTACK_RESTRICTION_POWER_MAX ||
				Member->get_power()*100/mPower < PLAYER_ATTACK_RESTRICTION_POWER_MIN) continue;
...
CPlayer::battle_record_list_html(bool aFromHead, int aStartBattleRecordIndex, int aNumberOfBattleRecord, int *aTotalRecord)
{
	...
	while (Index >= 0 && Index < BattleRecordList.length() && Number < aNumberOfBattleRecord)
	{
		...
		List.format("<IMG SRC=\"http://%s/image/as_game/bu_view.gif\" WIDTH=\"44\" HEIGHT=\"11\" BORDER=\"0\">\n",
					(char *)CGame::mImageServerURL);
		List += "</A>\n";
		List += "</TD>\n";
...
CPlayer::privateer(CFleet *aFleet)
{
	...
	if (Battle.privateer_fleet_detection_check(true))
	{
		...
		Report += "<BR>\n";
		Report += GETTEXT("You were attecked by the defensive fleets on the planet.");
		Report += "<BR>\n";
	...
	else
	{
		...
		{
			int Capa 
					= aFleet->get_current_ship() * (aFleet->get_body()-4000) * 2;
			if (Capa > 100) Capa = 100;
			...
			change_reserved_production(Amount/2);
			...
			Report.format(GETTEXT("The privateer fleet %1$s has gained %2$s PP on %3$s,<BR>"
									"and the fleet admiral has gained 250 exp."), 
							aFleet->get_nick(),
							dec2unit(Amount/2),
							Planet->get_name());
			Report += "<BR>\n";
		...
		if (Battle.privateer_fleet_detection_check(false))
		{
			if (Amount)
				Player->time_news(
						(char *)format(GETTEXT("%1$s succeeded in the privateer. %2$s PP has been stolen from you."), 
						get_nick(), dec2unit(Amount/2)));
			else 
				Player->time_news(
						(char *)format(GETTEXT("%1$s succeeded in the privateer. But nothing has been stolen from you."),
						get_nick()));

			Report.format(GETTEXT("You are detected after the privateering.<BR>"
									"%1$s knows who was the privateer."),
							Player->get_nick());
		}
		else
		{
			if (Amount)
				Player->time_news(
						(char *)format(GETTEXT("Someone succeeded in the privateer. %1$s PP has been stolen from you."),
						dec2unit(Amount/2)));
			else
				Player->time_news(GETTEXT("Someone succeeded in the privateer. But nothing has been stolen from you."));

			Report.format(GETTEXT("You were not detected out after privateer.<BR>"
									"%1$s didn't know your identity."),
							Player->get_nick());
		}
...
#define STEAL_TECHNOLOGY				8008
	...
	switch (Spy->get_id())
	{
		...
		case RED_DEATH :
			...
			{
				AttackRoll = 50 + ((mControlModel.get_real_spy() + 3)*10);
			}
			else
			{
				AttackRoll = 50 + (mControlModel.get_real_spy() * 10);
			}
		...
		case STEAL_TECHNOLOGY :
			...
			{
				AttackRoll = 50 + ((mControlModel.get_real_spy() + 5)*10);
			}
			else
			{
				AttackRoll = 50 + (mControlModel.get_real_spy()*10);
...
CPlayer::get_relation_lost_per_day_by_rank()
{
	...
	return relation_lost[get_court_rank()];
}
...
CPlayer::grant_boon( int aAction )
{
	...
	if( has_ability(ABILITY_DIPLOMAT) )
		SupportCheck = SupportCheck * (20+get_control_model()->get_diplomacy()+3) / 20;
	else
		SupportCheck = SupportCheck * (20+get_control_model()->get_diplomacy()) / 20;

	if( SupportCheck <= 11 ){
		Reward = CEmpireAction::EA_REWARD_LOW_PRODUCTION;
	} else if( SupportCheck <= 21 ){
	...
	} else if( SupportCheck <= 105 ){
		Reward = CEmpireAction::EA_REWARD_LEVEL56_TECH;
	} else if( SupportCheck <= 115 ){
		Reward = CEmpireAction::EA_REWARD_LEVEL710_COMMANDER;
	} else if( SupportCheck <= 125 ){
		Reward = CEmpireAction::EA_REWARD_CLASS34_FLEET;
	} else if( SupportCheck <= 130 ){
	...
	switch( Reward ){
		...
		case CEmpireAction::EA_REWARD_CLASS34_FLEET :
			break;
	...
	switch (aEffect->get_type())
	{
		case CPlayerEffect::PA_CHANGE_PLANET_RESOURCE :
		{
			...
			if (Planet != NULL)
			{
				...
				Buf.format(GETTEXT("The status of %1$s planet has been upgraded to %2$s."),
							Planet->get_name(),
		...
		case CPlayerEffect::PA_LOSE_PLANET :
		{
			...
			if (Planet != NULL)
			{
				...
				Buf.format(GETTEXT("You lost the planet %1$s."), Planet->get_name());
			}
		}
			break;
		...
		case CPlayerEffect::PA_DESTROY_ALL_DOCKED_SHIP :
		{
			...
			Buf = GETTEXT("You lost all ships in your dock.");
		}
			break;
		...
		{
			if (know_tech(aEffect->get_target()) == true)
			{
				CKnownTech *
					Tech = mTechList.get_by_id(aEffect->get_target());
				Buf.format(GETTEXT("You lost the tech %1$s."), Tech->get_name());
				forget_tech(aEffect->get_target());
			}
		}
			break;
		...
		case CPlayerEffect::PA_LOSE_PROJECT :
		{
			...
			if (Project != NULL)
			{
				Buf.format(GETTEXT("You lost the project %1$s."), Project->get_name());
				lose_project(aEffect->get_target());
			}
		}
			break;
			...
			if (Planet != NULL)
			{
				switch(aEffect->get_argument2())
				{
					...
					case BUILDING_RESEARCH_LAB :
					{
						...
						Buf.format(GETTEXT("You lost %1$d %2$s on %3$s."),
									-Lost,
		...
		case CPlayerEffect::PA_CHANGE_PRODUCTION :
		{
			...
			if (DiffProduction > 0)
			{
				Buf.format(GETTEXT("You gained %1$s PP cause of the effect \"%2$s\"."),
							DiffProduction,
							CPlayerEffect::get_type_name(CPlayerEffect::PA_CHANGE_PRODUCTION));
			}
			else if (DiffProduction < 0)
			{
				Buf.format( GETTEXT("You lost %1$d PP cause of the effect \"%2$s\"."),
							-DiffProduction,
							CPlayerEffect::get_type_name(CPlayerEffect::PA_CHANGE_PRODUCTION));
			}
		}
		...
		case CPlayerEffect::PA_GAIN_TECH :
		{
			...
			if (NewTech != NULL)
			{
				if (know_tech(aEffect->get_target()) == false)
				{
					...
					Buf.format(GETTEXT("You discovered a new tech %1$s."),
								NewTech->get_name());
				}
			}
		}
		...
		case CPlayerEffect::PA_GAIN_SECRET_PROJECT :
		{
			...
			if (Project != NULL)
			{
				if (mPurchasedProjectList.get_by_id(aEffect->get_target()) == NULL)
				{
					...
					Buf.format(GETTEXT("You gained a new project %1$s."),
								Project->get_name());
				}
			}
		}
		...
		case CPlayerEffect::PA_COMMANDER_LEVEL_UP :
		{
			...
			if (Admiral != NULL)
			{
				...
				Buf.format(GETTEXT("Your Commander %1$s has gained a level."),
							Admiral->get_name());
			}
		}
			break;
		...
		case CPlayerEffect::PA_LOSE_COMMANDER :
		{
			...
			if (Lose > 0)
			{
				...
				Buf.format(GETTEXT("You lost %1$s fleet commander(s)."), dec2unit(Lose));
			}
		}
			break;
		...
		case CPlayerEffect::PA_KILL_COMMANDER_AND_DISBAND_FLEET :
		{
			...
			if (Fleet != NULL)
			{
				...
				Buf.format(GETTEXT("You lost your fleet %1$s and its commander %2$s."),
...
CPlayer::initialize_fortress(CMySQL *aMySQL)
{
	...
	for (int i=1 ; i<=4 ; i++)
	{
		for (int j=1 ; j<=pow(2, (5-i)) ; j++)
		{
			for (int k=1 ; k<=pow(2, (5-i)) ; k++)
			{
