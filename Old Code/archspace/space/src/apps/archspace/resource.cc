...
CResource::change(EResourceType aResourceType, int aResourceValue)
{
	...
	if (aResourceValue > 0)
	{
		if ((long long int)aResourceValue >
			(long long int)MAX_RESOURCE - (long long int)mResource[aResourceType])
		{
			mResource[aResourceType] = MAX_RESOURCE;
		}
		else
	...
	{
		if (mResource[aResourceType] + aResourceValue > MAX_RESOURCE)
		{
			mResource[aResourceType] = MAX_RESOURCE;
		}
		else if (mResource[aResourceType] < -aResourceValue)
		{
			mResource[aResourceType] = 0;
		}
		else
