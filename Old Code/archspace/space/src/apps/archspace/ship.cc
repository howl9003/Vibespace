...
CShipDesign::query()
{
	...
	switch( type() ) {
		case QUERY_INSERT:
			query.format( "INSERT INTO class ( owner, design_id, body, name, armor, engine, computer, shield, weapon1, weapon2, weapon3, weapon4, weapon5, weapon6, weapon7, weapon_number1, weapon_number2, weapon_number3, weapon_number4, weapon_number5, weapon_number6, weapon_number7, device1, device2, device3, device4, device5, time, cost ) VALUES ( %d, %d, %d, '%s', %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d)", 
			mOwner, mDesignID, mBody, (char*)add_slashes((char*)mName), mArmor, mEngine, mComputer, mShield, get_weapon(0), get_weapon(1), get_weapon(2), get_weapon(3), get_weapon(4), get_weapon(5), get_weapon(6), get_weapon_number(0), get_weapon_number(1), get_weapon_number(2), get_weapon_number(3), get_weapon_number(4), get_weapon_number(5), get_weapon_number(6), get_device(0), get_device(1), get_device(2), get_device(3), get_device(4), mBuildTime, mBuildCost );
			break;
		case QUERY_UPDATE:
			query.format( "UPDATE class SET time = %d WHERE owner = %d AND design_id = %d", mBuildTime, mOwner, mDesignID );
			break;
		case QUERY_DELETE:
...
CShipDesign::print_html(CPlayer *aPlayer, int aPP )
{
	...
	Class.format("<TD COLSPAN=\"3\" CLASS=\"tabletxt\">&nbsp;%s </TD>\n",
						get_name());
	Class += "<TD CLASS=\"tabletxt\" ALIGN=\"CENTER\">";
	if (can_delete(aPlayer) == DELETE_OK)
	{
		Class.format("<A HREF=\"ship_design_delete_confirm.as?DESIGN_ID=%d\">",
						mDesignID);
		Class.format("<IMG SRC=\"http://%s/image/as_game/bu_delete.gif\" BORDER=\"0\">",
...
CShipDesign::class_info_for_building_html(CPlayer *aPlayer)
{
	...
	if (!RealShipProduction)
	{
		CString
			TurnString;
		TurnString.clear();
		TurnString.format(GETTEXT("%1$s turn(s)"), "?");
	...
	} else
	{
		...
		CString
			TurnString;
		TurnString.clear();
		TurnString.format(GETTEXT("%1$s turn(s)"), dec2unit(TimeCost));
...
CShipDesignList::building_class_select_html(CPlayer *aPlayer)
{
	...
	ClassSelect += GETTEXT("# of Ship(s)<BR>in the Pool");
	ClassSelect += "</FONT></TH>\n";
	ClassSelect += "</TR>\n";
...
CShipToBuild::ship_to_build_info_html(int aIndex)
{
	...
	Info.format("<A HREF=ship_building_delete.as?INDEX=%d>",
					aIndex);
	Info.format("<IMG SRC=\"http://%s/image/as_game/bu_delete.gif\" WIDTH=\"57\" HEIGHT=\"11\" BORDER=\"0\"></TD>\n",
				(char *)CGame::mImageServerURL);
	Info += "</A>\n";
	Info += "</TR>\n";
...
#undef SPACE_MINING_MODULE
	...
	Upkeep = get_number() * UpkeepTable[get_body()-4001];
...
CDockedShip::get_power()
{
	...
	return (get_body()-4000)*TotalLevel*get_number();
}
...
CDockedShip::docked_ship_info_html(int aIndex)
{
	...
	ShipInfo.format("<INPUT TYPE=\"hidden\" NAME=\"DESIGN_ID\" VALUE=\"%d\">\n",
					get_design_id());
	ShipInfo.format("<INPUT TYPE=\"image\" SRC=\"http://%s/image/as_game/bu_ok.gif\" WIDTH=\"31\" HEIGHT=\"11\"></TD>\n",
					(char *)CGame::mImageServerURL);
	ShipInfo += "</FORM>\n";
	ShipInfo += "</TR>\n";
...
CDock::print_html()
{
	...
	Buf += "<TR>\n";
	Buf.format("<TD>%s</TD>\n", GETTEXT("Ship Class"));
	Buf.format("<TD>%s</TD>\n", GETTEXT("# of Ship(s) Available"));
	Buf.format("<TD>%s</TD>\n", GETTEXT("Size"));
	Buf.format("<TD>%s</TD>\n", GETTEXT("Scrap Income per Ship"));
	Buf.format("<TD>%s</TD>\n", GETTEXT("# of Ship(s) to Scrap"));
	Buf += "</TR>\n";
...
CDock::dock_info_html()
{
	...
	DockInfo.format("<FONT COLOR=\"666666\">%s</FONT></TH>\n",
						GETTEXT("# of Ship(s)<BR>Available"));
...
CDock::radio_ship_reassignment_html(int designID)
{
	...
	DockInfo += GETTEXT("# of Ship(s) Available");
	DockInfo += "</FONT></TH>\n";
