...
/**
*/
class CArchspaceDatabase: public CDatabase
{
	public:
		CArchspaceDatabase(): CDatabase() {}
		virtual ~CArchspaceDatabase() {}

		bool initialize(int aArgc, char **aArgv);
		virtual const char *debug_info() const { return "archspace database"; }
};
...
class CArchspaceStoreCenter: public CStoreCenter
{
	public:
		...
		inline bool packet_empty() { return mOutput.empty(); }

		void push();

		static void* send(void *aArg);
		static void* write(void *aArg);

		static inline void lock_sql();
		static inline void unlock_sql();

		static inline void lock_packet();
		static inline void unlock_packet();

		static void spawn_database_sql_thread();
		static void spawn_database_packet_thread();
		static void kill_database_sql_thread();
		static void kill_database_packet_thread();

	protected:
		static pth_mutex_t mSQLMutex;
		static pth_mutex_t mPacketMutex;

		static pth_t mSQLThread;
		static pth_t mPacketThread;
};
