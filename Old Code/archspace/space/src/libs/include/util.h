...
class CCollection: public CBase
{
	public:
		...
		CNode* append(CNode* aNode);

		CNode* insert(CNode* aNode);

		bool remove(CNode* aNode);

		bool remove_without_free(CNode* aNode);
		...
		int length() { return mLength; }				// Added by YOSHIKI(2001/05/03)
...
/**
*/
class CCommandSet:public CBase
{
	public:
		CCommandSet();
