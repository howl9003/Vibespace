...
/***************************************************************************
    ...
    $Id: editpost.php,v 1.62 2001/02/08 06:37:32 thefinn Exp $
...
if($submit) {
   ...
   if(!$delete) {
	 ...
	 if(!$r = mysql_query($sql, $db))
      ...
      echo "<br><TABLE BORDER=\"0\" CELLPADDING=\"1\" CELLSPACEING=\"0\" ALIGN=\"CENTER\" VALIGN=\"TOP\" WIDTH=\"$tablewidth\">";
      echo "<TR><TD  BGCOLOR=\"$table_bgcolor\"><TABLE BORDER=\"0\" CALLPADDING=\"1\" CELLSPACEING=\"1\" WIDTH=\"100%\">asd";
      echo "<TR BGCOLOR=\"$color1\" ALIGN=\"LEFT\"><TD><font face=\"Verdana\" size=\"2\"><P>";
...
<TR ALIGN="LEFT">
		...
		<?php
			...
			$now_min = date("i");
			list($hour, $min) = split(":", $time);
			$temp = is_moderator($forum, $userdata[user_id], $db);

log_("THEDAZ: forum = $forum");
log_("THEDAZ: GAME_ID = $GAME_ID");
log_("THEDAZ: user_level = $userdata[user_level]");
log_("THEDAZ: $temp");
log_("THEDAZ: _________________________");
