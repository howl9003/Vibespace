...
/***************************************************************************
    ...
    $Id: admin_users.php,v 1.21 2001/02/05 05:22:39 thefinn Exp $
...
switch($mode) {
	case 'moduser':
		if($submit && $user_id) {
			$sql = "UPDATE users SET username = '$username', user_email = '$email', user_rank = '$rank', user_level = '$level' WHERE user_id = $user_id";
		...
		echo "<td align=\"center\" width=\"100%\" bgcolor=\"$color2\"><font face=\"$FontFace\" size=\"$FontSize1\" color=\"$textcolor\"><P><BR>&nbsp;&nbsp;Click <a href=\"$url_admin\">here</a> to return to the Administration Panel.<P>Click <a href=\"$PHP_SELF?mode=moduser\">here</a> to modify another user.</font><P><BR><P></TD>";
...
<?php
				...
				$moduserdata = get_userdata_from_id($user_id, $db);
...
<TR ALIGN="LEFT">
	...
	<TD BGCOLOR="<?php echo $color2?>"><INPUT TYPE="TEXT" NAME="username" VALUE="<?php echo $moduserdata[username]?>" MAXLENGTH=40 SIZE=25></TD>
</TR>
<TR ALIGN="LEFT">
 ...
 case 'badusernames':
   if($edit || $add || $delete) {
			if($add) {
				...
				$sql = "INSERT INTO disallow (disallow_username) VALUES ('$username')";
				if(!$r = mysql_query($sql, $db))
				...
				$username = addslashes($username);
				$sql = "UPDATE disallow SET disallow_username = '$username' WHERE disallow_id = '$id'";
				if(!$r = mysql_query($sql, $db))
...
<?php
	...
	if($m = mysql_fetch_array($r)) {
		do {
			echo "<TR BGCOLOR=\"$color2\" ALIGN=\"CENTER\">\n";
			echo "<TD><FORM ACTION=\"$PHP_SELF\" METHOD=\"POST\"><INPUT TYPE=\"TEXT\" NAME=\"username\" VALUE=\"" . stripslashes($m[disallow_username]) . "\" MAXLENGTH=\"40\" SIZE=\"25\"></TD>\n";
...
<TR BGCOLOR="<?php echo $color2?>" ALIGN="CENTER">
	<TD><FORM ACTION="<?php echo $PHP_SELF?>" METHOD="POST">
	    <INPUT TYPE="TEXT" NAME="username" MAXLENGTH="50" SIZE="25">
	</TD>
...
<?php
	...
	case 'remuser':
		if($submit) {
		   ...
		   echo "</tr><TR><TD><TABLE width=\"100%\" cellspacing=\"0\" cellpadding=\"0\"><TR>";
		   echo "<td align=\"center\" width=\"100%\" bgcolor=\"$color2\"><font face=\"$FontFace\" size=\"$FontSize1\" color=\"$textcolor\"><P><BR>&nbsp;&nbsp;Click <a href=\"$url_admin\">here</a> to return to the Administration Panel.<P>Click <a href=\"$url_phpbb\">here</a> to return to the forum index.</font><P><BR><P></TD>";
		   echo "</TR></table></TD></TR></TABLE>";
		}
                else {
...
<?php   
   ...
   ?>
          ...
          Go <a href="<?php echo $url_phpbb?>">Back</a>
          </TD></TR></TABLE></TD></TR></TABLE>
     <?php
}
