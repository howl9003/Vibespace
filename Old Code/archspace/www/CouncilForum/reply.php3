...
$GAME_NAME_DECODE = urldecode($GAME_NAME);
   ...
   $sql = "INSERT INTO posts (topic_id, forum_id, poster_id, post_text, post_time, poster_ip, poster_name) VALUES ('$topic', '$forum', '$userdata[user_id]', '$message', '$time', '$poster_ip', '$GAME_NAME_DECODE')";
   if(!$result = mysql_query($sql, $db)) {
      error_die("Error - Could not enter data into the database. Please go back and try again");
   }
   $sql = "UPDATE topics SET topic_time = '$time' WHERE topic_id = '$topic'";
   if(!$result = mysql_query($sql, $db)) {
      error_die("Error - Could not enter data into the database. Please go back and try again");
   }
...
$GAME_NAME_DECODE = urldecode($GAME_NAME);
     ...
     if ($user_logged_in) {
log_("THEDAZ: \$user_logged_in TRUE");
...
<?PHP
	if (!$user_logged_in) { 
log_("THEDAZ: \$user_logged_in FALSE");
...
//		echo "<INPUT TYPE=\"HIDDEN\" NAME=\"password\" SIZE=\"25\" MAXLENGTH=\"25\" VALUE=\"55555\">\n";
// </TD> \n";
//		echo "    </TR> \n";
	}
?>
...
*/
		...
		if($quote) {
log_("THEDAZ: quote");
			$sql = "SELECT p.post_text, p.post_time, u.username FROM posts p, users u WHERE post_id = '$post' AND p.poster_id = u.user_id";
		...
		<TD  BGCOLOR="<?php echo $color2?>"><font size="<?php echo $FontSize2?>" face="<?php echo $FontFace?>"><TEXTAREA NAME="message" ROWS=10 COLS=45 WRAP="VIRTUAL"><?php echo $reply?></TEXTAREA></TD>
	</TR>
	<TR>
