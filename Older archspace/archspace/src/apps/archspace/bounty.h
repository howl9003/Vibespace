...
{
	private:
		std::map<int, COfferedBounty*> mBounties;
	...
	public:
		...
		std::vector<COfferedBounty*> *get_available_bounties(CPlayer *aPlayer);
};

class CBountyList
{
	private:
		std::map<int, CBounty*> mBounties;
	...
	public:
		...
		inline std::map<int, CBounty*> *get_current_bounties();
		inline int size();
};

inline map<int, CBounty*> *
