	...
	switch(CStore::type())
	{
		case QUERY_INSERT :
			Query.format("INSERT INTO %s.%s"
				" (id, user_level, dev_level, name, password,"
				" age, email, country, firstlogin,"
				" ip)"
				" VALUES (%d, '%d', %d, %s,"
				" %s, %s, %s, %s, %s, %s)",
				        PortalDB,
...
#define STORE(x, y, z) \
			...
			{
				STORE(STORE_PASSWORD, ", password = '%s'",
						(char *)add_slashes((char *)mPassword));
			}

			if (mEmail.length() > 0)
			{
				STORE(STORE_EMAIL, ", email = '%s'",
						(char *)add_slashes((char *)mEmail));
			}
            ...
            Query.format(" WHERE id = %d", mPortalID);

			break;

		case QUERY_DELETE :
			Query.format("DELETE FROM %s.%s WHERE id = %d", 
                        PortalDB,
                        PortalTable,
						mPortalID );
			break;
