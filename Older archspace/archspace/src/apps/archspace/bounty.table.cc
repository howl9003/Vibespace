...
{
	std::map<int, CBounty*>::iterator aIter = mBounties.find(aID);
...
{
	std::map<int, CBounty*>::iterator aIter;
	for (aIter = mBounties.begin(); aIter != mBounties.end(); aIter++) 
	{
		if ((aIter->second)->get_target_player()->get_game_id()==aID) return aIter->second;
	}
	return NULL;
}
...
CBountyTable::remove_by_target_id(int aID)
{
	...
	for (aIter = mBounties.begin(); aIter != mBounties.end(); aIter++) 
	{
		if ((aIter->second)->get_target_player()->get_game_id()==aID) remove(aIter->second);
	}
}
