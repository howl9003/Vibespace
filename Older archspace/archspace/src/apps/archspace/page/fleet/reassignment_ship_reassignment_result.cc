...
//	system_log( "start page handler %s", get_name() );
	...
	int LostExp = 0;
	if (ChangeFleetSize > 0)
	{
		int BaseExp = 25 + aPlayer->get_control_model()->get_military()*3; //the min exp the fleets should ever have
		int OriginalExpWeight = Fleet->get_current_ship()*Fleet->get_exp();
		int NewExpWeight = ChangeFleetSize*BaseExp;
		LostExp = (OriginalExpWeight+NewExpWeight)/FleetNewSize;
	}
