...
//	system_log( "start page handler %s", get_name() );
	...
	switch (MissionID)
	{
		case CMission::MISSION_TRAIN :
		{
			...
			if (FleetExp < CMission::mFleetExpMinTrain) 
							FleetExp = CMission::mFleetExpMinTrain;
			
			Message.format(GETTEXT("After training, each of them will get %1$d exp. Their admirals will get at least %2$d exp., depending on the number and size of ships in each fleet."), 
							FleetExp, 
							CMission::mAdmiralExpMinTrain);
