...
CCouncilActionDeclareTotalWar::feedback()
{
	...
	if (Relation->get_relation() == CRelation::RELATION_TOTAL_WAR)
	{
		...
		mOwner->send_time_news_to_all_member(
				(char *)format(GETTEXT("Total war is over cause %1$s hours have passed."),
								dec2unit(IntervalInHour)));
	}
}
...
{
	return (PERIOD_COUNCIL_BREAK_ALLY * CGame::mSecondPerTurn);
}
...
{
	return (PERIOD_COUNCIL_DECLARE_TOTAL_WAR * CGame::mSecondPerTurn);
}

time_t
CCouncilActionDeclareWar::get_period()
{
	return (PERIOD_COUNCIL_DECLARE_WAR * CGame::mSecondPerTurn);
}

time_t
CCouncilActionImproveRelation::get_period()
{
	return (PERIOD_COUNCIL_IMPROVE_RELATION * CGame::mSecondPerTurn);
}

time_t
CCouncilActionMergingOffer::get_period()
{
	return (PERIOD_COUNCIL_MERGING_OFFER * CGame::mSecondPerTurn);
}
