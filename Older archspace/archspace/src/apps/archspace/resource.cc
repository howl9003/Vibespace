...
CResource::change(EResourceType aResourceType, int aResourceValue)
{
	...
	if (aResourceValue > 0)
	{
	    if (aResourceValue + mResource[aResourceType] > MAX_RESOURCE)
	    {
	        mResource[aResourceType] = MAX_RESOURCE;
	    }    
	    else if (aResourceValue + mResource[aResourceType] < mResource[aResourceType]) // if overflow
	    {
	        mResource[aResourceType] = MAX_RESOURCE;
	    }    
	    else
	...
	{
		if (mResource[aResourceType] + aResourceValue < 0)
		{
			mResource[aResourceType] = 0;
		}
		else
