...
CBountyList::accept_bounty(COfferedBounty* aBounty, CPlayer *aPlayer)
{
	...
	mBounties[newBounty->get_id()] = newBounty;
...
CBountyList::insert_bounty(CBounty* aBounty)
{
	...
	mBounties[aBounty->get_id()] = aBounty;
	return true;
}
...
CBountyList::expire_bounties()
{
	...
	{
		if (current->second->is_expired()) 
		{
			current->second->expire();
...
{
	CBounty *aBounty = mBounties.find(aID)->second;
	if (aBounty==NULL) return;
	aBounty->expire();
	mBounties.erase(aID);
}
...
CBountyList::remove_bounties_on_player(int aID)
{
	...
	for (current = mBounties.begin(); current != mBounties.end(); current++)
	{
		if (current->second->get_target_player()->get_game_id() == aID) 
		{
			BOUNTY_TABLE->remove(current->second->get_id());
		}
	}
}
...
CBountyList::get_bounty_by_player_id(int aID)
{
	...
	for (current = mBounties.begin(); current != mBounties.end(); current++)
	{
		if (current->second->get_target_player()->get_game_id() == aID) return current->second;
	}
	return NULL;
}
