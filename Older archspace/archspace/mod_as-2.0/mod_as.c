...
static int ArchspaceHandlers(request_rec *r);
static int archspace_handler(request_rec *aRequest);
static void *archspace_create_dir_config(apr_pool_t *aPool, char *aPath);
static void *archspace_create_config(apr_pool_t *aPool, server_rec *aServer);
...
static int ArchspaceHandlers(request_rec *r) {
	if ((strcmp(r->handler, "archspace-handler")) ||
		(strcmp(r->handler, "archspace-script")) ||
		(strcmp(r->handler, "application/x-archspace"))) {
		
		return archspace_handler(r);	
	} else {
		return DECLINED;
	}
}
...
module AP_MODULE_DECLARE_DATA as_module =
{
...
util_read(request_rec *aRequest, const char **aRequestBuffer)
{
	...
	if (ap_should_client_block(aRequest))
	{
		...
		*aRequestBuffer = (const char*)ap_pcalloc(aRequest->pool, Length+1);
...
static void*
archspace_create_config(apr_pool_t *aPool, server_rec *aServer)
{
	SArchspaceConfig 
		*ArchspaceConfig;
